...
节点类型固定为 source / agent / gate / sink，未来可能加 join（汇合点）。变化通过 skill（装备卡）承载，不通过加节点类型：
...
- **Sink（出厂码头）：** 成品交付。可以是输出到屏幕、写入数据库、调用 API、发布到平台、喂给另一条产线。
...
- Packet/Artifact 分层正式落地，支持非文本产出
