...
| 功能 | 方向 | 状态 |
|------|------|------|
| MCP Client | agent-world 接入外部 MCP Server，调用外部工具 | ✅ 已实现：三种传输；运维走 `MCP_SERVERS` 环境变量（含 stdio），用户在设置页自助接入远程 http/sse（per-user 连接池、凭证随设置静态加密、保存即试连）。见 [design-skill.md](design-skill.md) §13.1 |
