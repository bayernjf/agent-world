...
| 节点 | 能力 |
|------|------|
| **循环** | for/while，批量处理一组数据 |
| **Webhook 触发** | 接收外部事件触发产线 |
...
- [ ] HTTP 请求节点能调用任意 REST API，支持 GET/POST/PUT/DELETE
- [ ] 代码执行节点能跑 Python/JS，能访问上游数据，能输出结果给下游
- [ ] 条件分支节点能根据条件路由到不同分支
- [ ] 节点间能传递任意 JSON 数据
- [ ] 能用这三个节点搭一个"调用 API → 处理数据 → 条件判断 → 输出"的产线
...
| 能力 | 说明 |
|------|------|
| **并行/聚合** | 并行跑多个分支，然后聚合结果 |
| **子流程调用** | 产线调用另一个产线（类似函数调用） |
| **错误处理** | try/catch、重试策略、降级、死信队列 |
| **AI Agent 节点** | 增强版 agent，支持多轮工具调用循环（ReAct），能自主规划 |
| **人工审批节点** | 产线运行中等待人工输入/审批，然后继续 |
| **状态机** | 复杂业务流程建模 |
| **变量持久化** | 产线运行间共享状态 |
