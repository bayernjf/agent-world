...
> 定位：在现有 2D 编辑画布上，加一个「受限 3D」展示视图（等距俯视 + 水平旋转 + 平移），2D 与 3D 一键切换。**2D 编辑为主，3D 展示 + 可拖节点调整布局**。创建：2026-09-06。
>
...
**边界（明确不做）**：自由视角 3D、数据层改动、3D 内连线编辑、2D 行为变化。**3D 支持拖节点移动布局**（左键拖节点，管道实时重路由，松手写回 graph）。
...
**节点方块**：`BoxGeometry(PLANT_W, 50, PLANT_H)` = `(150, 50, 92)`，中心放 `(worldX, 25, worldZ)`，Y 轴朝上为高度；整组绕 Y 轴旋转 22.5°，锚点同步旋转对齐面中心。
...
| 俯角 | 固定 60°（与垂直轴夹角，即 30° 与水平面夹角） | OrbitControls `minPolarAngle = maxPolarAngle = Math.PI/3` |
...
| 编辑 | 左键拖节点移动布局（管道实时跟随，松手吸附网格）；连线仍只读 |
...
**第二期（完整效果，2026-09-06 完成）**：

1. 29 种程序化几何造型（双点医院式示意）
2. 卡车沿 3D 管道跑（复用 `PacketRuntime`，独立 rAF，不动 2D `PacketLayer`）
3. 运行状态亮灯（质检/返工/失败）

**第三期（交互收尾，2026-09-06 完成）**：

1. 3D 点击节点弹 Inspector（位移 >4px 判拖拽，点击才展开）
2. 水平旋转多输入源（滚轮上下 + 方向键 ←/→ 也转）
3. minimap 缩放滑块
4. minimap 遮罩拖动修复（钳 rect 中心、对称溢出）
5. 3D 遮罩拖动 = 平移（target+position 同步，遮罩框跟随光标）
6. 3D 遮罩区滚轮缩放画布
7. 3D 适应按钮真 fit

**第四期（视觉打磨 + 节点拖动，2026-09-06 完成）**：

1. 默认 RTS 等距视角（左后上方 30° 俯角，帝国时代风）
2. 适应时对齐 2D 布局（按图宽高比选 yaw，长边水平）
3. 节点体积感（高度 50 + 多面材质 + 左后上方光照）
4. 节点绕 Y 轴旋转 22.5°，锚点同步旋转对准面中心
