...
- [ ] 产线列表页：显示所有 graphs（`GET /api/graphs` 已有）
- [ ] 新建产线：空白或从模板创建
- [ ] 复制产线：深拷贝 graph，新 id
- [ ] 重命名、删除
- [ ] 产线切换不丢失编辑状态
- [ ] 退出条件：workspace 里能管理多条产线
...
- [ ] `AgentConfig` 加 `budgetUsd?: number`
- [ ] 引擎在节点成本超限时停该节点（不是整条线）
- [ ] 前端节点上显示该节点预算和已花费
- [ ] 退出条件：单个厂房超预算不影响其他厂房
...
- [ ] 运行历史列表页（分页、按产线筛选、按状态筛选、按时间排序）
- [ ] 运行详情：回放任何历史运行（已有 replay 能力，需加 UI 入口）
- [ ] 运行对比：选两次运行对比结果和成本
- [ ] 删除运行记录
- [ ] 退出条件：能找到上周跑的任何一次运行并回放

### 3.2 成本报表

- [ ] 按产线聚合成本：总花费、平均每次成本、趋势图
- [ ] 按节点聚合：哪个厂房最费钱
- [ ] 按 attempt 聚合：返工花了多少冤枉钱
- [ ] 按时间聚合：日/周/月成本
- [ ] token 用量细分：input/output/cached
- [ ] 导出 CSV
- [ ] 退出条件：回答"钱花在哪了"一目了然

### 3.3 断线重连

- [ ] 前端 SSE 断开后指数退避自动重连（现在静默停住）
- [ ] 重连时带 `Last-Event-ID` header，从上次 seq 续传
- [ ] 重连中 UI 显示"重连中…"状态
- [ ] 服务端 SSE 已支持从 seq 续传，验证边界情况
- [ ] SSE 心跳验证：确认阶段 1 加的 ping 帧在真实代理下保持连接
- [ ] 退出条件：跑十几分钟的产线中途断网，恢复后状态正确、事件不丢不重

### 3.4 结构化失败面板

- [ ] 失败面板取代顶部红横幅
- [ ] 显示：失败节点、错误码、错误信息、发生时间、影响范围
- [ ] 一键操作：重试该节点、返工到上游、报废整条线
- [ ] 失败历史：本次运行中所有失败记录
- [ ] 退出条件：失败后知道该点哪、做什么

### 3.5 数据库与并发加固

- [ ] **schema_version 表 + 有序迁移**，取代现在 try/catch ADD COLUMN 的方式，支持回填、多步迁移、跨版本升级
