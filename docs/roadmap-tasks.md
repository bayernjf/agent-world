...
- [x] 内置模板扩充至 27 个业务模板 + 1 个空白产线入口（BLANK_TEMPLATE 独立导出，不计入模板数）。分类已收口为 core `TEMPLATE_CATEGORIES` 有序 11 类（选择器按此分组展示，空白钉最前）：营销内容 6（淘宝详情/小红书/短视频/批量工坊/人工审核发布/资讯播客）、数据分析 5（运营周报/多源简报/竞品监控/深度调研/数据报表）、写作 2（草稿/翻译）、办公协同 3（文档审查/智能解析/扫描OCR）、开发集成 3（自定义模型/发版PR/代码审查）、法律合规 2（合同审查/证据清单）、财务审计 1（费用报销初审）、IT 运维 1（巡检告警）、客户服务 1（工单自动处理）、教育 1（课程大纲）、生活 2（旅游行程/菜谱）。收并过程：`开发`→`开发集成`、孤儿`审查`→`办公协同`，原「研究/内容生产/文档处理/工作流」四类并入上述同族。2026-09-01 新增 7 个模板（客服工单/代码审查/数据报表/合同审查/课程大纲/旅游行程/菜谱），模板节点覆盖增至 25 种节点类型中的 23 种（database / subprocess 仍无模板），branch/notify/table/fileParse 等 12 种节点从单点覆盖变为双点覆盖；2026-09-01 再增证据清单整理（法律合规第二模板：拆条编号→时间索引→清单起草→缺口分析→质检，引擎级冒烟覆盖）；2026-09-01 再增费用报销初审（财务审计首个模板：code 规则校验超额/重复单号/日期异常→table 异常清单→初审报告→质检，引擎级冒烟覆盖）
- [x] 模板选择器按分类分组展示（区块标题 + 数量徽标，空白卡片钉在所有区块之前；分类清单单一事实源在 core，见 design-templates §6）
- [ ] 模板预览图
...
- [x] database connector：SQLite 只读 SELECT 查询，json/csv 输出（2026-09-01，见 design-connector-database.md；PostgreSQL 2026-09-08 已落地，MySQL 见 deferred）
...
- [x] 示例产线集（docs/examples.md，12 条示例：改写循环/商品生成/多源聚合/视频广告/表单驱动/A/B 测试/定时报告/webhook 触发/运营周报/巡检告警/多源简报/竞品监控，后 4 条标注对应内置模板 id，其余需手工搭建；文档开头另有 27 个内置模板的分类导览表）
...
- [ ] 真实计费与配额（事后计量是地基）
- [ ] 按用量计费 / 订阅套餐 / 企业定制（模型分层设计见 product-vision-discussion §九「模型分层与商业化」——内置模型订阅制只读选择，用户自定义模型自主增删改）
- [ ] 月度账单和用量预警
- [ ] 团队级预算
