...
- **L25**（topoSort O(V·E)）：纯性能优化，单机阶段图规模小（<50 节点），O(V·E) 毫秒级，无感知延迟 → 暂缓
- **L26**（TriggerConfig 约束）：运行时已有兜底（非法触发器配置静默跳过），加严格编译期校验有历史数据风险（旧图可能含宽松配置），迁移成本 > 收益 → 暂缓
- **L27**（rework body 祖先）：确认为设计意图——rework 循环体包含从入口到 gate/agent 的整条正向路径上的所有祖先节点，这是 rework 语义的正确实现（返工需要重新执行整条链路），非 bug → 暂缓
