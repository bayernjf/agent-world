...
| 审计日志（audit_log） | 方案已定稿（独立表 + 路由层埋点 + detail 红线：永不含值）；当前单人自用无「谁在何时改了什么」的取证诉求 | 对外/多用户部署启动合规准备时，或出现账户异常需要回溯时 | [design-audit-log.md](design-audit-log.md)（2026-09-05 定稿） |
...
| 服务端日志收编与默认落盘 | 方案已定稿（console 收编 + DB 同目录默认落盘 + 请求中间件）；当前 8 处 Logger 调用 + run 事件表可支撑日常，排障缺口在极端场景才暴露 | 真实排障需求中再次出现「日志缺失定位困难」（引擎/节点执行路径无结构化日志） | [design-logging.md](design-logging.md)（2026-09-05 定稿） |
