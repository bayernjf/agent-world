...
| 🔐 ~~**节点级 `apiKey` 明文落库**（imageGen / videoGen / audioGen）~~ **已修复 2026-09-02（`f7c333f`）——未等触发条件，登记当轮就修了** | ~~审计 L3 的静态加密只盖了两类位置~~ → 实际范围比登记时更广：除三个媒体节点的 `apiKey`，还有 generic 节点 `apiKey`、notify `secret` 与 `webhookUrl`（群机器人 URL 路径里就嵌着 token）、source 连接器的 `http.auth.token` 与 auth 类 `headers`——全部曾以明文进 sqlite 图 JSON、随版本快照留存、也能随图导出被带走。修法：`sealGraphDoc`/`openGraphDoc` 改为**按字段名递归遍历**（而非硬编字段路径），同时保留：无前缀旧明文直通（lazy 迁移）、已加密值不重复包裹（幂等）、字段顺序不变 + 无凭证图返回同一引用（明文 content hash 可比，“与运行版本一致”标记不坏）。**已声明的边界**：~~藏在未列入名单的自定义 header 名（如 `X-My-Auth`）里的凭证仍拦不到~~ → **已收口 2026-09-02（`ff223bb`）**：header 名由用户自定、名单枚举不可能穷尽，改为在 `headers` 记录内按**名字模式**（auth/token/key/secret/credential/signature/password/session/cookie/bearer）加密其值，良性 header 仍明文可排查、只含良性 header 的图仍返回同一引用（sealer 四条性质不变，db 集成用例直读四处原始字节取证）；**新残留边界**：http 节点/连接器 `url` 查询串里内嵌的凭证（`?token=…`）仍明文落库（按字段名无法区分带 token 的 URL 与普通 endpoint） | ~~开多用户或对外部署之前必须完成~~ → 已完成；剩下的同族事项是"搜索/VCS 节点级凭证入口"（见集成线）与 URL query 凭证 | [security-audit-2026-08-31.md L3](security-audit-2026-08-31.md) + `packages/server/src/at-rest.ts` `SECRET_KEYS`/`AUTHISH_HEADER`/`mapSecrets` + [design-at-rest-encryption.md](design-at-rest-encryption.md) |
