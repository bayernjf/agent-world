...
| 事项 | 缓做/低优原因 | 触发条件 | 决策详情 |
|---|---|---|---|
| 多租户/权限/团队协作 | **基础 RBAC 方案已定稿 2026-09-05（见 [design-rbac.md](design-rbac.md)，含全局 owner/admin/user + 资源级 editor/viewer 共享 + resource_access 表）**；账号隔离（owner 判定）与共享网格的底层就位，但"团队协作"本身（第二个真实协作者协作同一个图）仍是抢跑 | 出现团队共用同一产线的真实需求（RBAC P1 越权基线改造随之全量落地，届时瓶颈从"能不能共享"转为"要不要共享出安全事故面"） | [design-rbac.md](design-rbac.md)（2026-09-05 定稿）
...
| ~~公告（announcements）~~ **P1+P2 已实施 2026-09-05**（迁移 30 + GET/read/管理 API（env 白名单）+ 铃铛下拉/warning 横幅/critical 模态 + 专项测试），剩余 P3（target 定向逻辑） | P3 未做：首期只有全员公告，定向列已建但无消费场景 | target 定向：出现「某模板/图废弃要通知到相关用户」的真实场景 | [design-announcement.md](design-announcement.md)（P1+P2 已实施，P3 待触发） |
