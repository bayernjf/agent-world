...
**实施后追加的词表**（RBAC P3 与用户反馈线）：`role.update`（detail `{grantee, role}`，owner 专属操作）、`access.grant` / `access.revoke`（graph ACL，detail `{resourceType, resourceId, grantee, role}`）、`announcement.create` / `update` / `delete`、`feedback.submit`（detail 只含 `{category}`——**不含 message 正文**，正文属用户内容非取证要素，且避免敏感叙述进审计表）、`feedback.status_change`（detail `{to}`，同状态幂等不写）、`feedback.announce`（反馈→公告联动，detail `{count, level}`，objectType=announcement、objectId=新公告 id）。
