...
- **写 `compliance` 模板时，`extraBanned` 是「补词表」不是「保险」。** 两处同日真机狗粮暴露的坑（`cd6371f` / `78e7811`）：① 广告法词表（`AD_LAW_BANNED_WORDS`）本身含**嵌套对**（`第一` 与 `第一品牌` 相邻），而 `autoFix` 曾按原串算命中区间、却逐个往已变长的结果串上套，于是后一个命中的偏移切进前一个的替换结果里——长短不等时产出「（已删除）秀的」这种没法上架的文本，恰类等长时输出「领先品牌」读着没问题，所以长期没人发现。已改为「区间全部锚在原串、一趟拼出、重叠取长词」，`platforms.test.ts` 两条用例先证红再修。② 别把词表已经管的词再抄进 `extraBanned`（`tpl-compliance-precheck` 首版 8 条里 6 条重复），尤其**不要写裸字**：`最` 会命中「最近」「最多」，把正常用法洗成删除标记。`templates.test.ts` 有守护：每条补充词必须 ≥2 字、必须真查得到、且**不能本来就在词表里**。
