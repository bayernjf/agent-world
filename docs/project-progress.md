...
| 模板体系 | 97% | 🟡 主体完成 | 31 个实用模板覆盖 25 种节点类型中的 23 种（database / subprocess 无模板）；分类收口为 core `TEMPLATE_CATEGORIES` 有序 11 类，选择器按分类分组展示、空白钉最前（法律合规「合同审查」+「证据清单整理」+「隐私政策合规审查」+「批量合同审查」、财务审计「费用报销初审」+「银行流水对账」+「发票批量 OCR 台账」）+ TemplateField 参数化全链路 + 空白产线入口（BLANK_TEMPLATE 独立导出，不计入模板数）；模板市场（发布/安装）缓做 — [design-templates.md](design-templates.md) |
