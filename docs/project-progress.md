...
| 回归测试与质量门 | 100% | ✅ 已完成 | core 188 / server **838** / mcp 50 / **web 1542**（web 2026-09-03 从 176 提升到 1460，+1284 用例——组件测试全覆盖 39 个组件，P0/P1/P2/P3 四批全部完成；基础设施 @testing-library/react + jsdom + vitest.config.ts + setup.ts + utils.tsx；过程中修复 Inspector.tsx 可选链 bug；全量稳定通过无回归；**2026-09-05 合规运营批次（审计日志/服务端日志/RBAC/公告/用户反馈）后 server 747→838、web 1500→1542**）；core-path 回归基线 17 用例，Node 24 下稳定复跑 — [handoff.md Quality gate](../handoff.md) |
...
| 合规与运营能力（RBAC / 审计 / 日志 / 公告 / 反馈） | 90% | 🟡 主体完成 | **2026-09-05 合规/运营批次落地**：RBAC P0-P3 全量（全局 owner/admin/user + 资源级 owner/editor/viewer 共享 + `resource_access` 表 + `permissions.ts`/`rbac.ts` 判定层替换裸 user_id 隔离做越权基线改造 + CollaboratorsModal 共享 UI + AdminPanel 用户管理/跨用户审计；owner=最早注册用户自动提升）；审计日志 P1+P2（append-only `audit_log` 表 + `audit()` helper 全动作词表埋点 + `GET /api/audit`，detail 只记字段路径永不含值红线）；服务端日志 P1+P2+P3（裸 console 收编 Logger + 默认落盘 + `/api/*` 请求中间件 + 启动/迁移/触发器关键路径）；公告 P1+P2（双语表 + level 驱动 UI + 管理 API + AnnouncementBell）；用户反馈 P1+P2（`feedback` 表迁移 33 + 提交/管理 API（服务端上下文白名单二次脱敏 + 每用户滚动小时 10 条限流 + 截图 ≤1MB）+ FeedbackModal（截图粘贴 + 诊断勾选）+ UserMenu 入口 + AdminPanel 反馈 tab）。**密钥轮换方案定稿未实施**（触发：合规准备启动）；各 P3 项按 deferred-items 触发条件推进 — [design-rbac.md](design-rbac.md) · [design-audit-log.md](design-audit-log.md) · [design-logging.md](design-logging.md) · [design-announcement.md](design-announcement.md) · [design-feedback.md](design-feedback.md) |
