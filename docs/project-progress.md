...
| 回归测试与质量门 | 100% | ✅ 已完成 | core 182 / server 731 / mcp 50 / **web 1500**（2026-09-03 从 176 提升到 1460，+1284 用例——组件测试全覆盖 39 个组件，P0/P1/P2/P3 四批全部完成；基础设施 @testing-library/react + jsdom + vitest.config.ts + setup.ts + utils.tsx；过程中修复 Inspector.tsx 可选链 bug；全量稳定通过无回归）；core-path 回归基线 17 用例，Node 24 下稳定复跑 — [handoff.md Quality gate](../handoff.md) |
...
8. **i18n 国际化** —— 🟡 收尾阶段（2026-09-03）。i18next + react-i18next + 9 命名空间 + 完整 zh/en 双语翻译包（1800+ keys）+ 语言自动检测 + localStorage 持久化全部落地，全量 1499 测试无回归。**组件迁移已完成（41/41 组件 + 顶层 App.tsx）**：全部业务组件迁到 useTranslation；顶层 App.tsx 的 commandItems/hud/确认弹窗迁入 common.app + modals.commandPalette.commands；Inspector 通用 chrome + 表格操作 + 错误码 + 运行时 UI 迁入 nodes.inspector；语言切换 UI（LanguageSwitcher 集成 UserMenu）已落地；keys.test.ts 守护 zh/en 对齐。**剩余**：Inspector 内 25 种节点配置字段（约 250 处）+ 本地化格式 i18n/utils.ts — [design-i18n.md](design-i18n.md)。
