...
| L8 | 前端细节：未提交改动把"全选 / 清空"按钮改成"全选"但 `toggleAll` 仍是切换行为，文案与行为不符；`routingWorker` 对缺模态的 provider 静默返回空结果（有注释与测试，但会掩盖配置错误） | `ModelAssignModal.tsx:208-210`、`providers/index.ts:68-80` |
