...
> 状态：**方案定稿待实施（2026-09-05），方案移至 [design-data-interpolation.md](design-data-interpolation.md)**。源起：F4 落地后复查发现 source 简报字段与商品库数据是「双来源文本拼接」，双填优先级无从推理；`graph.ts` ProductConnector 注释宣称的「字段级映射」与实现不符。
>
...
### 现实约束（先讲清楚，避免踩坑）
淘宝/小红书/抖音**没有面向中小商家/个人的免费内容发布开放 API**：官方开放平台多需企业/商家资质且接口范围受限；第三方模拟上传属于灰色地带、有封号与合规风险。因此不能承诺"一键自动发到小红书"。按可落地程度分阶段：

> 注意先拆分两个场景：**「电商上架」**（淘宝/Shopify 商品上架）有正规 API 路径可走；**「内容发布」**（小红书笔记、抖音短视频）才是真正没有官方 API、只能人工或半自动 RPA 的战场。两者风控与出路完全不同，勿混为一谈。

**阶段 A（本方案主推，零风险）——平台化导出包 + 人工发布**
- 新增 `publish` 节点（NodeKind，归 integrations）：按选定平台 profile（复用 F3）把上游产物整理成"待发布包"——标题、正文、话题标签、按比例裁好的图片清单、一键复制按钮 / 下载 zip；输出 `uri`/`json` artifact 并标记 `ready_to_publish`。
- 价值：人工到各平台粘贴时零排版成本（全选复制→粘贴→发布，1 分钟内完成），且保证合规（与 F3 联动）。
- **闭环增强（发布后回写）**：发布后标记 `published`，回写内容日历（F8 状态）与效果表（F6 的 `published_at`），让人工发布也纳入 ROI 闭环，不因"人工"而断链。
- 工作量：小。

**阶段 B（对接正规 API 渠道，上架为主）**
- 优先支持有正规 API 的：**Shopify/独立站商品上架**、微信公众号草稿箱、企业飞书/钉钉审核流、Webhook 到自建中台。
- 抽象 `Publisher` 适配器接口（provider/action/token，模式对齐现有 vcs/notify，token 走同一套加密），新增 `publish_targets` 表存渠道配置。
- 价值：面向有企业资质/独立站的商家，"上架"这半条路真正自动化，零合规风险。
- 工作量：中（每个适配器独立增量）。

**阶段 C（半自动 RPA，可选高级功能，读数据优先）**
- 定位：**只做「半自动」，不做「全自动无人值守」**——RPA 帮你打开发布页、填好标题/正文/话题标签、挂好裁好的图，**停在「发布」按钮前由人手动点最后一下**。省掉"复制粘贴排版"的 80% 机械量，又保留真人介入以大幅降低封号风险。
- 硬约束：顶部常驻封号风险提示 + 用户自备账号 + 限速；**默认不做全自动灰产发布**（商家的账号是比工具更贵的资产）。
...
### 工作量
A 小 / B 中 / C 读数据回传中、半自动发布大（带风险，持续维护）。
...
### 合规采集策略（数据回流分级，2026-09-04 补）

**原则：效果数据回流只做「读」，不做「写」；采集手段按合规程度分级，高危做法一律不用并记录原因。**

| 分级 | 做法 | 合规性 | 说明 |
|---|---|---|---|
| ✅ 首选 | **Webhook 回流**：商家自建中台 / 第三方数据工具（蝉妈妈/飞瓜等）推送到 `/api/metrics`（已预留） | 完全合规 | 数据由商家自己授权、自己推送，平台 ToS 无涉 |
| ✅ 首选 | **官方开放 API**：平台官方数据接口（有企业资质时） | 完全合规 | 走官方授权，零风险 |
| ✅ 兜底 | **手动录入 / CSV 导入**（现状） | 完全合规 | 零自动化，但零风险 |
| 🟡 谨慎 | **RPA 回读后台**（Playwright 读创作者后台曝光/点击/GMV） | 灰色但风险可控 | 只「读」不「写」，风控风险远低于自动化发布；需用户自备账号 + 限速 + 常驻风险提示 |
| ❌ 禁用 | **RPA 全自动发布**（无人值守上传内容） | 高危 | 平台 ToS 明令禁止自动化上传，封号风险高 |
| ❌ 禁用 | **第三方模拟上传 / 绕过风控 / 批量小号** | 违规 | 灰产，有封号与法律风险，绝不提供 |

**为什么不用高危做法（记录原因）：**

1. **RPA 全自动发布**：平台（小红书/抖音/淘宝）ToS 明令禁止自动化上传，风控（设备指纹/行为检测/滑块验证）就是冲着机器人来的；全自动无人值守是典型灰产行为模式，一旦触发风控，封的是商家的号——比工具贵得多的核心资产。因此「写」只做半自动（自动填表 + 停在发布键由人点，见 §F7-C），只有「读」才允许 RPA。
2. **第三方模拟上传 / 绕过风控 / 批量小号**：属灰产，有封号与法律合规风险，任何情况下都不做、不提供、不引导——即使商家主动要求，也明确拒绝并说明原因。
