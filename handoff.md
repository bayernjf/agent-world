...
38. 🔵 **商业化 M0：本地运行环境部署（2026-09-07 立项，部署已就绪，剩验收）**：把 agent-world 部署成可 7×24 跑真实产线的**单机服务**（Ubuntu 纯 Server 笔记本 / Node 24 / systemd 托管 server / nginx 同源托管 web / `CODE_SANDBOX=bwrap` 关审计 H4），作为商业化 P0 成本计量回采的运行床。挂 [design-monetization.md](docs/design-monetization.md) §8.13 里程碑 M0。文档：[deploy-ubuntu-execution-plan.md](docs/runbooks/deploy-ubuntu-execution-plan.md)（阶段 0-7 方案）+ [deploy-ubuntu-execution-log.md](docs/runbooks/deploy-ubuntu-execution-log.md)（逐步执行记录，敏感信息一律占位符）+ [deploy-ubuntu-server.md](docs/runbooks/deploy-ubuntu-server.md)（命令手册）。当前进度：**阶段 0-6 已全部完成**（免密 A1-A4 / Node 24 / bwrap / nginx / agentworld 用户 / mask suspend / rsync+构建 / .env / systemd active / nginx 反代 / 备份 cron 均就绪；`AGENT_WORLD_ENV=staging` 已注入 `.env`，health 返回 `env:staging branch:dev commit:4409494`）；**剩阶段 7 商业验收**（8 项：注册 / 配 AGNES key / 建产线跑通 / 成本计量 / cron 触发 / code 沙箱 / subscriptions 表 / 备份恢复，需用户在浏览器配 key + 跑产线）+ **CI/CD deploy.yml 待 push 合并到 dev 后生效**（runner 已 online，基础设施就绪）。
...
1. **fix(web) 切换产线被 409 静默阻断（2026-09-08，未提交）**——根因三连：① `scheduleSave` 自动保存不带 If-Match 且成功后不回写 `serverVersion`，服务端 upsert 已 `version+1` 而本地仍是旧值；② 切换产线时 `flushSave` 拿过期 `If-Match` 发条件 PUT，服务端 `updateGraphIfVersion` 匹配不到行必返 409（只要编辑过一次就 100% 复现）；③ 8b8969b 给 `flushSave` 加了 rethrow（M22），但 `switchGraph` 的 `await flushSave()` 在 try/catch 外，409 直接把切换打断且无任何提示。修复：自动保存成功后回写 `serverVersion`；新增 `enqueueSave` 保存队列串行化 autosave 与 flush（消除并发双 PUT 竞态）；`switchGraph` 捕获 flush 失败 → toast 报错并中止（保住未保存修改，不再静默）。+3 回归用例（graph.save.test.ts），web 1571→1574。

2. **fix(server) PG 演练方言修复 9 类（2026-09-08）**——Docker postgres:16 + dev 库（25 表/12.2 万 events）端到端搬迁演练：行数全对齐、71 图 doc 逐字节一致、enc:v2 密文可解密、PG 冒烟（注册 bootstrap owner/登录/建图/更新 version 1→2/版本列表/自动快照）全通。演练抓出并修复 9 类静态评审漏掉的方言缺口：① DDL 缺 `artifacts.graph_id/role`+`graphs.origin_template_id`（fresh PG 库缺列）；② toPgDdl 不识 BLOB（feedback.attachment→bytea）；③ `rowid` tie-break ×7（PG 无 rowid→参数化 ctid）；④ `LIMIT -1`（PG 报错→LIMIT ALL）；⑤ `date(...,'unixepoch','localtime')` 日桶（→to_char(to_timestamp)）；⑥ `INSERT OR IGNORE`（→ON CONFLICT DO NOTHING 两边通用）；⑦ upsert `SET version=version+1`/`amount=amount+excluded.amount` 列名歧义（PG 42702→限定表名）；⑧ node-pg bigint→string（countOwners===0 恒 false、Date.now()-created_at 得 NaN→setTypeParser(20,parseInt)）；⑨ PG 别名小写折叠（`AS graphId`→`graphid` 行形状破坏 ×7→加双引号）。+1 用例（pg-sql BLOB→bytea），server 928→929。详见 design-postgres-migration.md §8。

3. **feat(server) PostgreSQL 阶段 3：DB_DRIVER 开关 + 搬迁脚本（2026-09-08）**——① `db.ts` `openDatabase()` 按 `DB_DRIVER` 分派（默认/空值=sqlite；未知值 fail-closed；PG 连接 `DATABASE_URL` 或 `PG_HOST`+`PG_DATABASE`+可选 `PG_PORT/PG_USER/PG_PASSWORD/PG_SSL`）+ `index.ts` 启动接线 + driver 暴露 `kind`；② FTS 知识库 PG 下诚实降级 `NoopMemoryBackend`（空结果+启动警告），`backfillExistingData` 仅 sqlite 执行；③ `migrate-to-postgres.ts`（`pnpm --filter @agent-world/server migrate:postgres`：VACUUM INTO 快照→toPgDdl 建表→流式批量 INSERT→行数校验，`--dry-run`/`--verify-only`）；④ **修复 DDL 契约缺口**：`resource_access`/`subscriptions`/`usage_ledger`/`idempotency_keys` 4 表只在迁移 32-35 建、DDL 常量缺失（fresh PG 库会缺表），已补 DDL。+9 用例（开关 5 + 搬迁 4），server 919→928。
3. **fix 审计修复 25 项收尾（2026-09-08）**——高危 8 项清零 + 本轮 25 项：server L5/L6+M1/M3/M5（L5 graph_variables 加密、L6 clientIp trusted proxy、M1 abort 挂起、M3 产物 id 前缀、M5 文本产物事件）、mcp 8 项（M15-M19+L15/L16/L18）、core M31-M36+L23/L24（M36 旅行模板目的地字段）、web 4 项（M21/M22/M24/M27）。审计报告 61/77 修复，剩 13 项 low 级暂缓（Canvas3D 竞态/性能/历史数据风险/设计意图）。
4. **chore(ci) dependabot 依赖治理（2026-09-08）**——① 关闭 zod 4 major 升级 PR #182（`z.record` 单参数移除 + `z.infer` 推断变化，致 ~1300 处连带类型错误），`.github/dependabot.yml` 加 ignore zod major，迁移登记 deferred；② 合并 4 个依赖 PR（jose 6.2.11 / nodemailer 10 / i18next 26.4.2 / hono 4.13.7，#178-#181，解决 hono 的 lockfile 冲突）。server 911→912。
5. **feat(ci) SAST（CodeQL）（2026-09-08，P1 安全）**——`.github/workflows/codeql.yml`（CodeQL 周度 + push/PR 触发，结果进 Security tab）。
7. **docs 运营三件套（2026-09-08，P1 运营流程）**——postmortem 模板（`runbooks/postmortem-template.md`）+ SLA/SLO 定义（`production-ops.md §7`）+ 变更管理（`runbooks/change-management.md`）。
8. **feat(server) 数据归档 + 一致性校验（2026-09-08，P1 数据域）**——`pruneOldEvents` + `scripts/prune-events.ts`（清理 90 天前 events，snapshot 保留完整状态）+ `db.verifyIntegrity()`（PRAGMA integrity_check）。server 909→911。
9. **feat(ci) 依赖漏洞扫描（2026-09-08，P1 安全）**——CI 加 `pnpm audit --audit-level=high` 门禁 + `.github/dependabot.yml` 周度自动 PR；顺带修 glob 高危 CVE（overrides 强制 >=10.5.0）。
10. **feat(devx) pre-commit hooks（2026-09-08，P1 DevEx）**——husky + `.husky/pre-commit` 跑 `pnpm typecheck`（提交前拦截类型错误；未引入 lint/format 工具，避免全量格式化大改动）。
11. **feat(server) 覆盖率门禁（2026-09-08，P1 测试工程）**——`@vitest/coverage-v8` + `test:coverage` 脚本 + 阈值门禁（lines 75 / stmts 72 / funcs 74 / branches 62；基线 79.3/76.7/78.4/67.3，防退化不强制提升）。
12. **feat(server) migration 回滚（2026-09-08，P1 发布工程）**——`Migration.down` 可选字段 + `rollbackLatestMigration` + `scripts/migrate-down.ts`；纯 DDL 迁移写 down，无 down 的迁移回滚拒绝（不猜）。server 908→909。
13. **feat(server) 幂等审计（2026-09-08，P1 可靠性）**——建 run 幂等：`Idempotency-Key` header + `idempotency_keys` 表（迁移 35），重复提交返回同一 runId（`replay:true`），堵「双击运行重复烧钱」。server 907→908。
14. **feat(server) feature flag 灰度开关（2026-09-08，P1 发布工程）**——`feature-flags.ts`（`FEATURE_FLAGS` 注册表 + `isFeatureEnabled`，未知 flag fail-closed）+ `AppConfig.featureFlags`（per-user 覆盖默认）；首个 flag `rpa-metrics`（合规风险默认关，使用点待 RPA 接 API）。server 904→907。
15. **feat(server) Metrics（RED）+ /metrics 端点（2026-09-08，P1 可观测性）**——`metrics.ts` 零依赖内存聚合（counter/gauge/histogram + Prometheus 文本）；HTTP 埋点（requests/errors/duration）+ run 埋点（total/failed/cost/active，terminal 判定去 halted）；`GET /metrics` 无鉴权端点；顺带修复请求日志中间件注册顺序（health/auth 此前不经过）。server 899→904。
16. **feat(core+server) PostgreSQL database connector（2026-09-08）**——`DatabaseConnector` 加 `driver:"postgres"` + `host/port/database/user/password/ssl`（core `graph.ts`）；`connectors.ts` 分派 `queryPostgres`（异步 `pg` 连接 + SELECT 白名单 + 会话级 `default_transaction_read_only=on` 只读双保险）；密码对齐静态加密（`at-rest.ts` `SECRET_KEYS` 加 `password`）；MySQL 预留扩展点（enum 加 `"mysql"` 即可）。server 896→899、core 188→190。
17. **P1 优雅关闭 + 优雅启动（2026-09-08）**——① 优雅关闭：`index.ts` 监听 SIGTERM/SIGINT → `server.close()` 停新请求 → drain 在途 run（`AGENT_WORLD_SHUTDOWN_GRACE_MS` 默认 10s，超时 abort）→ 关 DB → `disposeIsolatedWorkers` → exit（避免 `systemctl restart` 硬杀 running run）；② 优雅启动：`/api/health` 的 `ok` = DB/encryption/JWT 关键检查全通过，未就绪返回 503（readiness gate）。冒烟实测 SIGTERM 100ms 内 drain 完成。
18. **docs+ops P0 八项全部收尾（2026-09-08）**——① 备份恢复演练：`restore-agent-world-drill.sh`（恢复→校验→临时 server 启动→清理），实测 RTO<1min / RPO<24h，见 [deploy-ubuntu-server.md §六之一](docs/runbooks/deploy-ubuntu-server.md)；② gitleaks 扫全历史 1081 commits 无泄露 + `pnpm audit` 无漏洞；③ `.env.example`（60+ 变量分类 + 占位符，见 [production-ops.md §3](production-ops.md)）；④ 一键回滚：deploy.sh 记 `.last-known-good` + 健康检查 + `rollback.sh`（`scripts/deploy/`，已落地 Hasee，见 [deploy-cicd.md §4.4.1](docs/runbooks/deploy-cicd.md)）；⑤ E2E 冒烟 + 一键启动：`smoke.test.ts`（HTTP 核心链路 health→注册→建产线→跑→done，fake 模型离线可跑）+ `scripts/dev.sh`（Node≥24 检查 + install + typecheck + dev）；⑥ `scripts/healthcheck.sh` 环境体检脚本（服务/配置/依赖四层一键判定）。P0 八项全部完成。
