...
9. ✅ **设计 Token 体系完善（2026-09-03 立项，基础设施已完成）**：当前只有 26 个基础 CSS 变量（颜色/字体/层级），缺失间距/圆角/阴影/字号/动画等基础 token，无语义化层，不支持明暗主题切换。方案见 [docs/design-design-tokens.md](docs/design-design-tokens.md)。**基础设施已落地（commit `9259a38`）**：① 补充完整 Primitive token 层——间距 12 级（8pt grid）、圆角 7 级、阴影 6 级、字号 8 级、行高 4 级、字重 4 级、动画 3 级（duration + easing）；② 新增 Semantic token 层——背景 7 角色、文字 5 角色、边框 5 角色、功能色 4 组（success/warning/error/info 含 bg 变体）、accent 交互色 4 角色、语义间距/圆角/阴影各 4-5 角色；③ 明暗主题切换——`[data-theme="light"]` 属性驱动，所有 semantic token 完整映射浅色值，滚动条颜色同步；④ 保留原有 26 个原始 token 向后兼容，semantic token 映射到 primitive，主题切换单属性即可。**剩余渐进式迁移**：组件逐步替换硬编码值为 semantic token（基础组件 → 画布 → 弹窗 → 节点 → 清理），优先级 P1
10. ✅ **i18n 国际化（2026-09-03 立项，基础设施已完成）**：当前无任何 i18n 基础设施，所有 UI 文本硬编码中文，无语言切换，无本地化格式。方案见 [docs/design-i18n.md](docs/design-i18n.md)。**基础设施已落地（commit `008c844`）**：① 安装 i18next + react-i18next（运行时）+ i18next-parser（开发工具）；② i18n 初始化——7 个命名空间（common/canvas/nodes/modals/settings/run/errors），语言自动检测（localStorage > 浏览器语言），变更时持久化 + 同步 document lang 属性；③ 完整中文（zh）翻译包——common 120+ keys、canvas 90+ keys、nodes 80+ keys、modals 500+ keys（25 种弹窗类型）、settings 200+ keys、run 200+ keys、errors 400+ keys（6 大类）；④ 完整英文（en）翻译包——与 zh 同 key 覆盖，全部 UI 文本已翻译；⑤ main.tsx 引入 i18n 初始化，TypeScript 编译通过，全量 1460 测试无回归。**剩余渐进式迁移**：组件逐步用 `useTranslation()` hook 替换硬编码中文（预计 200+ 处），添加语言切换 UI，本地化格式（日期/数字/货币/相对时间），优先级 P1
...
1. **feat(web) 设计 token 体系基础设施 + i18n 国际化基础设施（2026-09-03）**——① 设计 token（`9259a38`）：补充完整 Primitive 层（间距 12 级/圆角 7 级/阴影 6 级/字号 8 级/行高 4 级/字重 4 级/动画 3 级）+ Semantic 层（背景/文字/边框/功能色/accent/语义间距圆角阴影）+ 明暗主题切换（`[data-theme="light"]`），保留原有 26 个 token 向后兼容；② i18n（`008c844`）：安装 i18next + react-i18next + i18next-parser，7 命名空间初始化（common/canvas/nodes/modals/settings/run/errors），完整 zh/en 双语翻译包（1800+ keys），语言自动检测 + localStorage 持久化，全量 1460 测试无回归
2. **test(web) P3 组件测试全部完成（19 组件/652 用例，2026-09-03）**——ProtectedRoute/Timeline/TemplateFieldDialog/SkillPicker/FormConnectorModal/NewGraphDialog/UserMenu/Onboarding/BrandTermsModal/Popover/ShortcutsHelp/SourceImages/AccountDialog/GraphSwitcher/VariablesModal/ABDialog/AuthPages/SourceFiles/RunCompare，web 测试总数 176→**1460**，组件测试覆盖 39 个组件/1223 用例，全量 56 文件稳定通过无回归
