...
> - **① M1 四产线 09-14 降频后观察小结（⛔ 卡在授权，未取数）**：这台 Windows 的 SSH 公钥未入 Hasee authorized_keys（agentworld/jiangfeng 免密均 Permission denied）；HTTP `/metrics` 仅进程级计数器、给不了分产线历史；内置浏览器开 http://192.168.31.14 跳 /login（认证阻塞）。**待用户三选一**：(a) 在浏览器 /login 亲自登录后我用登录态调 /api 聚合；(b) 把本机公钥 `C:\Users\24670\.ssh\id_ed25519.pub` 加入 Hasee agentworld 的 authorized_keys（或给 SSH 密码），我用 python3 sqlite3 只读统计；(c) 改回 Mac 跑（Mac 有 hasee-2016-server 免密）。观察窗 09-17/19，已取到的最新结论见 #41 的 09-17 两条。
