...
34. 🔵 **公告管理 UI 已落地 + 角色权限方案已设计（2026-09-05 推进 31-④）**：**公告已完成**——管理员点铃铛下拉出「管理公告」`AnnouncementManager`（`GET /api/announcements/manage` 全量列表 + 新建/编辑/删除，双语正文 + level + 生效/失效窗口），权限沿用 `ANNOUNCEMENT_ADMIN_EMAILS` env 白名单（`2467055074@qq.com`，`.env` gitignored），`/api/auth/me` 增 `canManageAnnouncements`；server 780/780、web 1500/1500。**RBAC 方案已定稿**：[docs/design-rbac.md](docs/design-rbac.md)——全局角色 `owner/admin/user`（owner=首个注册用户，bootstrap 规则 + 既有库最老用户升 owner）+ 资源级 `owner/editor/viewer` + `resource_access` 表（迁移 v31/v32）+ 判定层 `permissions.ts`（`requireGlobalRole`/`requireResource`/`visibleResourceIds`）替换 `isAnnouncementAdmin` 与裸 `user_id` 隔离（越权基线改造）+ Collaborators 共享 UI + 权限变更审计。**分阶段**：P0=users.role+owner bootstrap+env 退役 /me 返回角色；P1=resource_access+越权基线改造（资源共享真正落地）；P2=共享/运营 UI；P3=admin 用户管理。deferred-items「多租户/权限」触发行已更新（底层就位，团队协作本身仍待真实需求）；公告 env 白名单将在 RBAC P0 退役。
