...
46. 🟡 **RTS 阶段 B 设计细化 + 技术预研（2026-09-11 启动，M1 等待期先行，不碰业务逻辑）**：设计文档 `docs/design-rts-stage-b.md`（B1-B9 结合当前代码库逐项复核 + 5 项技术风险标注）；技术预研原型 `apps/web/src/canvas/CanvasPark.tsx`（InstancedMesh 100 厂/per-instance color 呼吸/Sprite billboard/raycast instanceId 映射/mount-once 双 effect/选中环/锁俯角/完整 cleanup）。**B2 parkLayout 纯函数已落地**——`packages/core/src/parkLayout.ts`（类别聚簇 + 类 BFS 行布局 + 距离碰撞 + 阿基米德螺旋黄金角兜底，5 常量，FACTORY_SIZE=200，HARD_CAP=200）+ 11 例测试全绿 + core 导出，CanvasPark 改从 core import、删本地实现（commits `76ad634`/`a252d4f`/`fe4bd30`）。**B1 园区布局持久化·后端已落地（2026-09-11，未提交待原子 commit）**：graphs 表加 park_x/park_z（base DDL + 迁移 37 双改，PG 走 toPgDdl），driver 三方法 getParkCoords/setParkCoord/clearParkCoord（双 scope、owner-only 写、**刻意不刷新 updated_at**），REST `PUT/DELETE /api/graphs/:id/park-coord` + overview 带 parkX/parkZ，新增 13 测试全绿（park-coord 6 + api.park-coord 6 + pg-sql 1），server tsc/build 通过、同口径对照零回归。落地中根治一个 node:sqlite 硬坑：CREATE TABLE 体内的 `--` 注释会让 DROP COLUMN 重建报 incomplete input，已把 graphs 建表注释整体上移（设计文档 B1.2 有记）。**B3/B4/B6/B7/B8 已落地（2026-09-11，commits `a4bdc08` server / `2b5f62e` web，未 push）**：B3 overview 补 category（origin_template_id 反查模板类别、兜底「自定义」）+ pendingReview（=halted），server +2 测；B4 CanvasPark 生产级重写（mount-once 双 effect、InstancedMesh 低模工厂、core parkLayout 聚簇、React.lazy 独立 chunk、接真实 overview 15s 轮询）；B6 单 rAF 每帧 per-instance 状态色（running 呼吸/done 蓝/failed 红/halted 黄/idle 灰）+ 待审角标，不走 setState；B7 InstancedMesh raycast instanceId→graphId + HTML 浮层（进入常显、重试仅 failed、暂停 cron 仅 hasCron）+ 选中环；B8 view-mode 扩 2d/3d/park + 持久化 parkCamera + drilledFromPark，进入淡切 L1 单厂、返回恢复 L0 机位。新增 park i18n 命名空间（zh/en 各 8 key）、运营台头部 + ⌘K 两处入口、token 化样式、CanvasPark 4 测。**验证**：全包 typecheck 四包绿；web 全量 1768 过 +1 既有 flaky（PublishTargets 表单 reset 时序，隔离 10/10 绿、未触碰该文件，零回归）；内置浏览器 live 走查通过（10 厂按 5 类聚簇、done/idle 状态色、点厂浮层、钻取演示产线 L1 无串厂、返回恢复，临时建的 9 条冒烟产线已删除复原）。**B5 帧率实测已完成（2026-09-12，commit `b91afd6`）**：CanvasPark 加可开关 FPS 调试 overlay（`?debug=1` 或 ⌘⇧D，显示 FPS/厂数/draw calls/zoom，关闭零开销）；本地基准测试造 100 条 B5-BENCH 产线（smoke test 账号，测后已全部删除复原），**101 厂满载 = 60 FPS / 15 draw calls**，放大钻取视角同样 60 FPS / 15 draw calls——InstancedMesh 1 draw call + billboard ~14 draw calls，性能远超目标（≥30fps），**per-instance 视口剔除判定为过度设计，不做**。**B9 i18n/token 收尾 + 全量回归 + 真机验收已完成（2026-09-12，commit `7dbd61f`）**：新增 park.debug i18n 命名空间（fps/factories/drawCalls/zoom/toggleHint，zh/en 同构），FPS debug overlay 硬编码英文改 t() 调用、硬编码颜色改 CSS 变量（--ok/--mono/--ink-faint），Canvas 2D 角标颜色提取为命名常量（COLOR_ALERT/COLOR_INK 镜像 CSS token）；全量 1811 web 测试通过、typecheck 干净；真机验收（本地 dev，commit 7dbd61f）：FPS debug overlay 中文显示正确（帧率/厂区/绘制调用/缩放/开关）、CSS 变量生效（--ok=#66e07a、--mono、--ink-faint=#5d707c）、帧率 60 FPS、钻取/返回无串厂（B8 已验证无回归）。**RTS 阶段 B B1-B9 全部完成，阶段 B 正式闭环。****B1 前端拖拽持久化已落地（2026-09-12，commit `5dbe3cd`）**：CanvasPark 工厂拖拽（pointerdown raycast 命中→禁用 OrbitControls→pointermove 地面 y=0 raycast 实时更新 instanceMatrix+billboard→pointerup 保存 override+400ms 防抖 PUT `/api/graphs/:id/park-coord`），manual overrides 存 ref、data-sync effect 在 parkLayout 后应用、overrideVersion state 触发重算；api.ts 加 putParkCoord/deleteParkCoord。108 canvas 测试全绿、全包 typecheck 通过。**仍未做**：Hasee 副本库迁移 37 升级演练（M1 期不部署）。阶段 B 整体商业化仍需闭环 + 真实多产线场景（人均 ≥3 活跃产线）触发；阶段 C（完整 3D RTS）同挂 deferred。
...
48. 🔵 **商业化 M3：收款与账单落地（2026-09-14 启动，方案 docs/design-monetization-m3-implementation.md，S1-S6）**：在 M2 订阅 gate 基础上加装账单层。**S1 ✅ commit `4981059`**：invoices 表（迁移 38，base DDL + sqlite + PG 派生）+ invoiceService（generateInvoice 幂等 / generateAllInvoices / listInvoices / getInvoice / markInvoicePaid / voidInvoice，状态机 draft→open→paid/void，金额从 PLAN_PRICES 取，line_items JSON）+ driver 层 6 方法 + listAllSubscriptions + CLI `pnpm generate:invoices`（--dry-run/--period）；17 单测 + 37 回归全过，四包 typecheck 绿。**S2 ✅ commit `12187d7`**：API 端点 GET /api/invoices（列表）+ GET /api/invoices/:id（详情，跨用户 403）；web client Invoice 类型 + getInvoices/getInvoice；InvoiceList 组件（表格 + 详情弹窗含 line_items/支付信息/备注）；集成到 BillingTab（用量与套餐对比之间）；i18n billing.invoices 命名空间（zh+en）；CSS 账单表格/状态徽章/详情网格；7 API 测试全过，i18n 守护 + 四包 typecheck 绿。**S3 ✅ commit `70a4a08`**：HTML 发票模板（invoiceTemplate.ts，自包含 HTML/CSS，打印友好 A4，内联样式，系统字体，状态徽章，明细表格，合计，备注，客户邮箱）+ 下载端点 GET /api/invoices/:id/download（text/html + Content-Disposition attachment，跨用户 403，不存在 404）+ web client downloadInvoice（blob 下载）+ InvoiceList 下载按钮（列表行 + 详情弹窗头部）+ i18n billing.invoices.download（zh+en）；11 API 测试全过（7 S2 + 4 S3 下载），i18n 守护 + 四包 typecheck 绿。**S4 ✅ commit `f91cc4d`**：admin 手动收款闭环——API 端点 GET /api/admin/invoices（owner 专用，列出所有用户账单）+ POST /api/admin/invoices/:id/mark-paid（owner 专用，已支付/已作废返回 409）+ POST /api/admin/invoices/:id/void（owner 专用，已支付返回 409）；web client adminListInvoices/adminMarkInvoicePaid/adminVoidInvoice；AdminPanel 新增「账单管理」tab（owner 专用，账单表格 + 标记已支付/作废/下载按钮 + 加载/错误/空状态）；i18n modals.adminPanel invoices 相关 key（zh+en）；CSS admin-invoice-table 样式；20 API 测试全过（7 S2 + 4 S3 + 9 S4），i18n 守护 + 四包 typecheck 绿。**S5 ✅ commit `5effafc`**：Team 套餐 seats 限制——core PlanQuota 加 seats 字段（free/starter/pro=1, team=5）；server PUT /api/graphs/:id/access 在添加新 collaborator 时检查 seatsUsed >= seatsLimit 返回 403 seats_exceeded（更新已有 collaborator 不消耗新 seat）；web CollaboratorsModal 显示 seats 用量/上限，满员时禁用添加按钮；i18n seatsUsed/seatsFull（zh+en）；CSS .collab-seats；2 个新 seats 测试 + 现有 ACL 测试升级 owner 到 team plan；16 access 测试全过，四包 typecheck 绿，i18n 守护绿。**✅ PR #280 待合并（CI 全绿 2026-09-14）**：S1-S5 全部代码 + 测试修复（迁移版本断言 37→38、rollback 测试改回滚 invoices 表、park-coord API 测试升级 owner 到 team、CollaboratorsModal test mock getSubscription），CI 首次因 PublishTargets flaky test 失败，rerun 后全绿。**✅ Hasee 部署完成（2026-09-14 14:20 UTC）**：PR #280 合并 → CI 自动部署（merge `f65a9c6`）→ 迁移 38 自动执行（invoices 表已存在）→ 运行 `generate:invoices` 生成 9 月周期账单（4 张：owner pro $29 + 3 free $0，状态 open）。**修复**：generate-invoices.ts 从 src/scripts/ 移到根 scripts/（与其他 CLI 一致），导入路径修正为 ../src/*（commit `db83b87`，已 push）。服务正常运行，健康探针全绿。**M3 S1-S5 全部完成**：手动收款 + 账单流水 + 发票下载 + admin 账单管理 + Team 席位限制。**✅ M3 UI 验证通过（2026-09-14 14:30 UTC）**：浏览器真机验证 Settings → 套餐与用量——当前套餐专业版 $29/月、用量面板四 meter 正常（Token 132.5万/200万、视频 13/10 红色超限、存储 62.2MB/50GB、并发 2/5）、账单流水显示 9 月账单 $29.00 待支付、查看详情/下载发票按钮正常、套餐对比四档价格正确。**S6 ✅ 设计文档完成（docs/design-monetization-m3-s6-stripe.md，待有收款主体后实施）**：架构概览 + 数据模型变更（迁移 39）+ API 设计（checkout/portal/webhook）+ 核心流程（升级/续费/失败/取消）+ 文件清单 + Stripe 侧配置 + 安全考虑 + 测试策略 + 6 步实施步骤 + 回滚方案。

> **下一步主线**：待办 #1–#45 已全部完成（活跃项为 #41/#46/#47/#48），**M1 成本计量回采 + 三问分析 + 价格校准已全部完成（2026-09-14）**——125 runs / $5.57 总成本，套餐价格已校准（Starter $9 / Pro $29 / Team $149），design-monetization.md §4.1 有完整校准依据。**M2 订阅 gate（P1）S1-S8 代码已全部完成、测试通过并部署 Hasee（2026-09-14 10:29 UTC，PR #277 merge `124bdca`）**——commit 序列 1d7a3e4→8a8a32f→7dece12→99d639b→9807988→f3ed6f4→d045f46；部署严格按手册顺序执行（回填→owner 升 pro→最后开 MONETIZATION_ENFORCE），健康探针全绿、日志无错误。**M3（P2 收款）S1-S5 代码已全部完成、CI 全绿、PR #280 待合并（2026-09-14）**——commit 序列 4981059→12187d7→70a4a08→f91cc4d→5effafc→252f18c→28e9fb0；合并后需执行 Hasee 部署：迁移 38 + 生成当前周期账单（pnpm generate:invoices）。**S6 Stripe 待收款主体**。M1 回采产线继续 cron 自动攒数据（每天约 57 次 run），用于 M2 上线后的二次校准。RTS 阶段 A 平面运营工作台已上线（待办 43）；RTS 阶段 B B1-B9 已全部落地（待办 46，阶段 B 正式闭环）；code-audit 全部 77 项已无未修复项（待办 45）。RTS 阶段 C（完整 3D RTS）待商业化闭环 + 人均 ≥3 活跃产线再启（deferred-items）。可选项：随时换付费 agnes key（换后 429 彻底消失、回采更顺畅）。
...
1. **feat(server) run.finished 记录具体失败原因 + ②翻译产线 maxAttempts 3→2（2026-09-13，`aae9c3e`/`92ac5f8`，PR #272 已合 dev，Hasee 已部署 commit `80e9e49`）**——engine.ts finish() 在 failed 状态时构建 failureNote（遍历 failed 节点收集 errorCode+nodeName+截断 error message，加上 stranded 节点），作为 reason 字段传入 run.finished 事件；run.ts haltedOf() 扩展为 failed 状态也返回 reason（原只返回 halted），使失败原因持久化到 DB 的 halted_reason 字段。此前 failed run 的 halted_reason 恒为 null/UNKNOWN，需钻 node_runs 才能诊断。**②翻译产线运维优化（Hasee DB 直接修改，非代码变更）**：qc-8s98z 节点 gate.maxAttempts 从 3 降到 2，单次 run 最多 API 调用从 12 次降到 9 次，降低 RATE_LIMIT 概率（原完成率仅 33.3%，主因是返工逻辑打爆限流）。typecheck 四包全绿；server 测试本机 Node v22+macOS seatbelt 下 code-sandbox 用例失败为环境问题（基线同样失败，非回归）。**部署验证（2026-09-13）**：Hasee 健康探针 `commit=80e9e49`，grep 确认 engine.ts 含 failureNote（第 837/883/913 行）、run.ts 含 failed 分支（第 248 行），功能已生效。
2. **fix(web) Guided Tour 按钮样式修复（2026-09-12，`dec70eb`，未 push）**——全局 `.btn` 的 `width:100%`/`margin-top`/无 `white-space:nowrap` 在 340px 引导卡片里把「上一步/下一步」挤成竖排单字、三按钮高度不齐；`.guided-tour__actions .btn` 作用域覆盖为 `width:auto`+`nowrap`+`min-height:38px`+`inline-flex` 居中，skip 按钮 `flex:1` 撑满左侧、导航组靠右；浏览器实测三按钮统一 38px 高、文字横排（跳过引导 155px / 上一步 63px / 下一步 71px）。
3. **feat(web) 新用户分步引导 Guided Tour + 多引导注册中心（2026-09-11，`b1bd81b`/`f0f9bd9`/`f854c43`/`a30efac`，未 push）**——可扩展「引导即数据」架构：`tours/`（TourDef/targeting/primitives + first-run 8 步脚本 + 注册表）、`store/guided-tour.ts`（版本化 seen、24h 自动弹冷却、priority 调度、0→1 建首条产线才自动触发、老用户零打扰）、`guided-tour-engine.ts`（placeCard 溢出翻面/clamp、holeRect、before-action 注册）、`GuidedTour.tsx`（聚光灯洞 + box-shadow 压暗、Esc/焦点 trap、a11y dialog、引导期强制 2D 关闭恢复、锚点缺失降级居中、`a30efac` 修页面横向残留滚动把锚点推出视口→启动复位 + scrollIntoView 自愈）、`GuidedToursMenu.tsx`（UserMenu「引导与新功能」What's-new 入口 + 已看标记）、⌘K 动态注册重看；tour i18n zh/en 双语、新增 `--color-scrim` token（暗/亮）。+42 测试（store 22/engine 10/GuidedTour 7/Menu 3），web 1769→**1811（87 文件）**；内置浏览器 live 走查全过（8 步聚光/自动展开控制栏与巡检员并选中首节点/双主题/完成写 seen/Esc）。设计文档 `docs/design-guided-tour.md`（§十二 多引导注册中心扩展法）。
4. **feat(web) RTS 阶段 B B4/B6/B7/B8 L0 园区总览（2026-09-11，`2b5f62e`，未 push）**——CanvasPark 生产级重写（mount-once InstancedMesh 低模工厂 + core parkLayout 聚簇 + React.lazy 独立 chunk + 真实 overview 15s 轮询），单 rAF per-instance 状态色/待审角标，raycast 点厂 HTML 浮层（进入/重试/暂停 cron 按态显隐），view-mode 扩 2d/3d/park + 相机记忆 + 钻取 L1 淡切往返；park i18n zh/en、运营台+⌘K 双入口、token 样式、CanvasPark 4 测，web 1765→1769。内置浏览器 live 走查全过。
5. **feat(server) RTS 阶段 B B3 overview 补 category/pendingReview（2026-09-11，`a4bdc08`，未 push）**——operationsByGraph 补选 origin_template_id，overview 反查模板类别（兜底「自定义」）、pendingReview=halted，只追加不动 A 阶段字段，+2 API 测试，server 1037→1039。
