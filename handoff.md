...
> 1. **媒体计量修复未上 Hasee（M1 数据正在被污染，不可逆）**：修复在 `6554769`，截至验收**仍在本地 feature/20260824、连 push 都没推**（本地领先 origin/feature 三个提交：`6554769` 及两个 docs），部署停在 `4eb87d2`（dev），已核实 `6554769` 不是 `4eb87d2` 的祖先 → 生产视频/音频仍按 $0 落库。M1 回采窗口里凡走视频/音频的 run，成本系统性偏低且 `cost_usd` 当场按 0 落库、**事后无法补算**。**push 并合并/部署 feature/20260824 到 dev 前，视频/音频成本数据不可信。**
