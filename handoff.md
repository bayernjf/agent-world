...
> 🔧 **待办 #55（2026-09-16，竞品痛点应对·安全子集两批均落地、待 push 走 PR）：步级可观测（G1 只读链路 + 完整输出懒加载）+ 上游契约纯函数/配置形态 + 超时纯函数**。依据 `docs/design-step-trace-and-robustness.md`，只落地**不改 run 执行状态机**的安全项（绝不能影响 Hasee M1 回采）。工作分支 feature/20260824，第一批 5 commit 已提交、第二批改动本轮写完待原子提交，均未 push：
...
> - **明确留待（改 run 执行核心，须单独评估、防打断 M1，本轮不做）**：G1.2 从节点 fork 重跑 API；**G2.2 engine 契约接线**（节点完成 output 后、下游派发前调 `validateContract`，违例 failed + `SCHEMA_VIOLATION`，需先定是否扩 ErrorCode enum；**它是让 G2.3 配置真正生效的前置**）；G2.4 给模板数据源节点批量预置 contract（**等 G2.2 接线后对照真实输出逐个核对字段名**，避免字段名写错在未来误拦；原料台/文本节点输出纯文本/Markdown，coerceOutputObject 判 notObject，本就不该配）；G4.2 timeout+degraded 降级状态机；G4.3 前端"继续/降级"按钮；G4.4 视频远端进度轮询；G3 节点 maxRetries 下放 + 预算 UI。**待用户 push → PR → CI → merge → Hasee 部署后真机走查"运行历史→步骤→展开完整输出"与 Inspector「配置」tab「输出契约（高级）」折叠区。**

> 🔧 **待办 #54（2026-09-16，代码已提交、待 push 走 PR）：窄视口（实测 CSS 视口宽 ~863px）顶部 HUD / 画布工具条按钮与统计文字被 flex 压成竖条，另发现折叠 Inspector 撑出 420px 横向溢出**。真机 DOM 探针定位 **9 处竖折**：`.hud__meta` 统计 span 被压到 12px 宽逐字竖排成 297px 高黑条（视觉上的"空黑框"即 meta 区本身）；`.hud__actions` 的 `.chip`（收起侧栏/待审核/成品库/术语表/快捷键？）被压到 38px 文字竖排；`.canvas-toolbar__btn` 的 +文坊/+画坊 59px 折两行（+原料台 69px 正常，反证是宽度阈值）。根因：`.hud`/`.canvas-toolbar` 无 flex-wrap、`.hud__meta`/`.chip`/`.canvas-toolbar__btn` 无 nowrap/不防 shrink。**另发现独立预存 bug**：`.inspector-slot--hidden` 用 `translateX(100%)` 滑出，但定位祖先 `.stage` 是 `overflow:visible`，折叠面板仍贡献 420px `scrollWidth`（移除本次 CSS 后溢出依旧，证明与竖折无关）。修复 commit **`a8e4a89`**（分支 `feature/hud-narrow-layout`，纯 CSS 10 行 `apps/web/src/styles.css`，无文案、不碰颜色/间距 token）：`.hud`/`.hud__actions`/`.canvas-toolbar` 加 `flex-wrap:wrap`；`.hud__meta` `flex:0 0 auto`+`nowrap`；`.hud__actions .chip` 与 `.canvas-toolbar__btn` `white-space:nowrap`；`.stage` 加 `overflow:hidden`。真机注入验证：竖折清零、横向溢出 420→0、Inspector 展开 420px 完整贴右缘不裁/开合正常；typecheck 4 workspace 全过。2d/3d/park 三视图共享该 CSS 同步受益。**待用户 push → feature PR → CI → merge → Hasee 部署 → 真机复验。**
