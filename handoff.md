...
> - **唯一新修（在途，未 merge/部署）**：从"选中过节点的 3D 视图"进入/返回 L0 园区时，右侧 L1「节点详情」Inspector 仍残留展开、空态挤占园区画布（新用户进 park 默认折叠，属边缘 UX）。修复在分支 **`feature/park-inspector-collapse`，commit `d2cf6d3`**（`App.tsx` 的 `enterPark`/`backToPark` 加 `setInspectorCollapsed(true)`，回 2d/3d 仍由现有 selectedId/inspectorOpen effect 随节点选中开合；pre-commit 四包 typecheck 全绿；纯前端无新文案）。**待 push 走 feature PR→CI→merge→部署后真机复验**（按用户规则未自动 push）。
