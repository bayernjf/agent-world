...
1. **feat(server,web) 连接器数据插值（2026-09-05，方案 design-data-interpolation.md）**——4 个原子 commit（§11 切分）：`e3fc957` D1+D2+D3+types（`ResolvedMaterial.data?` + engine `sourceMeta` 旁路 + 快捷名注册表 `product`/`products` + `run.ts` loader 带 data）；`7db162b` D4+D5+D6（`buildSourceBrief(fallbacks)` 仅事实字段留空回填/手填覆写、8 简报字段 `${product.*}` 插值、`graph.ts` 注释修正）；`2f1213e` 测试 `engine.products.test.ts` 2→11 例 + buildSourceBrief 4 单测；`09541db` D7 web hint（product connector 显示「留空自动取商品库值」+ zh/en i18n）。验收 server 868→**883/883**、web **1561/1561**、typecheck 全绿。**实现细节纠偏**：`loadProducts` 收到的是 `ProductConnector`（`.selection` 顶层）不是含 `.product` 的壳；brief 插值用 `evaluateTemplate` 单遍 replace（防重入），踩名优先级钉死节点 ctx 优先（⑪ 守护）。

2. **feat(server,web) 公告 target 定向 P3（2026-09-05，方案 design-announcement.md）**——`announcements.target` 三态全生效：① server 受众匹配 `announcementTargetsUser()`：NULL=全员 / `graph:<id>` 复用 `graphAccessRole`（owner/editor/viewer 均命中——废弃公告对只读协作者同样有意义）/ `template:<id>` 走新增 `db.userUsesTemplate`（自有 graph OR 被 shared 的 graph，origin_template_id 命中，双 prepared stmt）；未知前缀 fail-closed 谁都看不见（历史脏数据 `role:admin` 行自动静默退役），写入侧 `parseAnnouncementBody` 加 `ANNOUNCEMENT_TARGET_RE` 校验（`graph:`/`template:` + `[A-Za-z0-9_-]+`，非法 400，非字符串也 400）；`GET /api/announcements` 过滤替换 + 下发 `target` 字段，`/manage` 带 target 回显。② web 入口级展示：`AnnouncementAlerts.tsx` 新组件（`useActiveAnnouncements` fetch + `useTemplateAlerts`（templateId→最新公告标题，服务端 created_at DESC 取首条）+ `GraphAnnouncementBar`（当前图被定向时 header 下 in-flow 横幅（`.announcements__banner--graph` static 变体，不与全局 warning 横幅的 fixed 定位冲突），session 内 dismissable））；TemplatePicker 加 `alerts` prop（warning 边框 + 「有公告」角标 + Tooltip 显示标题），NewGraphDialog/Onboarding 接线。③ AnnouncementManager 表单三态受众（全员/按模板（33 模板下拉）/按产线（graph id 输入））+ 列表行受众徽标（模板显示名字反查 TEMPLATE_LIST、产线截 12 字符）+ 未知前缀 legacy 行编辑回落全员（保存即规范化为 NULL）。测试：server api.announcements.test.ts +7（graph 命中 owner/editor/viewer 不命中 outsider / template 命中自有+被共享 / target 字段下发 / 未知前缀全用户不可见 / 非法 target 6 形态 400 / 建改清定向可见性翻转 round-trip / manage 带 target）；web 新增 AnnouncementAlerts.test 6 + AnnouncementManager.test 6 + TemplatePicker +2。server 861→**868/868**、web 1549→**1561/1561**，typecheck 全绿。

3. **feat(server) 密钥轮换 P1+P2+P3（2026-09-05，方案 design-key-rotation.md）**——at-rest 主密钥从单密钥升级为可轮换 keyring：① **P1 keyring**（`at-rest.ts` 重写加载层）：`AGENT_WORLD_ENCRYPTION_KEYS` 逗号有序列表（第一个加密、其余仅解密）/ `.encryption-keys` JSON 数组文件（0600，DB 同目录）/ 旧 `AGENT_WORLD_ENCRYPTION_KEY` 单值与 `.encryption-key` 文件等价兼容 / 首启自动生成落盘；密文格式新增 `enc:v2:<keyId>:<iv>:<tag>:<cipher>`（keyId=派生材料前 6 hex，解密按 id 精确路由，未知 id fail-closed），v1 旧密文逐 key 尝试（GCM auth tag 保证错 key 只失败不错解）；keyId 前 6 hex 撞号在加载期即抛错（1/16M 概率也拒绝歧义路由）。② **P2 重加密收敛**：`src/key-rotation.ts`（库）+ `scripts/rotate-reencrypt.ts`（CLI，`pnpm rotate:reencrypt`）——覆盖五个密文面（settings.data / publish_targets.config_encrypted / graphs.doc / graph_versions.snapshot / runs.snapshot），幂等（已在新 key 的行跳过、中断可续跑）、fail-closed（解不开的行点名 `表.主键` 中止不静默跳）、`--dry-run` 只 census 不动库、`--table=` 分批、residue 报告 + 退出码门禁（0=可删旧密钥 / 1=残留必须留旧密钥）、whole-column 历史明文行顺手补封；复用 encryptString/decryptString/sealDocString/openDocString 不另写第二套规则（at-rest §4.3 教训）。③ **P3 运维手册** [runbooks/key-rotation.md](docs/runbooks/key-rotation.md)：定期轮换五步（生成→keyring 新在前→重启→收敛→residue 归零删旧 key 再重启）+ 泄露应急三差异（立即收敛/用户凭证视为泄露公告重置/连带轮换 .jwt-secret）+ 常见错误表 + 验证清单。测试：`at-rest.test.ts` 重写为 keyring 语义 35 例 + 新增 `key-rotation.test.ts` 7 例（五面收敛含 URL percent-encoded 拼写/contentHash 链不坏/幂等/fail-closed 点名行/dry-run/--table 子集与未知表拒绝/明文补封）；server 843→**861/861**，typecheck 全绿。启动日志 encryptionKeySource 保持只记来源不记材料。

4. **feat(server,web) 反馈→公告联动 P3（2026-09-05，方案 design-feedback.md）**——server：`POST /api/feedback/announce`（owner/admin，单请求「建公告 + 批量关反馈」避免半状态；feedbackIds 非空 ≤50 去重，未知 id 整体 404 fail-closed 无部分生效；公告体复用 `parseAnnouncementBody`；已 closed 幂等跳过不计入 `closed` 返回值；审计 `feedback.announce` detail `{count,level}`）。web：AdminPanel 反馈 tab 行首 checkbox 多选（切状态筛选清空）→「合并发公告（N）」→ 表单预填模板（标题按主分类（众数，平票取先者）+ 条数中英双语，分类名 `i18n.t(lng)` 双取不硬编码；正文折叠 ≤5 条消息各截 80 字符；默认 warning；endsAt 可选）→ 提交成功 toast + 清选 + 重载；Escape 分层（先表单后面板）。测试 server +5（403/400/404 无副作用/批量关闭含幂等/审计）、web +5；server **843/843** + web **1547/1547**（含 i18n 守护）。至此反馈方案 P1+P2+P3 全量落地。

5. **feat(server,web) 用户反馈 P1+P2（2026-09-05，方案 design-feedback.md）**——迁移 v33（feedback 表：message/category/context 白名单 JSON/attachment BLOB + mime/三态 status）+ 新库 DDL 双轨。server 4 路由：`POST /api/feedback`（message ≤2000 + category 白名单 + **服务端上下文白名单二次脱敏**——route/userAgent/locale/lastRunId/lastError{message,lineno} 之外一律丢弃 + 截图 base64 ≤1MB 限 png/jpeg/webp/gif + 每用户滚动小时 10 条限流 DB 计数）+ `GET /api/feedback`（owner/admin，LEFT JOIN email，`?status=` 过滤，列表不传 BLOB）+ `PATCH /:id`（三态流转，幂等 unchanged 不写审计）+ `GET /:id/attachment`（cookie 认证支持 `<img src>`）；审计 `feedback.submit`（detail 只 category）/`feedback.status_change`（detail `{to}`）。web：`FeedbackModal`（一句话 + 分类 + 粘贴截图预检预览 + 诊断勾选默认开，lastRunId 取 run store、lastError 由 `lib/lastError.ts` 全局采集）+ UserMenu「反馈」入口（登录即见）+ AdminPanel 第三个反馈 tab（状态筛选 + 乐观更新 + 失败回滚 + 附件懒加载）+ `feedback` i18n namespace（zh/en）。**与方案四处偏差**：v30→v33；env 白名单→RBAC 角色；Settings 区块→AdminPanel tab；加 mime 列 + 附件独立端点。测试 server 18 例 + web 20 例；server **838/838** + web **1542/1542**（含 i18n 守护）。
...
* `pnpm --filter @agent-world/web exec vitest run`：**1561/1561 通过**（62 文件；2026-09-05 公告 target 定向 +14 用例——新增 AnnouncementAlerts.test 6（模板角标/产线横幅/无公告不渲染）+ AnnouncementManager.test 6（受众三态表单序列化/编辑回显/legacy 回落全员）+ TemplatePicker +2（角标渲染）；2026-09-03 从 176 提升到 1460，+1284 用例——组件测试全覆盖 39 个组件，P0/P1/P2/P3 四批全部完成；基础设施 @testing-library/react + jsdom + vitest.config.ts + setup.ts + utils.tsx；过程中修复 Inspector.tsx 可选链 bug；2026-09-04 修正 TemplatePicker 模板数断言 27→33）
