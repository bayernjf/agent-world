...
31. ✅ **合规/运营批次五份方案（2026-09-05 定稿；**①密钥轮换、②审计日志 P1+P2、③日志 P1+P2+P3、④公告 P1+P2、⑤反馈 P1+P2+P3 全部已实施**）**：围绕「用户存的 key 能否合规安全保存」评估后补齐的设计文档，均已登记 [docs/README.md](docs/README.md) 索引与 [deferred-items](docs/deferred-items.md) 触发条件——① [design-key-rotation.md](docs/design-key-rotation.md)（密钥轮换：**P1+P2+P3 全量落地 2026-09-05**——keyring 加载（`AGENT_WORLD_ENCRYPTION_KEYS` 多值 / `.encryption-keys` JSON 数组 / 旧单值等价兼容）+ `enc:v2:<keyId>:` 密文格式（v1 全兼容）+ `scripts/rotate-reencrypt.ts` 重加密收敛（幂等 / fail-closed / dry-run / --table / residue 门禁）+ 运维手册 [runbooks/key-rotation.md](docs/runbooks/key-rotation.md)（定期轮换五步 / 泄露应急含 JWT secret 连带轮换 / 常见错误排查））；② [design-audit-log.md](docs/design-audit-log.md)（审计日志：**P1+P2 已落地 2026-09-05**——audit_log 表迁移 29 + `audit()` helper + 全词表埋点 + `GET /api/audit` + 专项测试；P3（180 天清理 + hash chain）待触发）；③ [design-logging.md](docs/design-logging.md)（服务端日志：**P1+P2+P3 全量落地**——见待办 33）；④ [design-announcement.md](docs/design-announcement.md)（公告：**P1+P2 已落地 2026-09-05**——announcements/announcement_reads 表迁移 30 + `GET /api/announcements`（窗口过滤 + 双语字段 + 本人 read 状态）+ `POST /:id/read`（幂等 upsert）+ 管理 API（`ANNOUNCEMENT_ADMIN_EMAILS` env 白名单 + 审计埋点）+ 前端 `AnnouncementBell`（铃铛下拉 / warning 横幅 / critical 模态，`announcements` i18n namespace zh/en）+ `api.announcements.test.ts` 专项测试（窗口过滤/read 幂等/权限/双语/CRUD 回路）；P3（target 定向）待触发）；⑤ [design-feedback.md](docs/design-feedback.md)（用户反馈：**P1+P2+P3 全量落地 2026-09-05**——feedback 表迁移 33 + `POST /api/feedback`（服务端上下文白名单二次脱敏 + 滚动小时 10 条限流 + 截图 ≤1MB）+ `GET /api/feedback`（owner/admin）+ `PATCH /:id`（三态流转）+ `GET /:id/attachment` + 前端 `FeedbackModal`（截图粘贴 + 诊断勾选）+ UserMenu 入口 + AdminPanel 反馈 tab + `feedback` i18n namespace + P3 反馈→公告联动（`POST /api/feedback/announce` 单请求建公告+批量关闭 + AdminPanel 多选合并表单））。**实施触发**：轮换=合规准备启动。**已完成的安全验证（非方案）**：settings 表落库加密断言测试（`api.security.test.ts` 新增「settings at-rest encryption」组，直读 sqlite 原始字节断言无明文 + decryptString 可还原）；`.env` 钉死 `DB_FILE` 绝对路径消除 cwd 漂移；删除仓库根幽灵空库。
...
1. **feat(server) 密钥轮换 P1+P2+P3（2026-09-05，方案 design-key-rotation.md）**——at-rest 主密钥从单密钥升级为可轮换 keyring：① **P1 keyring**（`at-rest.ts` 重写加载层）：`AGENT_WORLD_ENCRYPTION_KEYS` 逗号有序列表（第一个加密、其余仅解密）/ `.encryption-keys` JSON 数组文件（0600，DB 同目录）/ 旧 `AGENT_WORLD_ENCRYPTION_KEY` 单值与 `.encryption-key` 文件等价兼容 / 首启自动生成落盘；密文格式新增 `enc:v2:<keyId>:<iv>:<tag>:<cipher>`（keyId=派生材料前 6 hex，解密按 id 精确路由，未知 id fail-closed），v1 旧密文逐 key 尝试（GCM auth tag 保证错 key 只失败不错解）；keyId 前 6 hex 撞号在加载期即抛错（1/16M 概率也拒绝歧义路由）。② **P2 重加密收敛**：`src/key-rotation.ts`（库）+ `scripts/rotate-reencrypt.ts`（CLI，`pnpm rotate:reencrypt`）——覆盖五个密文面（settings.data / publish_targets.config_encrypted / graphs.doc / graph_versions.snapshot / runs.snapshot），幂等（已在新 key 的行跳过、中断可续跑）、fail-closed（解不开的行点名 `表.主键` 中止不静默跳）、`--dry-run` 只 census 不动库、`--table=` 分批、residue 报告 + 退出码门禁（0=可删旧密钥 / 1=残留必须留旧密钥）、whole-column 历史明文行顺手补封；复用 encryptString/decryptString/sealDocString/openDocString 不另写第二套规则（at-rest §4.3 教训）。③ **P3 运维手册** [runbooks/key-rotation.md](docs/runbooks/key-rotation.md)：定期轮换五步（生成→keyring 新在前→重启→收敛→residue 归零删旧 key 再重启）+ 泄露应急三差异（立即收敛/用户凭证视为泄露公告重置/连带轮换 .jwt-secret）+ 常见错误表 + 验证清单。测试：`at-rest.test.ts` 重写为 keyring 语义 35 例 + 新增 `key-rotation.test.ts` 7 例（五面收敛含 URL percent-encoded 拼写/contentHash 链不坏/幂等/fail-closed 点名行/dry-run/--table 子集与未知表拒绝/明文补封）；server 843→**861/861**，typecheck 全绿。启动日志 encryptionKeySource 保持只记来源不记材料。

2. **feat(server,web) 反馈→公告联动 P3（2026-09-05，方案 design-feedback.md）**——server：`POST /api/feedback/announce`（owner/admin，单请求「建公告 + 批量关反馈」避免半状态；feedbackIds 非空 ≤50 去重，未知 id 整体 404 fail-closed 无部分生效；公告体复用 `parseAnnouncementBody`；已 closed 幂等跳过不计入 `closed` 返回值；审计 `feedback.announce` detail `{count,level}`）。web：AdminPanel 反馈 tab 行首 checkbox 多选（切状态筛选清空）→「合并发公告（N）」→ 表单预填模板（标题按主分类（众数，平票取先者）+ 条数中英双语，分类名 `i18n.t(lng)` 双取不硬编码；正文折叠 ≤5 条消息各截 80 字符；默认 warning；endsAt 可选）→ 提交成功 toast + 清选 + 重载；Escape 分层（先表单后面板）。测试 server +5（403/400/404 无副作用/批量关闭含幂等/审计）、web +5；server **843/843** + web **1547/1547**（含 i18n 守护）。至此反馈方案 P1+P2+P3 全量落地。

2. **feat(server,web) 用户反馈 P1+P2（2026-09-05，方案 design-feedback.md）**——迁移 v33（feedback 表：message/category/context 白名单 JSON/attachment BLOB + mime/三态 status）+ 新库 DDL 双轨。server 4 路由：`POST /api/feedback`（message ≤2000 + category 白名单 + **服务端上下文白名单二次脱敏**——route/userAgent/locale/lastRunId/lastError{message,lineno} 之外一律丢弃 + 截图 base64 ≤1MB 限 png/jpeg/webp/gif + 每用户滚动小时 10 条限流 DB 计数）+ `GET /api/feedback`（owner/admin，LEFT JOIN email，`?status=` 过滤，列表不传 BLOB）+ `PATCH /:id`（三态流转，幂等 unchanged 不写审计）+ `GET /:id/attachment`（cookie 认证支持 `<img src>`）；审计 `feedback.submit`（detail 只 category）/`feedback.status_change`（detail `{to}`）。web：`FeedbackModal`（一句话 + 分类 + 粘贴截图预检预览 + 诊断勾选默认开，lastRunId 取 run store、lastError 由 `lib/lastError.ts` 全局采集）+ UserMenu「反馈」入口（登录即见）+ AdminPanel 第三个反馈 tab（状态筛选 + 乐观更新 + 失败回滚 + 附件懒加载）+ `feedback` i18n namespace（zh/en）。**与方案四处偏差**：v30→v33；env 白名单→RBAC 角色；Settings 区块→AdminPanel tab；加 mime 列 + 附件独立端点。测试 server 18 例 + web 20 例；server **838/838** + web **1542/1542**（含 i18n 守护）。

2. **feat(server,web) RBAC P3 管理员运营 UI + 跨用户审计（2026-09-05，方案 design-rbac.md）**——8 个原子提交：① server db 层 `listUsers`/`updateUserRole`/`listAuditAdmin`（LEFT JOIN 解析 email，limit 夹 1-500）+ 路由 `GET /api/admin/users`/`POST /api/admin/users/:id/role`（owner 专属；role 仅 admin|user，owner 自身→400，404，幂等 unchanged 不写审计）+ `GET /api/audit` owner/admin 全量 + `?userId=` 过滤（普通用户路径不变）；② `api.rbac.test.ts` P3 17 例；③ web `api.ts` 加 `AdminUser`/`AuditItem` + `adminListUsers`/`adminSetUserRole`/`listAudit`；④ zh/en `modals.json` 加 `userMenu.admin` + `adminPanel` 段；⑤ `AdminPanel.tsx`（owner 双 tab 用户/审计、admin 仅审计；grant/revoke 走 ConfirmDialog；审计行 email/action/detail 压缩 JSON/ip + 满页「加载更多」游标翻页）+ CSS（token 化）；⑥ `UserMenu` 加「管理」入口（role owner/admin 显隐）；⑦ web 测试 AdminPanel.test 17 例 + UserMenu.test 管理入口 5 例；⑧ docs。**与设计稿两处偏差**：路径 members→users（与列表资源命名一致）；用户列表 owner-only（最小权限）。typecheck 全绿 + server **820/820** + web **1522/1522**（含 i18n 守护）。

2. **feat(web) RBAC P2 前端 Collaborators 共享 UI（2026-09-05，方案 design-rbac.md）**——5 个原子提交：① `api.ts` 加 `sharedRole` 返回类型 + `getGraphAccess`/`putGraphAccess` 方法 + `Collaborator` 接口；② zh/en `modals.json` 加 `collaborators`/`sharedRole`/`shareButton`/`viewerRestriction` i18n keys；③ `CollaboratorsModal.tsx` 新组件（协作者列表 + email/role 添加 + 移除，静态 lookup 对象避免 keys.test 漏检）；④ `GraphSwitcher` 共享图显示 editor/viewer badge、隐藏 rename/delete（保留 duplicate）、owned 图加 Share(⤴) 按钮；⑤ `App.tsx` 接线 `shareTargetId` 状态 + `CollaboratorsModal` + 从 `graphs` 派生 `isViewer` 传 `readOnly` 给 `ControlPanel`（禁用 Dispatch + 提示文案）+ `store/graph.ts` 加 `readOnly` flag 抑制 `scheduleSave`/`flushSave`（防 viewer 自动保存 403 刷屏）。typecheck 全绿 + i18n keys 4/4 + web **1500/1500**。

3. **feat(server) RBAC P1 资源级共享 + 越权基线改造（2026-09-05，方案 design-rbac.md）**——迁移 v32（`resource_access` 表：`resource_type`/`resource_id`/`user_id`/`role`，PK 防重复授权）；`src/rbac.ts` 判定层：graph 是共享单元，`graphAccessRole`/`requireGraph`/`visibleGraphs` 解析 owned + shared；`runAccessRole`/`requireRun`/`artifactAccessRole` 通过 `graph_id` 向上继承；`hasAtLeast` 做 owner>editor>viewer 比较。**越权基线改造**（全部路由）：`GET /api/graphs` 合并 owned + shared（带 `sharedRole` 字段）；`GET /api/runs`、`GET /api/artifacts`、`GET /api/batches` 按 `visibleGraphs` 过滤；graph PUT/DELETE 视图→403/外人→404；run start/cancel/resume/rerun 需 editor 且以 graphOwnerId 执行（config/variables/cost 一致性）；run stats/graph/events/stream/artifacts 需 viewer；artifact 列表/详情走 graph ACL（回退 artifact.user_id）；batch retry 需 editor；AB start 需 editor 且以 ownerId 执行、AB report 需 viewer。ACL 管理（`PUT/GET /api/graphs/:id/access`）限 owner，`access.grant`/`access.revoke` 审计。db 新增 `getBatchUnscoped`/`listBatches(graphIds?)`/`abGroupGraphId`/`getRunGraphRef`/`getArtifactGraphRef`/`listArtifactsForRunUnscoped`。`api.access.test.ts` 14 例（graph ACL 授权/回收/幂等/校验/审计 + run/artifact 共享继承 viewer 可读/editor 可写/outsider 404）；`api.security.test.ts` H1 从 403→404（不泄露存在性）。server 771→**803/803**，typecheck 全绿。

4. **feat(server) RBAC P0 全局角色（2026-09-05，方案 design-rbac.md）**——迁移 v31：`users.role` 列（默认 `user`）+ `idx_users_owner` 部分唯一索引（单 owner 不变量）+ 旧库 bootstrap（无 owner 时最早注册用户升 `owner`，即 `2467055074@qq.com`）；`createUser` 首账号自动 owner；`isAnnouncementAdmin` 改读 `owner/admin` 角色（同步化），**`ANNOUNCEMENT_ADMIN_EMAILS` env 白名单退役**（`.env` 已删）；`/api/auth/me` 返回 `role`。新增 `api.rbac.test.ts` 3 例（首账号 owner + 单 owner 不变量 / owner 可管公告 plain user 403 / 旧库回滚到 v30 后重跑 v31 的 bootstrap 断言）+ `api.announcements.test.ts` admin 提升改走 SQL。server 783/783。

5. **feat(server) 审计日志 P1+P2（2026-09-05，方案 design-audit-log.md）**——`audit_log` 表（迁移 29，append-only，idx user+time）+ `src/audit.ts`（`audit()` helper 写失败只 warn 不抛；`changedFields` 递归 diff 产出字段路径，深度上限 4）+ index.ts 全词表埋点（account.register/login/login_failed/logout/password_change、settings.update/test_provider、graph.create/update/delete/restore_version、run.start/cancel、publish_target.create/delete；IP 取 X-Forwarded-For 首跳）+ `GET /api/audit`（仅本人、时间倒序、limit/before 游标分页）。**红线**：detail 只记字段路径永不记值（含脱敏值也不记）；新增 `account.password_change`（词表遗漏补上）；test_provider 记「真实 key 即将出站探测」时刻而非 result。新增 `audit.test.ts` 10 例（全动作覆盖 / detail 红线（真实 key、token、新密码全表扫描）/ 写失败不阻塞 / 本人隔离 + 分页）。server 760→**770/770**。
