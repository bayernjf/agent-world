...
49. 🔵 **RTS 阶段 C 技术内核 C1-C3（2026-09-15 启动；本轮圈定 C1-C3，C4-C8 留下一轮，原重启条件未满足、属 design-rts-overview §九自留出路）**：
   ...
   - **C3 ⏸️ 路线待拍板（唯一必须停下的决策点，未写码）**：目标用连续 zoom 替换 B8 现状——实测 B8 **并无** 250ms 交叉淡切，enterFactory/backToPark 是硬切，仅靠 store 的 parkCamera/camera3d 各自记忆视角。已核实 **L0 CanvasPark 与 L1 Canvas3D 同为 THREE.OrthographicCamera 固定 YAW/PITCH 等距视角、共用 `apps/web/src/canvas/iso3d.ts`（非透视）**，故相机 zoom/位置/target 可平滑插值、无正交↔透视异构问题；真正差异只在场景坐标系尺度（L0 园区全局 parkLayout vs L1 单厂 boardToWorld 局部板坐标），且两组件在 App.tsx 三元互斥挂载。两方案：**方案 A 单场景 LOD 融合**（把 L1 精细场景按工厂 footprint 嵌入 L0 园区、相机一路推近时低模盒子→精细场景 LOD 淡入，真·Google-Earth 式无缝；需统一两套坐标、改互斥挂载为共存、资源预加载/卸载时序，工作量大、动到已稳定的 B 阶段成果、回归风险高）；**方案 B 同构等距锚点交叉淡化 + 相机缓动（推荐）**（保持两组件独立，钻取时把被点工厂园区锚点/终态缩放传 L1，L1 首帧以「工厂居中、继承 L0 终态 zoom」姿态用 300-450ms ease 缓动到单厂 fit，容器 opacity 0→1 交叉淡入盖在 L0 冻结末帧上，返回反向；因两边同构等距，工厂恒在屏幕中心、缩放单调，视觉即「推近展开」，半天级、风险可控，复用 iso3d viewportCenterToWorld/zoomToFrustum 与 store 已有 one-shot camera3d*Request 机制；达成 C3 验收「无白屏/不掉帧/逐级返回」但非严格无缝 LOD）。**等用户在 A/B 间拍板后再写 C3 代码。**

> **下一步主线**：商业化 M1→M2→M3 S1-S5 全部完成并部署 Hasee。**M3 S6 Stripe 后端 A0-A4 + v39 迁移启动崩溃修复（7118990）已完成（本地未 push，mock 闭环），剩 A5 前端接线；真机待收款主体 + Stripe key；⚠️ 7118990 必须随下次 Stripe 代码一起部署，否则 Hasee（已在 v38）升级会启动即崩**。RTS 阶段 A+B 全部完成，**阶段 C 本轮 C1/C2 已完成（本地未 push，C2 浏览器走查通过），C3 连续 zoom 待在方案 A（单场景 LOD 融合）/方案 B（同构锚点交叉淡化，推荐）之间拍板后落码，C4-C8 留下一轮**。M1 回采产线继续 cron 自动攒数据。**下一步可选方向**：①S6 A5 前端 BillingTab 接线（本地可做）②RTS-C C3（待 A/B 拍板）③升级 agnes 付费 key（彻底解决 free tier 429）④真机 Stripe Step6（待收款主体）⑤翻译产线效果观察（3-5 天后看完成率变化）。
