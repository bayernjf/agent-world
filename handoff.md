...
State of Agent World as of 2026-09-19.
...
> **下一步可选方向**：①真机 Stripe Step6（商业化临门一脚）②升级 agnes 付费 key（彻底解决 free tier 429）③~~翻译 review 重跑丢原文~~ **已闭环**（`16db0a5` 已部署 + Hasee `71536df1` 实例已补边，待下一次 run 体检验证完成率 75%→94%） ④RTS 方案 A 单场景 LOD 融合（留档待真实多产线规模再评估）。
...
1. **style(web) 设计 token 历史债清理 + deferred RTS 阶段 C 对账（2026-09-19，工作分支 feature/20260824，`5db584f`/`0013c1b`/`6b3a4d2`，未 push）**——清账三笔：①`styles.css` 去 21 处 var() fallback：8 处硬编码色 fallback（Tailwind 残留 `#4ade80/#3b82f6/#22c55e/#e5484d/#35e0f0/#0f1623/#1f2937`，对应 token 明暗两套均有定义，删除后 dark 用标准语义色，并修掉 light 主题这些元素不随主题切换的潜在 bug）+ 嵌套 var/值 fallback（含 0 定义假名 `--text-muted`、`--radius-sm,6px`、`--mono,monospace`、`--z-canvas-chrome,5`、`--shadow-lg` 等，外层 token 均有定义、去掉等价）；顺带修真 bug `.compare-table` 把 shorthand `--hair`（值本身是 `1px solid …`）当颜色再套进 `1px solid` 致整条 border 声明被丢弃，改 `--border-primary`；运行时变量 `--inspector-width`（App.tsx 拖拽 setProperty 注入、420px 为首帧默认，width 不在禁列）明确保留。②7 个组件 10 处内联裸 px（CostReport/EvalReport/ABReport/ProductGallery×3/VersionPanel×2/KnowledgePanel 空态 padding 40/20、Settings fontSize12/marginTop8）改 `--space-10/--space-5/--space-2/--text-sm`（已核对 token 实值：--text-sm=12px、--space-2=8、--space-5=20、--space-10=40）。③deferred 表 RTS L0 行从「阶段 C 草案 / 预研模拟造 5–10 条产线」对账为已完成（C1–C3 PR #290、C4–C8 PR #292、polish PR #296 均合 dev 部署真机走查通过；C5/C6 按方案 B 裁剪、方案 A 留档待办 #50）。验证：web 顺序全量 102 文件/1942 测全绿（高并发一次 VersionPanel 停在「加载中」waitFor 超时为已记录负载 flaky，隔离单跑该文件 37/37 绿）、pre-commit 四包 typecheck 全 Done；无新增可见文案。

2. **fix(core) 翻译校对节点 gate 重跑丢英文原文（2026-09-19，`16db0a5`，已 push / PR #343 合 dev / PR #344 合 main，Hasee 部署 `da07741`，`71536df1` 实例已补边）**——治 #41 翻译 halted 真根因（09-19 node_runs 实证，见 Active work）：tpl-translation 的 review 只有 translate 一个 flow 前驱，inputFor 只聚合到中文初译、从不含英文原文，首次靠模型侥幸把中文润色过 QC，gate 退回重译后 review 仅收到中文便误当“原文”反问“初译在哪”，QC 判 VALIDATION halt（09-14/15/18 三次，修后翻译完成率 75%→约 94%）。修复：补 intake→review flow 边（置于 translate→review 前，inputFor 按 [原文, 初译] 聚合；rework 只重置 translate 及其下游、intake 产物保留，重跑 review 仍见原文），review prompt 显式标注【原文】/【初译】并禁止反问索材。新增 core 拓扑测试 + server 端到端 rework 回归测试（真实 tpl-translation 跑一次 gate 退回，断言 review 各 attempt 输入含两版；删边变异即红）。core 286 / server 1253 测全过、四包 typecheck 绿。✅Hasee M1 翻译产线 `71536df1` 已直接补边+换 prompt（2026-09-19 11:29 UTC，备份 `/tmp/m1-translation-before-fix.json`），待下一次 cron run 体检验证。**09-19 13:36 UTC 只读复核（Mac 经 hasee-2016-server SSH 活库 /var/lib/agent-world/agent-world.sqlite，mode=ro）**：实例补边与新 141 字 review prompt 均确认就位——review-02dfy 入边顺序为 intake-uqo4s→review 在 transl-oct5g→review 之前，prompt 显式【原文】/【初译】两段、严禁反问索材；但补边（11:29 UTC）后到复核时（13:36）尚无新翻译 cron run，最新 run 为 09-19 08:00（补边前）halted、qc-8s98z 仍因旧图「缺英文原文对照」，属修复前最后一次失败；翻译 cron 每 8h（00/08/16 点 UTC），**首次可验证窗口为 09-19 16:00 UTC（北京 09-20 00:00）启动、约 16:20 UTC 跑完**。当前 09-14 降频窗口完成率 13/18=72%、all-time 20/38=53%（interrupted 部署重启不计）；修复是否把完成率拉到预估 ~94% 须等该 run 证实，在此之前不下结论。


3. **test(server) Vitest 5 前向兼容：pg mock 提升到模块顶层（2026-09-18，工作分支 feature/20260824，`4e8ac88`，已随 PR #341 合 dev（merge `9a12d64`）部署 Hasee）**——Dependabot group PR #289（vite 6→8 / TS 5.7→7 / vitest 4→5 / coverage-v8 4→5 / undici 7→8，react/three minor 随附）CI build/typecheck 四包全过、core/mcp 全过，唯一硬失败是 server `connectors.test.ts` 在 describe 嵌套作用域调 `vi.hoisted`/`vi.mock`，vitest 5 判 Failed Suite（140 passed / 1 failed）致 PR UNSTABLE、已关未合。把该 postgres mock 整体移到模块顶层（mock 本就被提升、行为不变），全仓 grep 仅此一处嵌套；当前 vitest 4 下该文件 33 测全绿、pre-commit 四包 typecheck 全 Done。完整构建链多 major 升级留待 dependabot 重开 PR 专项（web 包在该失败 run 未跑完，见 deferred 安全/运维线）。

4. **fix(server) SSRF 防护 DNS 瞬时解析失败误拦模型域名（2026-09-18，工作分支 feature/20260824，`9f8a67e`，已随 PR #341 合 dev（merge `9a12d64`）部署 Hasee）**——治 Active work #41 观察项：Hasee 上 gate/质检节点偶发 `拒绝访问内网或私网地址（SSRF 防护）: apihub.agnes-ai.com`（run `a07c75eb`），实测该域名 30 次 DNS 全为干净 Cloudflare 公网 IP，根因是 `ssrf.ts` 对域名 `dns.lookup({all:true})` 瞬时 throw / 返空答案时 catch 也判 null（fail-closed），对外文案等同命中真内网。新增 `resolveDnsRecords`/`dnsSettle`/`DnsResolutionError`：仅 DNS 查询本身临时失败才有限退避重试（3 次、120/240ms 抖动），解析出内网/保留 IP 一次即拒不重试，耗尽仍 fail-closed，DNS-rebinding 双解析防护不削弱；`guardedFetch` direct 分支失败统一转 `GuardedFetchError("internal-target")`，`code-proxy.ts` 复用同一守卫。ssrf + code-proxy 共 37 测过、server typecheck 绿。

5. **fix(web) RTS 园区拖拽摆位 per-factory 防抖持久化（2026-09-18，工作分支 feature/20260824，`4204937`，已随 PR #341 合 dev（merge `9a12d64`）部署 Hasee）**——手动摆位 PUT 主体早已存在（后端 PUT/DELETE `/api/graphs/:id/park-coord`、SQL setParkCoord/clearParkCoord、读侧 parkX/parkZ、client `putParkCoord`、onPointerUp 防抖 90% 就绪），唯一真实缺陷：单个 `putTimerRef` 被所有工厂共用，拖完 A 后 400ms 内再拖 B 会 `clearTimeout` 掉 A 的 PUT，A 摆位只存本地 overrides、刷新回弹；unmount cleanup 也只 clear 不发最后一次。新增纯调度模块 `canvas/parkCoordScheduler.ts`（per-graphId `Map` 独立防抖，schedule/flush/dispose/pendingCount，persist 失败全吞不影响交互）+ 6 单测（含核心回归「A 后 300ms 拖 B 则 A/B 各发一次 PUT」），CanvasPark 接线、删共享 putTimerRef、unmount flush；scheduler 6 + CanvasPark 挂载 4 = 10 测过。同步订正 project-progress.md 中「手动摆位 onPointerUp 防抖仍未做」的过时表述。

6. **feat(web) 单厂 3D 节点常显名称标签 + graph-sync 资源释放加固（2026-09-18，工作分支 feature/20260824，`be72c8b`，已随 PR #341 合 dev（merge `9a12d64`）部署 Hasee）**——deferred 3D 美化⑦落地：每个节点建筑上方浮一个始终面向相机的名称 pill，无需 hover/选中即可辨认节点（与 RTS 园区工厂标签同款 billboard）。新增 `canvas/nodeLabel.ts`：模块级 `Map` 缓存 CanvasTexture（key `node-label|text|color`，避免重复创建、随页面生命周期存活），分类色文字（categoryColor）+ 深色圆角 pill，code-point 安全截断（MAX_LABEL_CHARS=12，emoji 代理对不拆），SpriteMaterial `depthTest:false/depthWrite:false`、renderOrder 10 保证不被建筑遮挡，`sprite.raycast=()=>{}` + userData.role=label 不抢节点拾取，name 缺失兜底 kind；jsdom 无 2d context 的 headless fallback 也写缓存以守住「同名同 kind 复用纹理」契约。**顺带修一个既有真实泄漏**：`disposeGroupChildren` 原为浅层遍历、只处理直接 isMesh 子节点，而 nodeGroup 直接子是 `shape.group`（isMesh=false）→ graph-sync 重建节点（重命名/拖动/增删）时旧建筑 geometry/material 不 dispose、也不从场景移除（累积旧网格、raycast 可能命中多个）；增强为对每个直接 child `traverse` 嵌套 Group/Line/Sprite，Mesh/Line dispose geometry、所有 material（含数组、Material 守卫）dispose、遍历后无条件 remove，Sprite 共享 three 内置 geometry 与共享 industrial / 缓存 label 的 material.map 绝不 dispose（避免误杀单例纹理）。nodeLabel 7 单测 + canvas 目录 11 文件 151 测全过、四包 typecheck 绿。

> 第 7 条及更早（**feat(server) gate 禁用词重写反馈逐处定位 + 命中计数（2026-09-18，`703c47d`，已随 PR #341 合 dev 部署 Hasee）**——新增 `nodes/prohibited.ts` `prohibitedHitsWithContext`，同词多处全报、列命中分句与真实计数，治旧逻辑每词只报首处致模型改不全反复退回 halt；7 纯函数测 + 1 集成测。**画布 3D 视角美化六项 + textGen 写实工业厂房原型（2026-09-17，`9f9cad3` 等，PR #327 merge `d651323b` 合 dev；ACES 色调/线性雾/地台描边/暗角/选中环呼吸/Bloom + 程序化混凝土厂房，详见 deferred 画布/可视化线与 project-progress 快照）**；**demo 零配置首跑 422 两轮修复并闭环（2026-09-16，PR #305 `5812aca` + PR #307 `76dbd42`，均合 dev 部署 Hasee；根因为未登录时 `getSettings()` 401 竞态把 model 清空，改为仅 settings 成功才置 modelOptionsReady，真机首跑不再 422）**；**跨厂物流拱线 + ROI 热度、部署脚本单一事实源 + 蓝图 P0/P1 对账（2026-09-15，`45f65bf`/`16d8503`，PR #296 merge `4b1cd90` 已部署 Hasee、引导链路真机验证通过）**——①RTS C2 polish：跨厂物流管线由贴地直线改为越过厂顶的抛物线拱（弧顶 y≈236 > 厂高 140，等距俯视不再被中间厂方块遮挡），opacity .32→.55，卡车沿拱爬升、crossGroup 重建时释放旧管线 geometry/material（卡车共享材质不释放）；新增并导出无 three 依赖纯函数 crossArchY/crossArchPoints。②C6：工厂热度 sprite 在 CTR·GMV 后补 `ROI=gmv/adSpend`（gmv、adSpend 皆正才显示，adSpend=0 绝不显示 ∞，顺序固定 CTR·GMV·ROI）；后端 ad_spend 链路（content_metrics 列→metricsByGraph→ParkGraphMetrics.adSpend）早已就绪、纯前端补；+5 单测（park-label 8→13）。③运维：修复服务器根 `/opt/agent-world/deploy.sh|rollback.sh` 为 **untracked 手工副本、git pull 永不更新**的漂移隐患——仓库 `scripts/deploy/*` 成为唯一逻辑源（deploy 改为「仅当部署前当前服务健康才写 last-known-good」，避免失败部署把坏 commit 写成回滚点；deploy/rollback 均最多 15s 轮询健康），服务器根两份改为 `exec bash scripts/deploy/*.sh` 引导（备份在 /tmp/*.root-bak），手动跑一次引导部署 `deploy OK: 4b1cd90` 验证链路；`docs/engineering-blueprint.md` 加「现状对账（2026-09-15）」（12 域自包含 P0 全具备，仅独立 Playwright E2E 冒烟缓至对外开放注册前，其余 P1/P2 卡域名/上云/规模化触发，当前不做以避免过度设计）。真机走查：3D 园区工厂方块/24h 排期轴/经济栏/倒计时环/节点详情正常、单产线 3D 链路正常、零 console error；staging crossEdges=0 且无 content_metrics，拱线/ROI 真机为空态（诚实不渲染），几何与格式化逻辑由 13 个 park-label 单测保证。；**feat(web) 版本对比长文本字段逐字高亮（2026-09-18，`0387f97`，PR #341 合 dev）**——graph-diff 新增 diffText/tokenizeText（token 级 LCS 逐字高亮，超长回退整段）+ VersionPanel InlineTextDiff，6 纯函数测 + 1 组件测
...
* 测试总数 **3550**（2026-09-18 Node 24 全量实跑全绿（EXIT=0）：**core 21 文件 285、server 153 文件 1252、web 102 文件 1942（vitest 全量；`--no-file-parallelism` 顺序 1942 全过）、mcp 3 文件 71，共 279 文件**。相对当日早前 snapshot（3532 / 277 文件）+18 测 / +2 文件：当日晚 #35 SSRF DNS 瞬时失败有限重试 server +5、#36 parkCoordScheduler web +6（新测试文件）、#37 nodeLabel web +7（新测试文件）；core/mcp 本轮零改动，沿用当日早前实跑值；数字一律以本次实跑为准）：
