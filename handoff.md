...
1. **★ Phase 2 持续推进：文件转换节点（本轮完成）**— **`convert` 节点落地**（core + server + web 全链路）：core 新增 NodeKind.convert + ConvertConfig schema（source / to: image|png|jpeg / quality 默认 85）；server 执行分支两类转换——**PDF → 图片**（复用 parse-file 重构出的共享 `extractPdfImages`：pdfjs 提取每页内嵌图片并 pngjs 重编码 PNG，扫描版 PDF 每页一张，与 OCR 天然串联）、**图片 → PNG/JPEG**（新增 `convert.ts`：魔数探测格式，pngjs + jpeg-js 纯 JS 解码/重编码，支持批量与 JPEG 质量）；无唯一上游 / 无可转换产物 / 纯文本 PDF → VALIDATION，读取或解码失败 → PROVIDER_ERROR；web 工具栏按钮 + Inspector 面板（来源 / 目标格式下拉 / JPEG 质量条件显示）+ 标签配色。core 4 + server 6 个新测试（PDF 提取、png→jpeg、jpeg→png、docx 双图批量、无输入 VALIDATION、坏数据解码失败；fileparse 6 用例回归确认重构无损）。**Phase 2 最后一项**：搜索（Google/Bing/SerpAPI）。HTML→PDF 未做（纯 JS 无中文字体排版方案，需要浏览器引擎或嵌入字体，待后续评估）。
...
- `pnpm -r typecheck`：全绿
- `pnpm --filter @agent-world/core test`：125/125 通过（含 convert / ocr / translate / fileParse schema + HTTP file 模式用例）
- `pnpm --filter @agent-world/server test`：345/345 通过（含 convert：PDF 提取图片、png↔jpeg 互转、批量转换、VALIDATION、解码失败；ocr 与 fileParse 全部用例）
