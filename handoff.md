...
> **推送 / 部署对账（2026-09-26 用 `git ls-remote` 直查远端，不看本地 remote-tracking ref）**：`origin/main` = **`9c5d764`**、`origin/dev` = `1d894c8`、`origin/feature/20260824` = **`90289b4` = 本地 HEAD**——即 ①②③④、MVP 收尾批、sink 收窄、以及本会话的 docs 提交**全部已在公网那份仓库上**。**本节下面各条目里写于当时的「未 push / 未合 dev」一律作废，以本行为准**；此前我自己也报错过一次（拿未 fetch 的 `origin/*` 算出「领先 N 个 commit」，那是假的）。仍未闭合的是**部署事实**：`9c5d764` 有没有真在 Hasee 上跑，仓内无法自证（针对 `eff8ebe` 的那次 deploy 结论是 `skipped`，评审 §9.2），且线上 commit 需要有人 SSH 看一眼 `/api/health`——归待办 #60（该项描述里的 `eff8ebe` 也已过期）。
