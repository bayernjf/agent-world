...
1. **feat(server,web) F5 批量任务编排（2026-09-03，commits `075ffea`/`3cadbfc`）**——按 design-ecommerce-roadmap §F5：server 增 `batch_jobs`/`batch_items` 表（迁移 23）+ runs 加 batch_id/batch_item_id + `/api/batches` create/list/detail/retry + `batch.ts` 并发调度器（逐行 startRun、onFinish 回写状态）；web 增 BatchManager（创建/轮询/展开详情/失败重跑）。新增 2 用例（server）。
