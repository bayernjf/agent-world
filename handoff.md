...
1. **feat(core,server,web) F7-A 平台化导出包（2026-09-03，commits `f5565f2`/`aefbf24`/`1fdbe2a`/`09acd9d`）**——按 design-ecommerce-roadmap §F7 阶段 A：core 增 `publish` 节点 + `buildPublishPackage`（复用 F3 profile 拆标题/正文/话题标签/比例清单）；server 增 publish 执行块；web 增「发布台」节点 + Inspector 面板。新增 7 用例（core 5 / server 2）。
