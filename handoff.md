...
- **核心能力**：4 类 AI 节点（agent / imageGen / videoGen / audioGen）+ **通用节点（HTTP 请求 / 代码执行 / 条件分支 / 映射 / 循环 / 并行聚合 / 表格处理 / 数据库查询 / 文件解析 / 翻译）**，**MCP Server（stdio + HTTP/SSE 双传输，6 工具 + resources + prompts）**，多产线管理，Inspector 模型下拉严格按 modality 过滤，多模态产出（Artifact 分层），流式 + SSE + 断线重连 + halt/resume，成本电表（token + 单价两种模式），评估体系雏形，产物落库归属流水线（artifacts 的 graph_id/role）
...
1. **★ Phase 2 持续推进：翻译节点（本轮完成）**— **`translate` 节点落地**（core + server + web 全链路）：core 新增 NodeKind.translate + TranslateConfig schema（source / target / model / temperature / budgetUsd）；server 执行分支复用 worker.runAgent（无 tools 的纯 LLM 调用）：读上游 text 产物（无 text 回退 JSON 序列化）→ 按目标语言构造翻译系统提示（忠实原文、保留结构、不加前后缀）→ 流式输出 + 重试（RETRYABLE 表）+ 成本核算（power.metered / 节点预算 BUDGET 封顶）；无唯一上游 / 上游无文本 → VALIDATION；web 工具栏按钮 + Inspector 面板（来源下拉 / 目标语言 / 模型 / 温度）+ 标签配色。core 4 + server 4 个新测试。**Phase 2 其余大项**（OCR、文件转换、搜索等）见 roadmap-generalization。
...
5. `c69788a` — **feat(web)**: **Inspector 模型未配置时显示去设置入口**。agent/image/video/audio 四个模型下拉在对应 modality 无可用模型时，显示「前往设置 · 模型与密钥」链接（原来只有占位项，无引导）。
...
- `pnpm -r typecheck`：全绿
- `pnpm --filter @agent-world/core test`：117/117 通过（含 translate schema + fileParse schema + HTTP file 模式用例）
- `pnpm --filter @agent-world/server test`：334/334 通过（含 translate：LLM 翻译链路、显式 source、无唯一上游 / 上游无文本 → VALIDATION；fileParse 与 database 全部用例）
