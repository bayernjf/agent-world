...
* [docs/design-demo-user.md](docs/design-demo-user.md) — 演示用户（免注册一键进真实产品，is_demo 标记真实账号 + 体验额度 + demoGuard 能力黑名单 + claim 原地转正 + TTL 级联清理；迁移 v40、D1-D6 分步，**2026-09-15 设计完成待评审，未写代码**）
...
52. 🔵 **演示用户（Demo User）：免注册一键进真实产品（2026-09-15 立项，方案 docs/design-demo-user.md，D1-D6 待开工，未写任何代码）**：新用户在登录页点「先体验演示」即建一个带 `is_demo` 标记的**真实账号**（复用 userId 隔离/JWT/订阅全套，转正时同 userId 原地保留数据），能力介于未登录与正式账号之间。**核心设计**：①迁移 v40 users 加 `is_demo`/`demo_expires_at`（sqlite+pg 双份、db.ts 抽象层）；②三端点 `POST /api/auth/demo`（ALLOW_DEMO 开关+IP 限流+同浏览器复用+克隆纯文本模板预置+短会话）、`GET /me` 带 isDemo、`POST /api/auth/claim`（原地转正保数据），登录路由拒 is_demo 密码登录；③`demo-guard.ts` 能力黑名单（禁改密/外联 publish/webhook/远端 MCP/自定义 connector/admin/billing，403 DEMO_LOCKED）；④独立 `DEMO_QUOTA`（默认 30k token/禁视频音频/并发1/≤15 run/20MB，不新增 PlanId 避免污染定价），run gate 加 demo 分支（注意 Hasee MONETIZATION_ENFORCE=1 且 free token=0，demo 必须正确放行）；⑤prune-demo-users 级联清理（24h TTL、--dry-run、删 users 带 AND is_demo=1 兜底）；⑥前端登录页入口+useSession store+DemoBanner+ClaimDialog+DEMO_LOCKED 统一引导+i18n。**6 个待拍板点（文档 §13，均有推荐默认）**：账号复用+24h TTL / 体验额度数值 / 预置哪些纯文本模板 / cookie 与账号有效期 / ALLOW_DEMO 默认关 / 转正原地保留。**下一步**：用户拍板 §13 后按 D1（迁移+driver）开工，估 4-5 工作日。

> **下一步主线**：商业化 M1→M2→M3 S1-S5 全部完成并部署 Hasee；**M3 S6 Stripe 全栈（后端 A0-A4 + v39 修复 7118990 + 前端 A5）已随 PR #287/#290 合 dev（曾为 `27f28ee`）部署 Hasee，schema v39 迁移成功、subscriptions Stripe 三列三索引就位、缺 key 优雅降级不阻断启动，只剩真机 Step6（待收款主体 + STRIPE_SECRET_KEY/WEBHOOK_SECRET/PRICE_IDS）**。RTS 阶段 A+B 完成，**阶段 C 全部完成：C1/C2/C3 随 #290、C4-C8 随 #292（merge `1f5da5e`）合 dev 并部署 Hasee、真机走查通过**（C5 改期为点击延后/cron 只读、C6 暂只 CTR/GMV 两处裁剪见待办 50；方案 A 单场景 LOD 融合仍留档待真实多产线规模再评估）。M1 回采 4 条产线继续 cron 自动攒数据 + 每日 10:30 体检。**下一步可选方向**：①真机 Stripe Step6（待收款主体 + Stripe 测试 key，用测试卡走端到端，商业化临门一脚）②升级 agnes 付费 key（彻底解决 free tier 429，方案 A）③翻译产线效果观察（09-14 降频+放宽 QC+retry 后，观察到 09-17/19 看完成率变化）④~~RTS 遗留 polish~~（**已随 PR #296 清理**：跨厂管线改走厂顶抛物线拱 + opacity 提亮至 .55、C6 ROI 已补——后端 ad_spend 链路本就就绪、现只待真实广告花费数据即可显示、所谓「3D 审计 6 项 low」经核已于 09-11 全部清零不存在未清项；另修复服务器部署脚本 untracked 手工副本漂移、完成蓝图 P0/P1 对账）。
...
* 测试总数 **3331**（2026-09-15 PR #296 后实跑：仅 web 变动 1869→1874 +5，server/core/mcp 未动沿用上次数；顺序跑 web 95 文件 1874 全过）：
  ...
  * `pnpm --filter @agent-world/web exec vitest run`：**1874 总数**（95 文件；组件目录零「有 .tsx 无 .test.tsx」。RTS-C C4-C8 增量 +24（ParkEconomyBar 5 / ParkScheduleAxis 6 / FactoryReviewCard 5 / park-label 8 等）；PR #296 再 +5：park-label 8→13（ROI 1 + crossArchY/crossArchPoints 4））。⚠️ **本机全量高并发下有既有负载 flaky**：个别文件（VersionPanel/ProductGallery/PublishTargets 等，每次不固定、均不在改动范围）因 waitFor 超时失败或 worker 提前退出，**隔离单跑全绿、`--no-file-parallelism` 顺序跑 95 文件 1874 全过、CI Linux 全 pass**，证实非回归；判断回归只看改动文件 + 顺序全量 + CI。
