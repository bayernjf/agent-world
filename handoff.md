...
1. **refactor(web) 设计 token 渐进式迁移全部完成（30 批，2026-09-03）**——styles.css 全局样式全部迁移到 semantic token（commits `09abc4d`~`7d0b9da`），覆盖全局基础/HUD/组件/画布/管道/产线/时间线/检查器/产物/画廊/产品/设置/触发器/引导/知识库/运行历史/模型分配等所有模块，全量 1460 测试每批验证通过无回归
2. **feat(web) 设计 token 体系基础设施 + i18n 国际化基础设施（2026-09-03）**——① 设计 token（`9259a38`）：补充完整 Primitive 层（间距 12 级/圆角 7 级/阴影 6 级/字号 8 级/行高 4 级/字重 4 级/动画 3 级）+ Semantic 层（背景/文字/边框/功能色/accent/语义间距圆角阴影）+ 明暗主题切换（`[data-theme="light"]`），保留原有 26 个 token 向后兼容；② i18n（`008c844`）：安装 i18next + react-i18next + i18next-parser，7 命名空间初始化（common/canvas/nodes/modals/settings/run/errors），完整 zh/en 双语翻译包（1800+ keys），语言自动检测 + localStorage 持久化，全量 1460 测试无回归
