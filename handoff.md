...
1. **feat(server,web) F9 内容级成本归因（2026-09-03，commits `069ee05`/`59424bc`）**——按 design-ecommerce-roadmap §F9：server 增 `content_costs` 表（迁移 26）+ `/api/content-costs` + `/api/costs?groupBy=` 内容级聚合；web 在 PerformanceDashboard 增「内容成本」区块（成本/GMV/ROI 聚合 + 录入）。新增 3 用例（server）。
