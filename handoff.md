...
State of Agent World as of 2026-09-17.
...
> 🔧 **待办 #55（2026-09-16，竞品痛点应对·安全子集两批均落地、已随 PR #319 合 dev 2026-09-16 16:44 UTC，merge `4163efb`）：步级可观测（G1 只读链路 + 完整输出懒加载）+ 上游契约纯函数/配置形态 + 超时纯函数**。依据 `docs/design-step-trace-and-robustness.md`，只落地**不改 run 执行状态机**的安全项（绝不能影响 Hasee M1 回采）。工作分支 feature/20260824，两批改动均已随 PR #319（merge `4163efb`，2026-09-16 16:44 UTC）合 dev：
> - **G1 只读时间线（端到端可用）**：①core `04a1a55` 新增纯函数 `buildTimeline(events)`（`packages/core/src/trace.ts`，按 nodeId×attempt×variant 聚合节点/尝试/状态/耗时/tokens/cost/model/score/gate 裁决/错误码/输出预览/工具调用/产物，tokens/cost 只计每节点最后一次 attempt 避免重试重复计，乱序未知事件忽略不抛错）；②server `02f5b65` 新增只读 `GET /api/runs/:id/timeline`（requireRun viewer，直接投影 events 表、**无 DB 迁移**，并从 run.snapshot best-effort 解析 nodeMeta 节点名/kind）；③web `4aba58d` 新增自包含 `RunTimelineView`（运行历史每行"步骤"按钮内联展开，展示每节点全部 attempt 含重试、状态徽章、耗时/model/token/成本/评分、gate ✓✕、错误码+信息、输出预览、工具/产物数、合计与预算熔断警告，zh/en i18n + CSS）。
> - **G2.1 上游数据契约纯函数（已写已测、尚未接线）**：`contract.ts` `validateContract(contract,output)`/`coerceOutputObject`/`describeContractFailure`（无契约/空断言恒通过=向后兼容；空串/空数组/空对象按缺失；自定义 `SCHEMA_VIOLATION` 字符串，因 ErrorCode enum 暂无该项）。
> - **G4.1 超时判断纯函数（已写已测、尚未接线）**：`deadline.ts` `isTimedOut`/`remainingMs`/`deadlineAt`（timeoutMs 空/非正=不限时）。
> - **验证**：core 新增 24 测（trace 8/contract 11/deadline 5）全绿、server timeline 3 测（401/主投影/404 跨用户）全绿、web 组件 2 测 + RunHistory 既有 40 测无回归；四包 `pnpm -r typecheck` 全绿。**注意 server/web 消费 core 的 dist，改 core 后须先 `pnpm --filter @agent-world/core build`（dist 被 gitignore）。**
> - **第二批①·G1 完整输出懒加载（端到端，只读自洽）**：server 在 timeline 端点后新增 `GET /api/runs/:id/nodes/:nodeId/attempt/:attempt/output`（viewer 授权；完整字符串直接从 `node.finished` 事件投影，**无需改 driver / 无 DB 迁移**；attempt 非整数/<1 返回 400；main 变体优先、缺失回退最后一个变体；8 HTTP 测全绿）。web `RunTimelineView` 截断输出旁加「展开完整输出 / 收起」按钮，按需懒加载、三态（加载中/失败重试/完整 pre，max-height 480 滚动），`lib/api.ts` 加 `getRunNodeOutput`（nodeId encodeURIComponent），zh/en run.json + CSS，组件 4 测全绿（测试环境为 en，断言用 class 选择器）。
> - **第二批②·G2.3 输出契约配置形态（core schema + Inspector UI，纯配置、G2.2 前不生效）**：core `contract.ts` 加 `ContractSpecSchema`（zod，requiredFields default []、types 为 `Record<string, CONTRACT_FIELD_TYPES>`），`graph.ts` 的 `GraphNode` zod schema 加**可选顶层字段 `contract`**（旧节点向后兼容），core build 通过、contract 测试 16 绿。web `Inspector.tsx` 新增 `ContractFields` 子组件挂在「配置」tab（所有 kind 可见；`<details>` 折叠，可增删必填字段、逐字段选 string/number/boolean/array/object 类型；空契约写 `{requiredFields:[]}` 恒通过校验、无副作用），zh/en `nodes.json` inspector.contract 9 key + scoped CSS，Inspector 测试 **20 绿（新增 3）**。**诚实标注：因 G2.2 engine 未接线，用户在此配的契约只保存、不会真正拦截 run**，UI 第二条 hint 已明示，禁止对外宣称"契约护栏已生效"。
> - **验证（第二批）**：四包 `pnpm -r typecheck` 全绿（web 初报 `types` Partial↔Record 不兼容，已把前端 cleanTypes 对齐为 zod infer 的 `Record<string, ContractFieldType>` 后转绿）；server timeline/output 8 测、web RunTimelineView 4 测 + Inspector 20 测、core contract 16 测分别全绿。**注意 server/web 消费 core 的 dist，改 core 后须先 `pnpm --filter @agent-world/core build`（dist 被 gitignore）。**
> - **明确留待（改 run 执行核心，须单独评估、防打断 M1，本轮不做）**：G1.2 从节点 fork 重跑 API；**G2.2 engine 契约接线**（节点完成 output 后、下游派发前调 `validateContract`，违例 failed + `SCHEMA_VIOLATION`，需先定是否扩 ErrorCode enum；**它是让 G2.3 配置真正生效的前置**）；G2.4 给模板数据源节点批量预置 contract（**等 G2.2 接线后对照真实输出逐个核对字段名**，避免字段名写错在未来误拦；原料台/文本节点输出纯文本/Markdown，coerceOutputObject 判 notObject，本就不该配）；G4.2 timeout+degraded 降级状态机；G4.3 前端"继续/降级"按钮；G4.4 视频远端进度轮询；G3 节点 maxRetries 下放 + 预算 UI。**已随 PR #319 合 dev（merge `4163efb`，2026-09-16 16:44 UTC）并由 CI self-hosted runner 自动部署 Hasee；真机走查入口：运行历史→某 run「步骤」→截断输出旁「展开完整输出」，与 Inspector「配置」tab「输出契约（高级）」折叠区。**

> ✅ **#55 续·执行核心四项收口（2026-09-17，工作分支 feature/20260824，5 个原子 commit，已随 PR #325 合 dev，merge `fb00657`）**：把上一条"明确留待"里不卡外部、空契约/默认值向后兼容、对 M1 回采零影响的四项点亮（依据 `docs/design-step-trace-and-robustness.md`，进度表已同步）：
> - **G2.2 输出契约 engine 接线**（commit `c7beaea`）：core `ErrorCode` 扩 `SCHEMA_VIOLATION`；engine 在唯一发包出口 `sendPackets` 开头加 `enforceContract` 闸门（`contractChecked` Set 去重；仅对 done 且声明了断言的节点用 core `validateContract` 校验，违例删产物 + 置 failed + emit `SCHEMA_VIOLATION` + 返回 false 拦截 flow 包），runNode finally 兜底末端不发包节点。**关键坑（勿重走）**：闸门必须在 handler 同步段的 `sendPackets` 内（schedule 的 launch while 在同一调用栈继续扫描），放 finally 会因 microtask 时序让同步完成节点（source）的下游提前启动。SCHEMA_VIOLATION 是确定性失败、不在 RETRYABLE 集、不重试，error 边可 catch。9 engine 集成测。Inspector 契约 hint 从"尚未接线"改为"已生效"，nodes.json zh/en 加 `errorLabel.SCHEMA_VIOLATION`。
> - **G1.2 从节点 fork 重跑**（后端 `3621db7` + 前端 `8dc75aa`）：语义=fork 点及其全部上游复用父 run 产物（合成零成本 `reused:true` 的 node.finished、不重跑不计费），只重跑 fork 点的 flow 下游（不含其自身）；新 run 全新 runId、seq 从 0、继承父 run `budget_usd`、`trigger="fork"`，父 run 审计轨迹不动。core `node.finished` 加可选 `reused`、`buildTimeline` 透传；engine 末尾新增 `fork()`（reconstructState→BFS 求 reset 集（从 fork 点 flow outgoing 起、不含自身）→清下游产物/计费/skip→seed states→只保留非 reset 集的 packet.sent→为存活 done 节点合成 reused 时间线→以 `resuming:true` 跑 scheduler）；`run.ts` 加 `forkRun()`（镜像 startRun 的建 run/事件/产物/计量/告警管线，建 run 前先校验 fork 点有产物，无则 422 不留即失败 run）；`POST /api/runs/:id/fork {fromNodeId}`（editor→403、缺参→400、running→409、坏节点/模型→422）。前端每个成功节点加「从此处重跑」按钮、reused 节点显示「复用」角标，RunHistory fork 后刷新并打开新 run。后端 8 测（engine 3 + HTTP 5）、前端 3 测。
> - **G3 节点 maxRetries 下放 + 预算 UI**（commit `ae31c38`）：核查发现**后端各 handler 早已读 `<kind>.retry.maxRetries`**（engine.reliability 已覆盖 0/2/3/4、退避、attempt 一致性）、run 级 `budget_usd` UI（ControlPanel 预算输入→startRun→超 budget halted）也早已端到端；真正缺口是前端无可视配置。新增共享 `RetryField`（clamp 到 schema [0,10]，文案区分 infra 重试与质检返工），挂到 7 类带 retry 策略的节点（textGen/http/code/translate/search/notify/vcs；generic 无 Inspector 字段面板、image/video/audioGen schema 无 retry 字段，均不挂），zh/en nodes.json 加 `inspector.common.maxRetries/maxRetriesTitle`，2 测。
> - **G4.4 视频远端进度轮询（仅本次运行内子集，commit `dfebcca`）**：Worker 加**可选** `submitVideoJob`/`queryVideoJob` 接缝（`VideoJobHandle/VideoJobPoll`）；videoGen 节点两方法齐备时"提交一次→指数退避（2s..20s）轮询→5min 上限 TIMEOUT"，远端 TIMEOUT/RATE_LIMIT 码透传；缺一方法字节级回退同步 `generateVideo`。**当前无 provider 实现该接缝，生产仍走同步路径，provider 就绪即自动启用**；5 测（退避纯函数/submit-poll-succeed/远端 failed/RATE_LIMIT 透传/轮询超时）。**跨 run 断点续跑（重启后凭 jobId 重新附着、不重投、不重复计费）依赖 G4.2 degraded/halt 状态机，明确随 G4.2 落地，不在本次范围。**
> - **验证**：Node 24 下四包 `pnpm -r typecheck` 全绿；core 20 文件 275 测、server 150 文件 1232 测、web 100 文件 1910 测（`--no-file-parallelism`）、mcp 3 文件 71 测，合计 **273 文件 3488 测全过**；i18n 守护 `keys.test.ts` 绿（新增 key zh/en 同构、无硬编码中文 JSX）；新增 CSS 仅 `.run-timeline-reused/-fork/-forkerror` 三个 class 且全用已核实 token（`.run-timeline-retry` 存量 `#ffc78a` 是既有违规未动）。server 高并发全量偶发 `audit write failed: table locked` flaky（SQLite 表锁竞争，重跑即过、非回归）。**已随 PR #325 合 dev（merge `fb00657`）；Hasee 部署状态未核实。**
> - **本轮后仍留待**：G2.4 模板预置 contract（G2.2 已接线，可对照真实输出核对字段名后预置）；G4.2 degraded 状态机 + G4.3 前端「继续此节点」+ G4.4 跨 run 续跑（三者绑定，改执行核心，避开 M1 回采关键期）。**G5 已于 2026-09-18 落地，见下条「#55 续二」。**

> ✅ **#55 续二·G5 prompt 热迭代闭环落地（2026-09-18，工作分支 feature/20260824，已随 PR #331 合 dev、merge `26ec4a5`，已部署 Hasee 且 health 探针 commit 一致；方案见 `docs/design-ab-testing.md` §5，进度表见 `docs/design-step-trace-and-robustness.md` §4）**：G 系列最后一块"只有方案、没写代码"的安全子集，纯只读投影 + 编译独立 run，**不改 run 执行状态机、不碰 M1 四条回采产线、不部署**。
> - **G5.2 core**（`packages/core/src/ab-gate.ts`，新增）：`findDownstreamGate(graph,targetNodeId)` 只沿 **flow 边 BFS**（不跟随 rework/error 边）找目标下游最近 gate，标记最短路径上是否经过内容变异节点（textGen/imageGen/videoGen/audioGen/generic/translate；branch 等控制节点不算）；`projectGateVerdict(...)` 复用 `buildTimeline` 折叠该 gate 各 attempt 的 verdict、取**最终一次**，给 `passed/score/reason/minScore/meetsBar/mutatesInBetween/history`，无 gate 返 null。10 单测，core 全量 285 过、build 成功。
> - **G5.1 + G5.2 server**：`POST /api/runs/ab` 增可选 `fromRunId`（fail-closed：不可见/不存在 404、跨图/非 done/snapshot 无目标 textGen/空 input 各 422，任一失败绝不发起 arm run）；取样时 Arm A 自动前置**当前 live graph** 的现网 prompt、input 投影自 runs.input 列、用户只填 ≥1 个新 prompt；不带 fromRunId 维持 ≥2 全手填向后兼容。`GET /api/runs/:id/timeline` run 对象带 `input`。abReport 每 arm 选代表 run（优先 done、最近）批量取 events，经 projectGateVerdict 挂 `ABArmReport.gate`（pg-driver 复用同一 createDriver，一改两驱动生效）；winner 推荐逻辑不变、gate 仅呈现。`api.ab.test.ts` 6 测 + `ab.test.ts` G5.2 集成 6 测，server typecheck 干净（macOS 全量 ~34 个代码沙箱子进程基线 failed 非本次回归，CI Linux 全绿）。
> - **G5.1 + G5.2 web**：RunTimelineView 在 run+节点均 done 且为 textGen 时加「以此样本做 prompt 对比」ghost 按钮（RunHistory/App 透传 `ABCompareSample`）；ABDialog 取样模式（目标锁定、input 只读预填、≥1 新 prompt、自动算 Arm A）；ABReport 增「质检站判定」列（通过/未过徽标+score、低于 minScore 提示、无 gate/无 verdict/中间变异四空态、reason 截断悬浮、注脚声明不参与自动推荐）；zh/en i18n 13 key。受影响 4 文件 96 测全过，web 全量 1922 测仅 EvalReport 1 个全量并发异步 flaky（单跑 27/27 过、与本次无关），web typecheck 干净。
> - ✅ **已收口（2026-09-18 复核）**：四个英文原子 commit 已落地——core `8ee7fe3`、server `cb396f1`、web `ff2e017`、docs `71c2d1f`，hash 已回填 design-ab-testing.md §5；已随 **PR #331 合 dev（merge `26ec4a5`）并由 self-hosted runner 自动部署 Hasee**，health 探针 `commit=26ec4a5 db=ok` 确认。

> 🔧 **#56（2026-09-18，Windows 设备会话）：gate 禁用词重写反馈增强 + handoff 对账 + Windows 本机质量门基线**。用户在 Windows 同一仓库（`C:\000mycodes\agent-world`，分支 feature/20260824）拍板“一口气搞 1/2/3/4”。
> - **③ gate 禁用词重写反馈带命中词与上下文（✅ 已落地，commit `703c47d`，未 push）**：治 deferred-items 编排线登记的真实痛点——旧 `prohibitedSnippets`（nodes/shared.ts）每个禁用词只取首处（indexOf）、固定 ±12 字窗口、`replace(/\s+/g,"")` 强去空白；同一禁用词在文中多处出现时重写反馈只点一处，模型改不全就反复命中直到 `onExhausted=halt` 零产出（事故 run `qc-k7qhs`，命中「最」）。新增纯函数 `nodes/prohibited.ts` 的 `prohibitedHitsWithContext`：按句读边界（`。！？!?；;\n\r`，中文逗号不切）取完整分句、超长无标点回退 ±16 字窗口、同一词多处全部上报（每词 perTerm=2、总 totalSnippets=6 封顶、去重）、`count` 报真实总次数；gate reason 改为“命中禁用词共 N 处（词A×n、词B），第 X 次质检。出现位置：「词」分句；…，请逐句改写并通读全文确认无其它遗漏处”。重写输入拼接链路（engine.inputFor 末尾 `[质检站退回原因]`）与 `onExhausted=halt` 停机策略**均不变**（纯增强、向后兼容）；删除 shared.ts 旧函数与 engine.ts 遗留 import。后端运行时 reason 沿用现有中文硬编码约定（非前端 JSX，不走 i18n）。测试：新增 `nodes/prohibited.test.ts` 7 纯函数测 + `gate-prohibited-feedback.test.ts` 1 集成测（两处「最」全报、reason 含“共 2 处/最×2”/两分句、第二次 forge input 含退回原因）全绿，phase1 13 测全绿（向后兼容），server typecheck 绿。
> - **② handoff/设计文档对齐真实 git 状态（✅ commit `f6ac9b5`）**：handoff 仍有“G5 待 commit/未 push”过时表述，核实 G5 四 commit 已随 **PR #331（merge `26ec4a5`）合 dev 并部署 Hasee**（health `commit=26ec4a5 db=ok`），改 handoff 索引/标题/待办 3 处 + design-ab-testing.md 状态 1 处。
> - **④ Windows 本机质量门快检（✅ 环境打通 + 基线甄别，详见 Quality gate「Windows 本机测试基线」）**：本机 PATH 上只有各应用自带 Node v22、仓库无 node_modules，装便携 Node v24.11.0（`C:\Users\24670\node24`，corepack 代理 pnpm、npmmirror 源）、`pnpm install` + core build 成功，server typecheck 绿。全量 153 文件 1247 测 **1112 过 / 135 失败（56 文件）**，逐类坐实**全部为 Windows 平台基线、与本次改动无关**：(a) 凡用临时 sqlite 目录的测试 afterEach `rmSync(dir,{recursive,force})` 抛 **EPERM**（Windows 文件句柄/WAL/索引器或杀软扫描未释放，db.* / costs / migrations / runs / graphs / events / invoice / subscription / usage-metering / isolation / graph-versions 整片）；(b) 代码沙箱后端缺失（Windows 无 bwrap/seatbelt/rlimit，code-sandbox 的 child_process 拒绝、RLIMIT_NPROC fork-bomb 2 测失败，engine.code/loop/table/map/parallel-join/generic、regression/core-path 连带）。**铁证**：`git stash -u` 在干净 HEAD 重跑 db.operations（8/8 失败）、engine.loop（5/6 失败）与带改动完全一致。Windows 判断回归只看改动相关测试 + typecheck，权威全绿以 CI Linux / macOS 为准。
> - **① M1 四产线 09-14 降频后观察小结（✅ 2026-09-18 05:33 UTC 由 Mac SSH 取数完成，走原方案 (c)——Mac `hasee-2016-server` 免密直连，Windows 公钥授权不再需要）**：优化（cron 4h→8h 等 + 放宽 QC + RATE_LIMIT/TIMEOUT 退避 retry）后窗口（09-14 15:00 UTC 起）完成率全面回升——①写草稿 84/87=**97%**（另 3 interrupted 为部署重启）、②翻译 8/11=**73%**（优化前累计仅 ~33%）、③短视频 8/8=**100%**、④批量 3/3=**100%**；累计完成率①94%/②51%/③86%/④82%。**②翻译自 09-16 00:00 起连续 7 个 cron run 全 done**（09-16/09-17 全天 8h 档 + 09-18 00:00），09-15 16:00 后「模型未翻译、反问要初译稿」的 halted 未再复现——该问题为间歇性（最近 3 次 halted 均在 09-13/09-14/09-15 16:00，属优化窗口早期），根因未修但当前被降频节奏自然压住，观察窗 09-19 前继续攒数据，不阻塞。结论：降频优化对四条回采产线均有效，M1 继续无人值守攒数。

> ✅ **#54 已闭环（2026-09-17 复核）：窄视口（实测 CSS 视口宽 ~863px）顶部 HUD / 画布工具条按钮与统计文字被 flex 压成竖条，另发现折叠 Inspector 撑出 420px 横向溢出**。真机 DOM 探针定位 **9 处竖折**：`.hud__meta` 统计 span 被压到 12px 宽逐字竖排成 297px 高黑条（视觉上的"空黑框"即 meta 区本身）；`.hud__actions` 的 `.chip`（收起侧栏/待审核/成品库/术语表/快捷键？）被压到 38px 文字竖排；`.canvas-toolbar__btn` 的 +文坊/+画坊 59px 折两行（+原料台 69px 正常，反证是宽度阈值）。根因：`.hud`/`.canvas-toolbar` 无 flex-wrap、`.hud__meta`/`.chip`/`.canvas-toolbar__btn` 无 nowrap/不防 shrink。**另发现独立预存 bug**：`.inspector-slot--hidden` 用 `translateX(100%)` 滑出，但定位祖先 `.stage` 是 `overflow:visible`，折叠面板仍贡献 420px `scrollWidth`（移除本次 CSS 后溢出依旧，证明与竖折无关）。修复 commit **`a8e4a89`**（分支 `feature/hud-narrow-layout`，纯 CSS 10 行 `apps/web/src/styles.css`，无文案、不碰颜色/间距 token）：`.hud`/`.hud__actions`/`.canvas-toolbar` 加 `flex-wrap:wrap`；`.hud__meta` `flex:0 0 auto`+`nowrap`；`.hud__actions .chip` 与 `.canvas-toolbar__btn` `white-space:nowrap`；`.stage` 加 `overflow:hidden`。真机注入验证：竖折清零、横向溢出 420→0、Inspector 展开 420px 完整贴右缘不裁/开合正常；typecheck 4 workspace 全过。2d/3d/park 三视图共享该 CSS 同步受益。**已由另一会话 cherry-pick（CSS `7cce04b` / handoff `dd4107a`）随 PR #319（merge `4163efb`）合 dev 并部署 Hasee**；2026-09-17 复核：health 探针 commit=`4163efb` db=ok，curl 拉取线上实际下发的 CSS bundle（`/assets/index-BnQd0IDn.css`）逐条确认 7 条规则（.hud / .hud__meta / .hud__actions / .hud__actions .chip / .canvas-toolbar / .canvas-toolbar__btn 的 wrap·nowrap + .stage overflow:hidden）全部在。**唯一未做项**：部署后像素级目视——当时内置浏览器到内网 `ERR_ADDRESS_UNREACHABLE`（同机命令行 curl/ssh/ping 全通，判为浏览器代理/网络栈问题、非服务故障），视觉效果以 2026-09-16 在同一 Hasee 页面注入完整 CSS 的实测截图为准；2026-09-18 回 Mac 后已在登录态画布页真机复验：视口 929px 实测 documentElement 横向 overflow=0、全叶文本扫描零竖折元素（无 width<68 且 height>38 的竖折文本）、截图目视 HUD 顶栏统计（写草稿/文坊·原料台·质检站·成品库·5条管道）与第二行按钮（收起侧栏/提示/待审核/成品库/术语表/快捷键/菜单/公告/账户）及 canvas-toolbar（2D/原料台/文坊/质检站/画坊/API口岸/成品库/更多）均横排正常。本 bu 环境 CDP 设备模拟报 session 错、未能精确压到 863px，但 929 已接近 bug 现场宽度，flex-wrap:wrap 语义保证更窄只会折成两行、不会文字竖排。#54 视觉复验完成，正式闭环。

> ✅ **已闭环（2026-09-16）**：demo「零配置首跑 422」经**两轮修复**并真机复验通过。首轮 PR #305（merge `5812aca`）部署后复验 422 仍在，二次定位到「login 屏未登录时 `getSettings()` 401 被 catch、`finally` 仍误置 `modelOptionsReady=true` 且选项为空」，二次修复 **PR #307（merge `76dbd42`，commit `27543bd`）** 改为仅 settings 成功才置 ready + 新增 401 竞态回归测（`graph.migrate` 7→8）。真机（Hasee `76dbd42`）：全新 demo 不碰模型分配直接派发，run 被接受执行、不再 422，DB textGen model 保留 `agnes-2.0-flash`；run 仅因 agnes free tier 429 在「润色」failed（外部配额，待升级付费 key）。Hasee 已挂 agentworld 用户每小时第 17 分 `prune-demo` cron（dry-run/APPLY 均验证）。根因/改法/测试见 #53 末「🔧 2026-09-16 修复闭环」与 docs/design-demo-user.md §十六。

> 🔵 **2026-09-16 续：claim 转正真机全链路通过 + RTS 园区 polish 走查（任务 3 完成 / 任务 2 一处修复在途）**：
> - **任务 3（demo claim 原地转正）真机完整走通**（此前 #53 仅到弹窗、未真注册）：demo 会话点「注册并保留我的工作」→ ClaimDialog 填邮箱+两次密码→「转正并注册」，转正后前端+DB 双验：**同 userId 不变**、`is_demo=0`、`demo_expires_at=NULL`、`/me` demo=null、email 更新、DemoBanner/转正按钮消失、graph `6807403a` 与 failed run 历史原样保留；退出后用新邮箱+密码可重新登录为正式账号、产线仍在。**副作用待用户拍板**：生产 Hasee 库因此多了一个正式测试账号 `claim-test-20260916@example.com`（userId `a29aed33-7b14-4282-8e22-9ae999457c6d`，密码用户设 `ffffff`，is_demo=0，**prune cron 不会清理**）；无现成"删正式账号"脚本，是否删除/如何删需用户决定。
> - **任务 2（RTS L0 园区 polish）真机走查结论：旧 polish 项均已完成且真机正常，无需再改**——状态色（failed 厂红色方块，正确反映 429 failed run）、标签深色 pill+青字可读性（#285）、跨厂厂顶抛物线拱+提亮+ROI（#296）、raycast 点选浮层+白色选中环（本次用页面内 NDC 网格扫描命中，浮层正确显示「写草稿 / 进入 / 重试」，failed 才显重试、无 cron 不显暂停、无待审不显审批卡，条件全对）。**注意：之前"点不中工厂"是浏览器自动化 `bu.click_xy` 坐标映射偏差（park canvas `rect.left=-131`），非产品缺陷**；真实鼠标命中正常（与 #50 C2 走查记录一致）。
> - **唯一新修（✅ 已闭环 2026-09-16，PR #310 merge `b957832` 已部署 Hasee）**：从"选中过节点的 3D 视图"进入/返回 L0 园区时，右侧 L1「节点详情」Inspector 仍残留展开、空态挤占园区画布（新用户进 park 默认折叠，属边缘 UX）。修复 commit `d2cf6d3`（`App.tsx` 的 `enterPark`/`backToPark` 加 `setInspectorCollapsed(true)`，回 2d/3d 仍由现有 selectedId/inspectorOpen effect 随节点选中开合；纯前端无新文案）。PR #310 四项 CI 全绿（Typecheck/build/test 2m17s / CodeQL / Analyze / Secret scan），merge 后 self-hosted runner 自动 deploy，Hasee 健康探针 `commit=b957832 db=ok`。
...
53. ★ **Demo User Hasee 真机走查 + 零配置首跑 422 修复闭环（走查 2026-09-15 / 修复 2026-09-16，PR #302 merge `b34fa29` 部署后）**：原 systemd override 只有 MONETIZATION_ENFORCE=1、漏 ALLOW_DEMO，补独立 drop-in `/etc/systemd/system/agent-world.service.d/demo.conf`（仅一行 `Environment=ALLOW_DEMO=1`，不动原 override；回滚=删文件 + daemon-reload + restart），重启 PID 36613，进程 environ 确认两变量同时在。**总结论：核心链路真机通过；走查发现 1 个必修 bug + 1 个部署待办，两者均已于 2026-09-16 闭环（见本节末「🔧 2026-09-16 修复闭环」）。**
   ...
   - ✅ DemoBanner（"演示模式 含 15 次运行 将于 23:30 结束"）、ClaimDialog 转正弹窗（邮箱 + 两次密码 + 取消/转正并注册）、退出演示（回 /login、会话清除）均正常；**claim 原地转正已于 2026-09-16 真机完整走通（同 userId、is_demo=0、graph/run 全保留、新密码可重登），见 Active work「2026-09-16 续」**。
   ...
   - 未逐项真机触发（已有单测 + 集成测兜底）：concurrentRuns=1 / maxRunsTotal 15 上限 / 20MB 上传上限（claim 转正已 2026-09-16 真机走通，见 Active work）。前端 `/api/runs?graphId=`、`/api/runs/:id` 查不到刚建 run（返回空/404），**run 状态一律以服务器 DB（python3 只读，无 sqlite3 CLI）为权威**。
   ...
   - 🔧 **2026-09-16 修复闭环（零配置首跑 422）——根因是前端竞态，改 `apps/web/src/store/graph.ts`**：
     ...
     - **状态（2026-09-16 已全链路闭环并真机复验）**：首轮修复随 **PR #305（merge `5812aca`）** 部署 Hasee，但真机复验 422 **仍复现**——取证发现 DB 里 textGen `model` 被 PUT 写空成 `""`（node 结构是 `node.textGen.model`，非 `config.model`）。**二次根因（比首轮更深）**：`refreshDefaultModel()` 在模块加载时（页面还在 `/login`、**未登录**）就跑，`getSettings()` 返回 401 被 `catch` 吞掉，**原 `finally` 仍把 `modelOptionsReady=true`**、而 `cachedModelOptions` 还是空 `[]`；进 demo 后不再重刷，`setGraph` 用空选项把内置 `agnes-2.0-flash` 判未知→清空→自动保存→422。**二次修复（PR #307 merge `76dbd42`）**：改为**仅 settings 成功才置 `modelOptionsReady=true`**，401/失败保持 false，`remapNodeModel` 从而保留非空 model；`graph.migrate.test.ts` 7→8（新增 login 屏 401 竞态回归：rejected fetch 后非空 model 不被清）。**真机复验（Hasee `76dbd42`）**：全新 demo 不碰模型分配直接派发 → run `31a41dcc` 被接受并执行（按钮变「停机」，过了 `validateModels`，**不再 422**），DB graph `6807403a` 两个 textGen model 均保留 `agnes-2.0-flash`。run 最终 failed 在「润色」节点，原因是 `[RATE_LIMIT] HTTP 429 free users`——agnes free tier 配额（与②翻译产线同一外部根因，待升级付费 key），非本 bug。**#53 零配置首跑 422 正式闭环。**
...
- **2026-09-16（只读体检，✅ 成本归集两 bug 修复均已验证，②翻译仍受 free tier 429 压制）**：①四产线 09-14 后 run 分布——①写草稿 done 123/failed 4/interrupted 4（96.8%）、②翻译 done 12/failed 13/halted 3/interrupted 1（**41.4%**，比 09-14 的 41.7% 基本持平，降频+放宽 QC 未明显拉升，根因仍是 free tier 配额耗尽，等付费 key）、③短视频 done 15/failed 3（83.3%）、④批量 done 13/failed 3（81.3%）。②**halted_reason 落盘验证 ✅**：PR #272 部署后新产生的 failed run 正确记录了 halted_reason（含节点名+错误摘要，如「[RATE_LIMIT] 润色: HTTP 429...」），早期历史 run 为 NULL 属正常（无法回填）。③**translate 节点 model=NULL 修复验证 ✅**：09-14 后无新增 model=NULL 且有 cost 的记录；存量 11 条全部在 09-12~09-14（Bug 2 修复前），历史无法回填。④**gate/qc 节点成本**：09-14 后 done 节点均有非零 cost/model（Bug 1 修复生效）。⑤全库 total runs=211 / done=174。⑥**users 表残留**：除已知 claim-test `a29aed33`，还有更早的 research-loop-test `734c4a28` 和 rltest2 `9a5a3f06`（均 is_demo=0，待用户决定是否清理）；4 个 demo 账号最早两个今晚 23:30/23:49 CST 才过期，prune cron 会自动清理。
- **2026-09-17（02:52 UTC 每日体检，✅ 整体健康，今日无新失败，③今日 03:00 UTC 尚未到点）**：SSH 正常（服务器 UTC 02:52）。**累计 run 分布**：①写草稿 done 140/failed 4/interrupted 6（≈97%，每小时第 10 分 tick 稳定）；②翻译 done 15/failed 13/halted 3/interrupted 1（≈54%，根因仍 free tier RATE_LIMIT）；③短视频 done 16/failed 3（84%）；④批量 done 13/failed 3（81%）。**今日（09-17 UTC）**：①3/3 done（00:10/01:10/02:10）✅、②1/1 done（00:00）✅、③0 条——**03:00 UTC 触发距体检仅 8 分钟，尚未到点，非漏触发**；④0 条——cron 每 2 天 06:00，下次 09-18，正常。**成本归集**：四产线 node 成本合计 **$8.9805**，③短视频 $8.6444（**96.3%**，视频生成大头，与 M1 三问结论一致）；①$0.2735（737 node_runs）、②$0.0405、④$0.0220。**零成本 node_runs 行**（①353/②86/③38/④72）多为原料台/映射/HTTP 等非 LLM 节点本就无成本，非 run 级 0 计费 bug。**失败原因归类**（节点级 error_code 累计）：RATE_LIMIT 18、VALIDATION 3、TIMEOUT 3、UNKNOWN 2、PROVIDER_ERROR 1；RATE_LIMIT 集中在②翻译（10，free tier 配额）与③/④手动补数据期（历史），今日 cron 自动触发无新 RATE_LIMIT。**新观察**：VALIDATION 3 次为节点级失败但所在 run 最终 done（被 error 边/catch 容错），非 run 级失败，暂不紧急。**处置建议**：cron 调度健康、今日全绿、成本归集完整；②翻译完成率低仍等付费 agnes key（free tier 配额硬约束，代码侧已尽力降频+放宽 QC）；M1 数据继续无人值守攒集中，**无需人工干预**。
- **2026-09-17（06:47 UTC ②翻译降频优化窗口专项复核，✅ 配额问题已压住、但暴露独立的"未翻译"质量问题）**：服务器在线（health commit `4163efb` db=ok）。②累计 done 15 / failed 13 / halted 3 / interrupted 1（46.9%，与 02:52 体检一致——02:52 后无新 run，cron `0 */8` 下次 08:00 UTC）。**按 09-14 15:00 UTC 优化落地切窗**：窗口内 8 run = done 5 / failed 1 / halted 1 / interrupted 1（**完成率 62.5%**，对比优化前累计 ~41% 明显回升），且 **09-16 00:00 / 08:00 / 16:00 + 09-17 00:00 连续 4 个 cron run 全 done**；窗口内 node 级 RATE_LIMIT 仅 1 次（09-14 16:00，优化刚部署初期）——降频（cron 4h→8h）+ 放宽 QC + RATE_LIMIT 5min retry 对 free-tier 配额问题确实有效。**新发现（非配额，建议排查）**：两条 halted（09-14 04:00、09-15 16:00）QC 判定均为「输出不是译文，模型反而回复"请提供初译稿 / 用户未提供初译文本"，未翻译任何原文」——cron 无人值守时翻译节点疑似没拿到待译原文（原料注入 / intake·brief 输入接线或 prompt 在空输入下回退成对话），与 429 无关。**下一步建议**：查②翻译产线 intake/brief 节点的原料注入与空输入兜底；观察窗口到 09-19，继续攒数据。

- **2026-09-18（Windows 设备，⛔ 体检未完成·授权阻塞）**：换 Windows 设备后本机 SSH 公钥未被 Hasee 授权（agentworld/jiangfeng 均 Permission denied）、内置浏览器 /login 需登录、/metrics 仅进程级计数，**未取到分产线数据，不编造结论**。09-14 降频+放宽 QC+retry 的效果暂以 09-17 06:47 专项复核为准（切窗后完成率 62.5%、连续 4 个 cron run done、窗口内 RATE_LIMIT 压到 1 次，但暴露独立的“未翻译”质量问题——QC 判模型回复“请提供初译稿”未真翻译，待查 intake/brief 原料注入与空输入兜底）。取数三通道见 Active work #56①，授权后补 09-19 窗口结论。
- **2026-09-18（07:52 UTC Mac SSH 补做，✅ 四产线全绿、降频效果巩固）**：Windows 授权阻塞解除后由 Mac `hasee-2016-server` 免密直连只读取数（health commit=`440ede9` db=ok，与 origin/dev 一致；全库 270 runs = 231 done / 29 failed / 3 halted / 7 interrupted）。**降频窗口（09-14 15:00 UTC 起）完成率**：①写草稿 86 done / 3 interrupted = **97%**（3 interrupted 为部署重启）、②翻译 8 done / 1 failed / 1 halted / 1 interrupted = **73%**、③短视频 8/8 = **100%**、④批量 4/4 = **100%**。**累计完成率**：①169/179=94%、②18/35=51%、③19/22=86%、④15/18=83%。**近 25h（09-17 06:47 后）四产线共 31 run 全 done**（①25 / ②3 / ③2 / ④1）。**②翻译关键**：窗口内唯一 RATE_LIMIT 在 09-14 16:00（优化刚落地初期），09-15 16:00 halted「未翻译、反问要初译稿」为该质量问题最后一次出现，**自 09-16 00:00 起连续 7 个 cron run（09-16 00/08/16、09-17 00/08/16、09-18 00）全 done、未翻译问题未复现**——间歇性根因（intake/brief 原料注入或空输入兜底）仍未修，但被 8h 降频节奏压住，继续观察不阻塞；窗口 node error 仅 RATE_LIMIT 1 + VALIDATION 1（即该 halted 的 QC 判定）。**成本**：四产线 node 成本合计约 **$10.67**——③短视频 $10.27（96%，视频 $0.5/次绝对大头）、①$0.33、②$0.05、④$0.03，与 M1 三问结论一致、归集健康。cron 调度健康（①每小时 / ②每 8h / ③每日 2 次 / ④每 2 日），**无需人工干预，M1 继续无人值守攒数**；根本解仍是升级 agnes 付费 key（free tier 配额硬约束，代码侧降频+放宽 QC+retry 已尽力）。
