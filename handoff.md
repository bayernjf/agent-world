...
1. **★ Phase 3 开启：通知节点（本轮完成）+ Phase 3 复盘标注**— **`notify` 节点落地**（core + server + web 全链路）：core 新增 NodeKind.notify + NotifyConfig schema（provider: feishu|dingtalk|wecom|email / message / webhookUrl / secret / to / subject）；server 新增 `notifier.ts` provider 抽象——三群机器人（飞书 text 卡片、钉钉 text + 可选 HMAC-SHA256 加签、企微 text），webhook URL 存节点配置（一图多群）；邮件走 SMTP env（SMTP_HOST/USER/PASS/PORT/FROM，密钥不入图），收件人/主题在节点；message 空时回退上游 text 产物（「搜索 → 总结 → 通知」最后一公里）；输出 json 产物（{sent,provider,detail,chars}）可审计；无消息/缺 webhook/缺收件人 → VALIDATION，env/SMTP 鉴权失败 → AUTH，平台 errcode 非 0 → PROVIDER_ERROR；web 工具栏按钮 + Inspector 面板（渠道/消息/webhook/加签/收件人/主题条件显示）+ 标签配色。**复盘发现**：roadmap Phase 3 的「定时触发增强」与「Webhook 输出」早已落地（cron/webhook/event/batch 四类触发器 + TriggerScheduler + Web 配置 UI；http 节点可调任意 webhook 兜底）——roadmap 表格已加状态列标注，无需重做。core 5 + server 8 个新测试。**下一步 Phase 3 候选**：GitHub/GitLab（提 PR/查 issue，REST + token）、Slack Bot、邮件收件/附件、内容平台 API（资质门槛高缓做）。
...
- `pnpm -r typecheck`：全绿
- `pnpm --filter @agent-world/core test`：136/136 通过（含 notify format/retry schema / search / convert / ocr / translate / fileParse + HTTP file 模式用例）
- `pnpm --filter @agent-world/server test`：365/365 通过（含 notify：飞书 interactive 卡片、钉钉/企微原生 markdown、网络抖动重试成功、Auth/Provider 不重试；search 与 notify.test.ts halt 通知全部用例）
