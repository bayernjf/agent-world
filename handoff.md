...
* [docs/mvp-readiness-review-2026-09-25.md](docs/mvp-readiness-review-2026-09-25.md) — 项目级 MVP 上线就绪评审（**最新版，含 §9 追评 2026-09-26 与 §9.6 改判，判定以 §9.5 + §9.6 为准**：§3 的四条一手复验阻断中三条代码项已闭合——sink 收窄为「结构性空→done 不归档 / 内容型空→failed」、A/B 补 `validateModels`、MCP 走 SSRF 闸（副作用：指内网/本机的远程 MCP 需 `ALLOW_PRIVATE_NETWORK=1`）；第四条口令 09-26 **改判为「已接受风险」**（不轮换、不改写历史，理由与失效条件见 §9.6）。**修订判定：自托管 ✅ 达到「产品核心完全可用的 MVP」，只剩一个运维签字：只跑单实例 + 有人 SSH 确认线上跑当前 main（实测 `origin/main` = `9c5d764`；报告正文里的 `eff8ebe` 是当日值，已过期）；对外 SaaS ❌ 仍不达标**；另含 5 条判据更正（G4 异步 seam 生产仍不可达 / 对象存储其实已实现 / CI runner 非 self-hosted / main 抖动口径）、质量门实测（core 346 · server 1397 · web 1968 · 零配置模板 35/36 可派）、待拍板 D-1 音频槽、**§8 记录了被证伪并弃用的扫描来源**）★ + [09-22 版](docs/mvp-readiness-review-2026-09-22.md)（历史基线：当时判自托管 MVP ✅ 达到；其「对象存储属阶段 5」「G4 已全部落地」两条已被更正）+ [09-21 版](docs/mvp-readiness-review-2026-09-21.md)
...
* [docs/design-model-catalog.md](docs/design-model-catalog.md) — 内置模型目录与插拔设计（**2026-09-25 定稿；①②③ 于 09-25、④ 于 09-26 全部落地（`9c7c5f1`/`54acc50`/`7c0dbe0` + `4c4e699`/`8266403`/`af9c6ed`）**；核心决策「改模型清单=数据、改接口方言=代码、改端点与凭证=env」；规则 A 下架即报错（已清三处静默路径）+ 规则 B `model:""`=跟随当前默认（在 startRun 解析，保 contentHash 与评测指纹）；④ 让内置目录的**模型名/模态/单价/启停**成为 admin 可改的数据：平台目录行（`__platform__`，零迁移、继承静态加密）+ 字段白名单合并 + `GET/PUT /api/admin/model-catalog` + `model.catalog_update` 审计（只记模型名不记价格）+ 写入响应直接列出「下架会打断哪些产线」+ Settings 维护界面；§十已结案，并订正一个错前提——**仓内不存在「按套餐限制具体模型」的机制**（`PlanQuota` 只有 tokens/concurrentRuns/storageBytes/videoSegments/seats），数据面能表达的因此不是「谁能看见」；含已修复的 worker 缓存导致改价不生效缺陷、剩余写死模型名清单与 A/B 口薄接线登记见其 §十一）
...
**#77 ★ AB 跨切面收口 + 兜底模型名清零（2026-09-26，feature/20260824，**已 push**——6 原子 commit `f94580f`→`91c8913`（drainRun 抽离 + 兜底清零）→ `06efaa7`（handoff 对账）→ `8ca53679`（pushed 标记）→ `b5b84be5`（#44 启动自检）→ `2557da25`（handoff #44 收口）；`origin/feature/20260824` @ `2557da25`）**：两条都出自 [design-model-catalog.md §十一](docs/design-model-catalog.md) 的「落地后剩余」清单。① **AB 实验 run 的跨切面缺口**（`drainRun` 抽出）——`ab.ts` 原先自己调 `execute`，只传了 `defaultModel`/`budget`，于是**合规词表、用户技能、搜索配置、媒体落库、月度预算、G4 远程任务、子流程、用量归集、告警、指标全缺**，而 run 照样报 done。修法不是给 ab 补一遍参数，而是把 `run.ts` 里 startRun/resumeRun/forkRun 三份近乎逐字重复（各 ~85 行）的「装配跨切面 → 跑引擎 → 落事件产物 → 收尾」抽成 `drainRun()`，**四条派发路径共用**，ab 只多一个刻意差异：`persistVariables:false`（N 条 arm 并发写同一份跨 run 图变量会互相覆盖，还会把实验状态写进产线的正式状态）。顺带让 AB run 也进 `runsActive` 与终态指标（此前不计）。守护两条：功能测钉「实验 run 也要进用量台账」（修前是 0，等于实验不占额度）；源码扫描守「从 `engine.js` 引入 execute/resume/fork 的文件必须引用 `drainRun`」（第一版用 `/\b(execute|resume|fork)\s*\(/` 误伤 `isolation.ts`/`mcp.ts`/`skills/registry.ts` 的同名方法，改为只解析 engine.js 的 import 语句；植入 rogue 文件验证过能红）。② **兜底模型名清零**——`engine.ts:1653/2119/2346` 的 `?? "agnes-2.0-flash"` 改为 `?? ""`（**兜底不再是任何模型名**：四个派发口都显式传 `defaultModel`，为空只可能是直接调引擎且没给默认，此时按规则 A 报 `UNSUPPORTED` 而非悄悄用写死的内置名——④ 之后管理员下架/改默认时那条老路会「跑成功但用错模型」）；`openai-compatible.ts` 三处（`runTextGen`/`judge`/`summarize`）改为 `requireModel()`，缺名字就抛 `UNSUPPORTED` 并点名是哪一处。**两处核实推翻了文档原文**：文档说「要清就得同时核约 20 个测试替身的期望值」——实测把兜底清空后 server 全量零测试依赖它（1425 过，只有 2 条既有 python-shim 环境红）；`judge` 那处看着像活路径（gate 节点没有 `textGen`），但 `providers/index.ts:153` 的 routingWorker 已用 `cfg.defaultModel` 填满再下发，所以直达 worker 时才可能为空。新增 3 例 provider 测（三条路径各自「空模型 → UNSUPPORTED 且不发请求」）。**质量门**：server 1429 → **1432**（1428 passed + 2 skipped + 2 既有环境红）、四包 `pnpm -r typecheck` 绿。有意保留的写死模型名：`templates.ts:1424` 的 `tts-1` 与 `tpl-custom-model`（BYOK 入口，非内置层）、`seed.ts`/`demo.worker.ts`/测试夹具。

### 活跃任务

**#76 ★ MVP 收尾批（2026-09-25，按 [mvp-readiness-review-2026-09-25.md](docs/mvp-readiness-review-2026-09-25.md) §7 最短路径，feature/20260824，**已 push**——6 原子 commit `68ecd99`→`4004057`，origin/feature/20260824 @ `4004057`；**2026-09-26 对账：这批连同其后的 `2c8bc22`/`66d337f`/`34b1aed` 都已合 dev 并到 `origin/main` = `eff8ebe`（`git merge-base --is-ancestor` 实测），写于当时的「未合 dev」已作废**）**：四阻断修复 + 五条口径更正 + 质量门实测。① **sink 空输入守卫**（`nodes/sink.ts`）——原空输入照样产「成品」，加 VALIDATION 守卫（空/纯空白 → failed + `VALIDATION`，不发 artifact/packets），新建 `sink.test.ts` 3 测。② **A/B 口补 validateModels**（`ab.ts`）——AB 不经 startRun 绕过模型可派发校验（媒体节点配错软跳过、run 假性 done），在 `buildABVariants(resolveModelSlots(...))` 后加 `validateModels(built[0].graph, cfg)` 过滤 error，非空抛 `RunStartError(422, modelErrors)`；ab/api.ab 测试夹具模型名 fake `"t"` → 内置 `agnes-2.0-flash`（通过校验）。③ **远程 MCP 裸 fetch → guardedFetch**（`mcp.ts` 4 处 @:237/:306/:361/:378，绕过自家 SSRF 闸）——全换 guardedFetch（redirect manual+最多 5 跳+跨 host 剥敏感 header）；mcp.test/api.mcp-user/mcp-pool 测试文件级 `ALLOW_PRIVATE_NETWORK=1` 逃生口（生产默认仍拒内网），mcp 系 58/58 绿。④ **production-ops 明文 sudo 口令**——commit 7b003f6 已 `<pw>` 占位，口令轮换归用户（Hasee sudo）**→ 09-26 用户决定不轮换，转为已接受风险（评审 §9.6）**。⑤ **.env.example 补 4 组变量**——`NODE_ENV`/`ALLOW_DEMO`/`STRIPE_SECRET_KEY+WEBHOOK_SECRET+PRICE_IDS`/`ERROR_REPORT_WEBHOOK_URL`；`ALLOW_REGISTRATION=1` 注释化（默认关、仅 bootstrap）。⑥ **§5 五条口径更正（文档）**——handoff「未 push」→已合 dev（git merge-base 核实）、「CI self-hosted runner」→GitHub 托管 ubuntu-latest（ci.yml:12,81）；design-step §3.4 与 deferred G4 行补「跨 run 续跑在生产不可达」（生产 routingWorker 无 `submitVideoJob`/`queryVideoJob`，仅测试替身）；production-ops M1 表补「仓内不可复现」警告；09-21/09-22 评审文档加更正注记。质量门：server 1390→**1393**（sink +3）、合计 3775→**3778**；四包 typecheck 绿。**结论：自托管 MVP 🟡 → ✅ 可签「完全可用+可上线」**（SaaS 侧门槛仍为收款主体/HTTPS/provider 真灾备/Postgres HA/Sentry，见 deferred）。
...
**最新体检（2026-09-26 02:41 UTC / 10:41 CST，触发环境 MacBook→SSH hasee-2016-server 直查 DB+journalctl）**：🚨 **需要人工干预——内置 agnes 模型 9 月免费额度（2,000,000 折算 token）已于 09-25 04:10 UTC 耗尽，M1 回采全线停摆（09-25 04:10 → 09-26 02:41 无任何新 run，09-26 全天 0 run）**。证据链：`cron tick fired` 自 09-25 12:04（PID 118721 active）起照常触发（①`c834cccd…` `10 * * * *`、②`trg_m1_trans_daily` `0 */8 * * *`、③`trg_m1_video_daily` `0 3,15 * * *`），但每个 tick 紧跟 `level:error trigger scheduler: 本月内置模型额度已用尽（2,000,000 折算 token），请加购或升级`——**月度预算熔断（startRun 硬熔断，09-21 dispatch-gate 批上线）在调度器层拦截建 run**，非 cron 故障、非触发器丢失、非产线被删。服务 09-25 当天被 SIGTERM 重启 3 次（04:15/05:39/07:37，12:04 起 PID 118721；07:37→12:04 空窗 4.5h 疑部署/维护），04:10 run `88faa9cb` 因 04:15 SIGTERM（inflightRuns:1）置 interrupted（非额度所致）。**四产线当日+累计**：①344=309 done/23 failed/12 interrupted，当日 2（1 done+1 interrupted）②56=34/15 failed/5 halted/2 interrupted，当日 0 ③36=31/5，当日 1 done（09-25 03:00，为 09-25 最后成功 run）④24=20/4，当日 0；完成率 ①89.8%（去中断 92.9%）②60.7% ③86.1% ④83.3%。**失败归类（非限流主导）**：①18× PROVIDER_ERROR fetch failed（初稿）+1× SSRF apihub.agnes-ai.com（历史）+1× RATE_LIMIT+3× NOCODE；②2× PROVIDER_ERROR 级联 STRANDED+多例 halted（QC 判「无译文/缺原文对照」）+1× RATE_LIMIT；③2× PROVIDER_ERROR+3× NOCODE；④1× PROVIDER_ERROR+3× NOCODE——**最新 run 无 429 打爆，429 残留全期仅 2 例**。**成本对账**：四产线累计 **$17.47**（①$0.62/②$0.07/③$16.75/④$0.03），③短视频占 95.9%（video $15.5+image $1.24+text $0.009）；全库 node_runs $18.02 与 09-24 $16.89 增量一致、四线合计差 $0.55 属其他产线；done 节点 0 成本项（①705/1611 等）均为 gate/确定性非模型节点，**无 0 计费碎片、无异常**。**处置建议（需用户拍板，Agent 无法代执行）**：①升级 agnes Token Plan/换付费 key（消除额度+429，M1 立即恢复）②等 10-01 月度额度重置（数据断档约 5 天，09 月回采提前收官）③降频无意义（额度已尽，不随降频恢复）。**结论：完成率/成本归集/429 三项通过，唯额度熔断为硬阻断，人工干预前 M1 回采无法继续。**

每日回采体检由豆包定时任务每日 10:30（CST）触发。
...
**2026-09-26（AB 跨切面收口 + 兜底模型名清零，feature/20260824，未 push）**：`run.ts` 抽出 `drainRun()`，startRun/resumeRun/forkRun/ab 四条派发路径共用一份跨切面装配（此前 ab 一条都没传、另三条各抄一遍）；`engine.ts` 三处兜底与 `openai-compatible.ts` 三处兜底不再写死 `agnes-2.0-flash`，改按规则 A 报 `UNSUPPORTED`。server 1429 → 1432，四包 typecheck 绿。详情见 Active work #77。

**2026-09-26（线上部署事实 SSH 闭合，docs）**：`ssh hasee-2016-server` 实测 `/api/health` = `ok:true / branch:dev / commit:93f6758`，部署目录 `/opt/agent-world` 同为 `dev @ 93f6758`，`ps` 单实例（PID 710）；`git ls-remote` 复核 `origin/dev`=93f6758 = 线上（无落后）、`origin/main`=`0d09cd5`（已过原记的 9c5d764）、`origin/feature/20260824`=`3e5354e`=本地 HEAD，`merge-base --is-ancestor 93f6758 0d09cd5` = YES。**口径更正**：待办里「确认 Hasee 真在跑当前 main」这句话本身是错的——Hasee 是 staging，按分支映射跟踪 dev 不是 main；换成等价命题「线上跑 dev 尖端 + dev 内容已全部进 main」已实测成立，**自托管 MVP 最后一个前置闭合**（残留一笔：main 上有个 09-09 的 dependabot bump `18e8c5b` 没回 dev，已记评审 §9.2）。

> **推送 / 部署对账（2026-09-26 用 `git ls-remote` 直查远端，不看本地 remote-tracking ref）**：`origin/main` = **`9c5d764`**、`origin/dev` = `1d894c8`、`origin/feature/20260824` = **`90289b4` = 本地 HEAD**——即 ①②③④、MVP 收尾批、sink 收窄、以及本会话的 docs 提交**全部已在公网那份仓库上**。**本节下面各条目里写于当时的「未 push / 未合 dev」一律作废，以本行为准**；此前我自己也报错过一次（拿未 fetch 的 `origin/*` 算出「领先 N 个 commit」，那是假的）。

> **2026-09-26 下午：远端三分支再刷新 + 部署事实已 SSH 闭合**（`git ls-remote` + `ssh hasee-2016-server`）：`origin/main` = **`0d09cd5`**（已过本行原记的 `9c5d764`）、`origin/dev` = **`93f6758`**、`origin/feature/20260824` = **`06efaa7` = 本地 HEAD（4 个本会话提交已于当日 push）**。Hasee `/api/health` = `ok:true, env:staging, branch:dev, commit:93f6758, db/jwt/encryption ok, providers.agnes configured`；部署目录 `/opt/agent-world` 亦为 `dev @ 93f6758`（工作区干净）；`ps` 显示 **单实例**（PID 710 / ppid 1）。`git merge-base --is-ancestor 93f6758 0d09cd5` = YES（main 多出的 102 提交里 101 个是 dev→main merge，唯一非 merge 是 09-09 的 dependabot bump `18e8c5b`）。

> **2026-09-26 稍晚（第二次 push 后复核）**：本会话后续提交已随用户 push 全部上远端——`origin/feature/20260824` = **`2557da25`** = 本地 HEAD（含 #77 的 5 个 commit + #44 配置收口的 2 个 commit）。`origin/dev`=`93f6758` 与 `origin/main`=`0d09cd5` 不变（这批 Feature 分支提交尚未合 dev/main，按分支映射是下一步）。Hasee 仍跑 `dev@93f6758`（staging 跟踪 dev，未自动吃 feature 分支），所以#77/#44 新代码要上 Hasee 需先合 dev 再部署。
> **口径更正**：原待办「确认 Hasee 真在跑当前 main」**这句话本身是错的、已作废**——Hasee 是 staging，按分支映射跟踪 **dev** 而非 main，health 报 `branch:"dev"` 是设计如此。换成等价命题「线上跑 dev 尖端 + dev 内容已全部进 main」，已实测成立。**自托管 MVP 最后一个前置闭合。** 残留一笔单向偏差（dev 少了 09-09 那次依赖 bump）已登记评审 §9.2。

> **2026-09-27 对账（git ls-remote 直查）**：远端三分支刷新——`origin/dev` = **`3e010d92`**（`Merge pull request #437 from bayernjf/feature/20260824`）、`origin/feature/20260824` = **`2557da25`**（#77/#44 链尾，已 push）、`origin/main` = **`62145d00`**（已过 09-26 记的 `0d09cd5`）。**更正 09-26 稍晚那条「这批 Feature 分支提交尚未合 dev/main」**：`git merge-base --is-ancestor 2557da25 origin/dev` = YES——#77（drainRun 跨切面 + 兜底清零）与 #44（failover 启动自检）已随 PR #437 合入 dev，进入 Hasee 部署轨道（Hasee 跟踪 dev；是否已自动拉到 `3e010d92` 需再 SSH health 复核，上次实测为 `dev@93f6758`）。**本地唯一未上远端的是本会话最后的文档同步 commit `d54c2a1d`（handoff push 状态 + production-ops §9.4 启动自检），`origin/feature/20260824` 仍停在 `2557da25`，待 push。** 另 `origin/main` 已前进到 `62145d00`，且 `git merge-base --is-ancestor 3e010d92 origin/main` = NO——dev 随 PR #437 等已前进，但 main 尚未合入这些新 dev 提交，故「dev 内容已全部进 main」在 09-27 快照下暂不成立（旧快照 `93f6758 -> 0d09cd5` 当时为 YES）。

**2026-09-26（内置模型目录 ④ 数据面 + 自己开的药方被 CI 打回，feature/20260824，4 原子 commit `eb11cf5`/`4c4e699`/`8266403`/`af9c6ed`，**写于当时：未 push；09-26 稍后 `git ls-remote` 复核：已在远端**（`origin/feature/20260824` = `90289b4`，见上方对账行）**：① **先记一次自纠**：#56 的 sink 空输入守卫是我按「空输入＝坏图」写的，PR #429 的 CI 用 **10 条测 / 4 个文件**红着告诉我它错——由 branch/gate/video 直接喂的 sink **结构上就该收到空串**（分支节点只发元数据、gate 走人审、媒体节点出 artifact 不出正文）。临时探针把上游 kind 打出来定性（`["branch"]`×7 / `["gate"]`×3 / `["videoGen"]`×1）、本地复现后改成「结构 vs 断链」二分（`nodes/sink.ts` 的 `NON_TEXT_PRODUCER` + `incoming(...,"flow")` 全非文本生产者 → `done` 且**不归档**，否则仍 `failed`+`VALIDATION`），commit `34b1aed`（已随 #429 合 dev/main）。副作用自查过：`role` 在 `run.ts:327-328` 只是返回列不是过滤器 → 成品库少的是「打不开的空卡」，视频/图片行不受影响。② **④ 落地**：内置目录的模型名/模态/**单价**/启停变成 admin 可改的数据（方案见 [design-model-catalog.md](docs/design-model-catalog.md) §六/§八）。存储用 `settings` 的保留行 `__platform__`（`user_id` 是 TEXT PRIMARY KEY 且无外键 → **零迁移**，静态加密由 store 边界白拿）；合并走**字段白名单 + 替换语义**（`.strict()` 直接拒 `baseUrl`/`apiKey`/`type`/`endpoints`/`videoAdapter`/`source` 六类越权字段，替换而非追加才能表达「下架」）；坏行 fail-closed 保留上一份好目录。有两条是**落地时改的设计**：合并点从 `parseRaw` 挪到 `loadConfig`（干净部署上 `parseRaw` 根本不被调用，挂那儿等于什么都没做——这条是被自己写的集成测逼出来的），以及写入响应直接回答「下架会打断谁的哪些产线」（`affected`，上限 50 + `affectedTruncated`）。③ **另一个自纠**：`modelCatalogView` 第一版把托管层真实 `apiKey` 一起发给任意 admin 浏览器，是我自己那条「响应里不得出现 `sk-`」的断言在提交前拦住的，现改为形状视图 + `hasKey`。④ **评审 §9 追评**（`eb11cf5`）：自托管口径升 ✅，但附两个**只能人来做**的前置——只跑单实例、以及轮换那台机器的 sudo 口令 + **确认 Hasee 真在跑 `eff8ebe`**（针对该 commit 的 deploy 尝试结论是 `skipped`，本机 `curl /api/health` 无返回，线上版本我无法自证）。**同日改判**：「轮换口令」这一条不做了——用户明确决定不轮换，转为**已接受风险**（这台是家庭内网 staging 机，口令换到的是它的 root，换不到任何外部账号；实测泄露面 + 「改写 git 历史也拿不掉，因为远端有 432 条 `refs/pull/*/head`」都写在评审 §9.6）。剩下的前置只有「有人 SSH 确认线上跑的是当前 main」，且这里的 `eff8ebe` 已过期：`git ls-remote` 实测 `origin/main` = `9c5d764`。**质量门（本轮实跑）**：core 346/346、server **1427 = 1423 passed / 2 skipped / 2 本机 python-shim 环境红**（167 文件）、web **1973/1973**（106 文件）、`pnpm typecheck`（含 scripts）绿、`i18n:check` 与 `i18n:prune -- --check` 0 未引用；④ 增量 server +30（`builtin-catalog.test.ts` 22、`api.model-catalog.test.ts` 8）、web +5。⑤ **还记一笔不可信来源**：本轮一个 subagent 安全扫掠交回的 5 条「高危」逐条复核后**全部证伪**（不存在的 share/embed 端点、没有登出路由、把 `ADMIN_USER_IDS` 当缺失……），整份丢弃，此后凡靠扫掠结论下的判定都要自己重推一遍。⑥ **补做真实进程冒烟**（vitest 覆盖不到的那一层）：起 `packages/server/dist/index.js` + 一次性 sqlite，用 curl 走 HTTP 实测——GET 视图里 `apiKey`/`baseUrl` 出现 0 次；四类越权载荷（baseUrl / apiKey / type / source）全部 400 且不改动已存目录；写入后**杀掉进程再重启**目录仍在（证明 boot 时加载这条路径真的通）；下架 `agnes-2.0-flash` → 影响扫描点名产线与两个节点（`初稿`/`润色`）→ `POST /api/runs` 回 422 且 message 含「已不可用」、diagnostics 2 条；审计行只有模型名、**没有价格数值**（按价格值 grep 命中 0）；`PUT {}` 回到随包目录；匿名 401、普通用户 GET/PUT 均 403、`/api/auth/me` 的 `canManageModelCatalog` owner=true / user=false；把 `__platform__` 行写成非法 JSON 后重启只 warn `model catalog not loaded` 并回落随包目录（不崩、不半加载，普通读接口仍 200）。**这块 UI 至今没在浏览器里真点过**（只有 jsdom 组件测；本沙箱的 browser-use 点击被分类器拦），真供应商端到端也没跑过（本机 agnes key 是 free tier）——两条都记在未做，不当已验。
