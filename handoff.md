...
48. 🔵 **商业化 M3：收款与账单落地（2026-09-14 启动，方案 docs/design-monetization-m3-implementation.md，S1-S6）**：在 M2 订阅 gate 基础上加装账单层。**S1 进行中**：invoices 表（迁移 38）+ 账单生成服务（generateInvoice/listInvoices/getInvoice/markInvoicePaid/voidInvoice，状态机 draft→open→paid/void，金额从 PLAN_PRICES 取，line_items JSON）+ 账单生成 CLI。**S2**：Settings billing tab 加账单流水列表 + 详情弹窗。**S3**：HTML 发票模板 + 下载端点。**S4**：admin 手动标记账单 paid 闭环（audit_log + 公告 + 订阅状态确认）。**S5**：Team 套餐 seats 限制（PLANS 加 seats 字段，team=5/其他=1，Collaborators 面板增强，加人超限拦截）。**S6**：Stripe 支付网关（P2 低优，待有收款主体后实施）。MVP 核心是 S1-S4（手动收款 + 账单流水 + 发票），S5 团队席位随 Team 套餐上线，S6 待收款主体。设计文档已写（docs/design-monetization-m3-implementation.md，含数据模型/API/测试/部署/回滚）。
