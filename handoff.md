...
* [docs/design-tts-provider.md](docs/design-tts-provider.md) — TTS Provider 接入设计（audioGen 配音链路；推荐 SiliconFlow 原生 OpenAI 兼容 `/audio/speech`，含 UTF-8 字节计费/音色命名/软降级口径与 P0 零代码验证、P1 产品化、P2 edge-tts 三档；2026-09-22 仅方案未写码）
...
**2026-09-21（晚，docs 索引 + i18n/依赖纪律复核清账，feature/20260824）**：① docs 场景导航补齐 5 个缺失入口（commit `7b7fc97` `docs: fill missing scenario entries in docs index`，+5/-1）：缓做项触发条件 deferred-items、第三方集成 integrations-future、web 组件测试 web-component-testing-plan、Phase4 高级编排 phase4-design、CI/CD 流水线 runbooks/deploy-cicd。② **i18n Inspector 29 种节点配置字段经 `pnpm --filter @agent-world/web run i18n:check` 复核确认早已完整**：zh/en 插值变量全对齐、check EXIT=0（899 个未引用 key 是动态 `t()` 的预期 warning，脚本自述 warnings only），Inspector.tsx 零硬编码中文、zh/en nodes.json 的 inspector 段同构覆盖全部节点；历史“~250 处硬编码”估计已过时，本轮无代码改动、不造提交。③ **@types/node 24→26 经复核决定维持缓做、本轮不升级**：运行时/CI/Hasee/便携环境均为 Node 24，类型升 26 会领先运行时两个大版本（TS 可能放行运行时不存在的 API）；dependabot.yml ignore + deferred-items 第 98 行 + 下方第 6 条三处纪律一致，触发条件 = 运行时升 Node 26，届时唯一已知代码点 `packages/server/src/artifact-store.ts:202` 的 `new Blob([buf])` 需同步验证 BlobPart（PR #186），当前在 @types 24 下无法预演其对 26 的效果，不做无触发条件的预防性改动。④ 本机踩到一次假 typecheck 失败：pull 到含 core 改动的提交后 core/dist 过期，server tsc 读旧 .d.ts 误报 `engine.ts:989 Property 'root' does not exist`（源码 contract.ts 已有 G2.4 array-root），rebuild core 后四包 typecheck 全 Done——非代码 bug，CI 先 build core 故全绿，已补进 Windows 基线。
