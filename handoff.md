...
1. **★ Phase 4 错误处理 PR①：重试去重 + 补全（本轮完成）**— 按 `docs/phase4-design.md` §1 优先级开干。**抽公共 `retry.ts`**（`withRetry(fn, policy, isRetryable, sleep)` + 指数退避），**notify.ts/vcs.ts 删私有 withRetry 改用公共**（去重两份复制粘贴）；**translate 的 retry 从硬编码提到 TranslateConfig**（engine 读 `cfg.retry` 而非硬编码对象）；**SearchConfig/HttpNodeConfig/CodeNodeConfig 加 retry 字段** + 三节点执行分支加重试（search 包 searchWeb、http 包 fetch+5xx 重试 4xx/timeout 不重试、code 包 spawn 且仅 spawn-error 重试非零退出码/超时不重试）；web DEFAULTS 四处补 retry。search +1 重试用例（瞬态 ECONNRESET 第二次成功）。**下一步 PR②**：skip + error 边（NodeState 已有 skipped 但未用，级联 skip + EdgeKind 加 error + predecessorsReady 区分 catch 语义）。
...
- `pnpm -r typecheck`：全绿
- `pnpm --filter @agent-world/core test`：142/142 通过（含 http/code/search/translate retry schema / vcs / notify slack / search / convert / ocr / translate / fileParse + HTTP file 模式用例）
- `pnpm --filter @agent-world/server test`：375/375 通过（含 retry：notify/vcs/search/http/code 共享 withRetry、search 瞬态重试成功、translate retry 读 config；vcs/notify halt 全部用例）
