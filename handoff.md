...
> 🔧 **待办 #55（2026-09-16，竞品痛点应对·安全子集已提交、待 push 走 PR）：步级可观测（G1 只读链路）+ 上游契约/超时纯函数**。依据 `docs/design-step-trace-and-robustness.md`，只落地**不改 run 执行状态机**的安全项（绝不能影响 Hasee M1 回采），3 个原子 commit（feature/20260824，未 push）：
...
> - **明确留待（改 run 执行核心，须单独评估、防打断 M1，本轮不做）**：G1.2 从节点 fork 重跑 API；G2.2 engine 契约接线（需先定是否扩 ErrorCode enum 加 SCHEMA_VIOLATION）；G2.3 Inspector 契约表单；G2.4 给模板补 contract；G4.2 timeout+degraded 降级状态机；G4.3 前端"继续/降级"按钮；G4.4 视频远端进度轮询；G3 节点 maxRetries 下放 + 预算 UI。**待用户 push → PR → CI → merge → Hasee 部署后真机走查"运行历史→步骤"。**
