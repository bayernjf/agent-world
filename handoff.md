...
1. **★ Phase 3 推进：vcs 节点 + notify Slack provider + 未来集成接口草案（本轮完成）**— **`vcs` 节点落地**（core + server + web）：NodeKind.vcs + VcsConfig（provider github|gitlab / action: create_pr|comment_issue|trigger_workflow|list_issues / owner-repo 或 projectId / 标题正文分支等 / 复用 RetryPolicy）；server 新增 `vcs.ts`：GitHub REST（Bearer token + x-github-api-version）+ GitLab REST（PRIVATE-TOKEN，GITLAB_API_URL 可自托管），body/title 空时回退上游 text（agent 起草 PR 描述 → vcs 提 PR），产物 json（API 响应），VcsAuthError/VcsProviderError + withRetry 瞬态重试，错误码 AUTH/PROVIDER_ERROR。**notify 扩 Slack provider**（chat.postMessage Bearer token + channel，Slack ok:false → ProviderError，复用 markdown/retry）。web 工具栏「代码仓库」按钮 + vcs Inspector 面板（平台/动作/owner-repo 或 projectId/条件字段）+ notify 面板加 slack+channel + 两节点配色。core 7 + server 9（vcs 6 + notify slack 3）个新测试。**未来集成接口草案**（[docs/integrations-future.md](docs/integrations-future.md)）：Notion、Linear、邮件收件/附件（作触发器扩展）、内容平台（走连接器市场不建原生节点）、Bitbucket/Gitea（vcs 同构扩展），每项含 schema 草案 + env + API + 入手路径。Phase 3 已落地项：通知（4 群机器人+邮件+markdown+重试）、vcs、Slack、定时触发、Webhook 输出。
