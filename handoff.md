...
- `pnpm -r typecheck`：全绿
- `pnpm --filter @agent-world/core test`：134/134 通过（含 notify / search / convert / ocr / translate / fileParse schema + HTTP file 模式用例）
- `pnpm --filter @agent-world/server test`：360/360 通过（含 notify：飞书/钉钉(加签)/企微群机器人、SMTP 邮件、上游 text 回退、AUTH/PROVIDER_ERROR/VALIDATION；search 与 notify.test.ts halt 通知全部用例）
