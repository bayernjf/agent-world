...
32. ✅ **search 成功路径补证（2026-09-06 完成）**：用用户在「设置 · 搜索服务」配置的 Tavily key 真实跑通 search 节点（最小图 source→search(tavily)→sink，run `d80040c5`，返回 3 条真实结果 taobao/jd/supercoddle）。搜索凭证本轮重构为**按源独立绑定 + 节点 gating**（详见待办第 8 条）。**audioGen 成功路径仍推迟（用户无 TTS/音频模型）**——待有可用音频模型后再补 tpl-news-podcast 音频链路。
