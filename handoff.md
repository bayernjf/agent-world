...
- `pnpm -r typecheck`：全绿
- `pnpm --filter @agent-world/core test`：129/129 通过（含 search / convert / ocr / translate / fileParse schema + HTTP file 模式用例）
- `pnpm --filter @agent-world/server test`：352/352 通过（含 search：ddg 静态查询与 uddg 解码、上游 text 回退 query、tavily env key 链路、AUTH/PROVIDER_ERROR/VALIDATION、空结果；convert 与 ocr 全部用例）
