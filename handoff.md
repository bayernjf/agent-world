...
1. **★ Phase 4 错误处理 PR②：级联 skip（本轮完成）**— `NodeState` 的 `skipped` 原仅用于 branch 未路由分支，现扩展到**失败节点的搁浅下游**。engine `schedule` 的 `running===0` 分支加不动点级联：所有 flow 前驱 terminal 且有 failed/skipped 前驱且无 done 前驱的 pending 节点 → 标记 skipped + emit `node.skipped`（不动点循环处理级联，合并点有 done 前驱则保留可运行）；halted/cancelled run 不级联（守卫 `status!=="halted"&&!=="cancelled"`，留给 resume）。core 加 `node.skipped` RunEvent + runtime.ts reduce 处理 + NodeRuntime.status 加 skipped；web STATUS_LABEL 加「已跳过」。2 个新测试（级联 skip 失败下游、合并点有 done 前驱不 skip）。**下一步 PR③**：error 边（EdgeKind 加 error + catch 节点 + predecessorsReady 区分 catch 语义「任一 ready」）——让失败节点能走兜底路径而非搁浅；之后 PR④ 失败告警 + rerun API。
...
- `pnpm -r typecheck`：全绿
- `pnpm --filter @agent-world/core test`：142/142 通过（含 node.skipped event + NodeRuntime.status skipped / http/code/search/translate retry schema + HTTP file 模式用例）
- `pnpm --filter @agent-world/server test`：377/377 通过（含 skip：失败节点下游级联 skipped + 合并点有 done 前驱不 skip；retry：notify/vcs/search/http/code 共享 withRetry；vcs/notify halt 全部用例）
