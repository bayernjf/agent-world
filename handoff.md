...
| 阶段 | 内容 | 状态 |
|---|---|---|
| P0 | `users.role` + owner bootstrap + 公告改走 `admin` 角色（env 白名单退役）+ `/api/auth/me` 返回角色 | 未做 |
...
前身：公告管理曾用 `ANNOUNCEMENT_ADMIN_EMAILS` env 白名单（`2467055074@qq.com`），P0 落地即退役；公告本身功能已完成（见 commit `3e41c16`/`356b1c7`/`8ac324b`/`07fb540`）。deferred-items「多租户/权限」触发行已更新为「底层 RBAC 就位，团队协作本身仍待真实需求」。
