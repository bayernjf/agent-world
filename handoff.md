...
1. `2d3dfcf`+`95c65a4`+`5cfd5bd` — **feat(core,server,web)**: **source 节点终于能投文件（狗粮 tpl-contract-review 驱动）——`SourceConfig.files`（结构化 `{uri,label,mimeType,sizeBytes}`）+ source 派发时产 `kind="file"` 产物，Inspector 新增 PDF/DOCX/PPTX 上传区（不支持类型/超 5MB 点名拒绝，不再静默丢弃）；fileParse 读不到字节的报错补上 5MB 内联上限、多文档只解析第一个时摘要点名未解析件数。合同审查模板由 ❌ 转 ✅（run `084b6f63`，rework 边首次真实跑通 + human 审批闭环）；契约写入 technical-design / design-artifact-display，脱节的旧用例已补真实上传路径 4 例。
2. `fa2bed0` — **fix(server)**: 节点分支意外抛错不再吞成 unhandled rejection——runNode 外层加安全网发诚实 `node.failed`（此前节点永久停在 "running"，事件流既无 finished 也无 failed，只有 CI 上一条查不出原因的红）；code 节点子进程起不来同样落 failed(SUBPROCESS)。回归用例改为真实退避 + 失败原因可读。
3. `c91f973` — **fix(server)**: **生成媒体产物 404 修复（狗粮 tpl-product 驱动）**——imageGen/videoGen/audioGen 的 run 产物行只携带 `up-…` 本地引用（字节在引用行桶下），`GET /api/artifacts/:id` 在本行 blob 缺失时改为跟随引用取字节；此前 UI 上历史所有生图 run 均为破图（“blob missing on disk”）。新增 `api.artifact-localref.test.ts`（跟随成功 + 跨用户仍 404），契约写入 [design-artifact-display.md](docs/design-artifact-display.md)。
4. `7b7faf0`…`530bfc5` — **狗粮验证四条修复系列（tpl-news-podcast 驱动）**：media 节点 modality 错配派发期阻断（`7b7faf0`）；audioGen/videoGen 静默跳过改诚实失败 node.failed 接入 error 边（`b6de7d9`）；search 不可达可行动报错（`bfc97dc`）+ `AGENT_WORLD_PROXY` opt-in 出站代理（`b82f89a`，SSRF trade-off 见 design-code-sandbox §12）；DDG 反爬页响亮报错不再静默 0 结果（`530bfc5`）。
5. `docs:` **文档-代码覆盖盘点（2026-09-01）**——补齐 [design-knowledge-memory.md](docs/design-knowledge-memory.md)（知识提取/FTS5/archive_search）与 [design-ab-testing.md](docs/design-ab-testing.md)（A/B 实验，此前四处文档误标"缓做"，实已落地）；technical-design 加时效注记并补 §3.1b/§4.1b 增量（25 节点/表/API 现状对齐）；docs-README 索引、project-progress、deferred-items 同步；handoff 最近 5 条 hash 经 git log 核实已全部回填。
