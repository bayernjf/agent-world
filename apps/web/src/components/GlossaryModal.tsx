...
const GROUPS: GlossaryGroup[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      { std: "audioGen", game: "音坊", note: "文字 → 语音 / 音乐" },
      {
        std: "generic",
        game: "多能坊",
        note: "自由选型 provider，按模态自动 dispatch",
      },
    ...
    rows: [
      {
        std: "generation",
        game: "AI 加工",
        note: "文坊(textGen) / 画坊(imageGen) / 影坊(videoGen) / 音坊(audioGen) / 多能坊(generic)",
      },
      {
        std: "control",
        game: "车间调度",
        note: "质检站 / 分拣闸 / 改料台 / 批处理站 / 汇流站 / 外包工坊",
      },
      {
        std: "data",
        game: "物料处理",
        note: "理货台 / 总账房 / 拆包台 / 换装台 / 翻译间 / 识图台 / 代码工坊",
      },
      {
        std: "integrations",
        game: "外接设备",
        note: "API 口岸 / 瞭望塔 / 广播站 / 档案柜 / 人工岗",
      },
  ...
  },
  {
    ...
    rows: [
      ...
      {
        std: "inputPolicy: all/last/truncate/summary",
        game: "全量 / 仅最近 / 截断 / 摘要下料",
        note: "",
      },
      {
        std: "artifact type",
        game: "产物类型",
        note: "text / json / html / image / file / table",
      },
      { std: "跨运行产物画廊", game: "成品库", note: "历史成品集中地" },
      {
        std: "knowledge / memory",
        game: "知识库 / 档案室",
        note: "经验沉淀（可检索）",
      },
  ...
  },
  {
    ...
    rows: [
      ...
      { std: "template", game: "产线模板", note: "" },
      {
        std: "A/B test",
        game: "A/B 对照 / 手臂对比",
        note: "换 N 版 prompt 比质量",
      },
      {
        std: "graph version / snapshot",
        game: "产线版本 / 版本快照",
        note: "可回放复原",
      },
    ],
...
export default function GlossaryModal({ open, onClose }: Props) {
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div
        className="modal modal--wide glossary"
