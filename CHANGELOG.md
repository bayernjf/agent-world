...
### Added
- **用户反馈（P1+P2）** — `feedback` 表（迁移 33）+ `POST /api/feedback`（消息 ≤2000 字符 + 分类白名单 + 服务端上下文白名单二次脱敏 + 截图 base64 ≤1MB + 每用户滚动小时 10 条限流）+ owner/admin 管理端（列表 / 三态流转 / 附件端点，cookie 认证支持 `<img src>`）+ 前端 `FeedbackModal`（分类 + 粘贴截图 + 诊断信息勾选）+ UserMenu「反馈」入口 + AdminPanel 反馈 tab；提交与状态变更写审计日志。详见 [docs/design-feedback.md](docs/design-feedback.md)。
