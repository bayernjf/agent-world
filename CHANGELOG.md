...
- **G4 运营台在途任务只读端点 + degraded 决策 UI 测试补齐** — 新增 `GET /api/admin/remote-jobs`（owner/admin 可见，对齐 `/api/admin/errors`；默认只返回 submitted/running 在途作业、按最久未轮询在前排序以便发现卡住的渲染，`?all=1` 含终态、`?limit=` 封顶 500、非法值兜底）；driver 新增 `listRemoteJobs`，SQLite 与 PostgreSQL 经共享 `createDriver` 同构继承（方言差异不出 driver）。driver +3 测、HTTP 401/403/200/`?all`/limit 共 +5 测。同时为 G4 web degraded 时间线补齐此前缺位的组件测试（橙色降级卡片与远端任务句柄、reattach 直调、accept-degraded 二次确认、`REMOTE_JOB_LOST` 时仅显示「重新提交（将重新计费）」、run 非 halted 时隐藏决策条、已接受降级角标，+6 测）。详见 [docs/design-step-trace-and-robustness.md](docs/design-step-trace-and-robustness.md) §3.4 末行与 §3.9 步骤 7。
...
- **首个承载 fanout + select 的产线模板 `tpl-variant-copy`（文案多变体择优）** — 此前 29 类节点中有 6 类没有任何承载模板（文档只记了 2 类，本轮按 `TEMPLATES` 与 `NodeKind` 逐一比对核实），`fanout`/`select` 是其中最便宜的一类：配置完全自足、零凭证。新链为 需求 → fanout 扇出 3 种写法（`strategy: "prompt"`，prompts 条数须等于 count）→ lane textGen → select `llm_score` topK=1 按 rubric 择优 → 成稿入库。**这两半的错误只在运行时暴露**（fanout 缺下游 select、select 缺上游 fanout 都会 VALIDATION 失败，而 `compile()` 不报），所以 core 新增链式测试专门钉住「lane 严格夹在 fanout 与 select 之间」与 prompts 条数=count 两条不变量。模板数 33→34（core `TEMPLATES` 与 web TemplatePicker 计数守护同步），节点类型覆盖 23/29→25/29。⚠️ **尚未真实狗粮**：只过引擎级 compile + 链式守护，已在 [template-checklist.md](docs/template-checklist.md) 标 ⬜ 待验证。详见 [docs/design-templates.md](docs/design-templates.md)。
- **常驻模板运行时冒烟门禁 `packages/server/src/templates-runtime.test.ts`** — 此前模板只有 `compile()` 一道保底，而 `compile()` 接受引擎拒绝执行的图（fanout 缺下游 select 编译干净、用户第一次跑就 VALIDATION）。新测试分两层：**tier 1** 对全部 34 个模板扫运行期强制、编译不报的不变量（fanout↔select 配对、每泳道一条非空变体 prompt）；**tier 2** 用 fake worker **真执行** 13 个不碰外部世界的模板，断言 `run.finished=done`、零 `node.failed`、实例化后每个节点都 `node.finished`、至少一个产物（`tpl-variant-copy` 的 fanout→lane→select→sink 全链在内，因此它无需 provider key 就拿到执行级证据）。剩余 21 个必须在 `REQUIRES_EXTERNAL_IO` 逐条登记原因，且该表要与推导出的排除集**完全相等**——漏登记和理由过期同样直接红（它第一次跑就抓到一条未登记的 `file` 连接器排除）。**同时更正记录**：template-checklist 原写的「引擎级冒烟 33/33 全绿」是 2026-08-31 的一次性脚本、从来不是门禁，新增模板根本不会被它拦住；另记一条写测试时踩到的事实——`instantiateTemplate()` 会给节点 id 加随机后缀，完成断言必须对实例化后的图做，模板原始 id 永不出现在事件流里。
