...
function echoWorker(): Worker {
  return {
    async *runAgent({ input }): AsyncGenerator<AgentChunk, { output: string; usage: { tokensIn: number; tokensOut: number; costUsd: number } }> {
...
function linearGraph(policy?: AgentConfig["inputPolicy"]): Graph {
  return Graph.parse({
    id: "policy",
    name: "policy",
    nodes: [
