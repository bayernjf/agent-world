...
async function runScheduler(opts: SchedulerOptions): Promise<AsyncGenerator<RunEvent>> {
  ...
  const runGate = async (node: GraphNode, nodeId: string, attempt: number) => {
    emit({ type: "node.started", nodeId, attempt });
    const output = await inputFor(node);
  ...
  const runGeneric = async (node: GraphNode, nodeId: string, attempt: number) => {
    emit({ type: "node.started", nodeId, attempt });
  ...
  const runNode = async (nodeId: string) => {
    ...
    try {
      ...
      if (node.kind === "subprocess") {
        await runSubprocess(node, nodeId, attempt);
        return;
      }
      ...
      if (node.kind === "gate") {
        await runGate(node, nodeId, attempt);
        return;
      }
  ...
  // --- Generic node: auto-dispatches by user-picked modality ---
      if (node.kind === "generic") {
        await runGeneric(node, nodeId, attempt);
        return;
      }
      // agent
