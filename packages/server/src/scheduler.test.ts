...
async function makeService(graph: Graph) {
...
describe("TriggerScheduler", () => {
  ...
  it("computes the next run for a cron trigger", async () => {
    vi.setSystemTime(new Date("2026-01-01T00:00:00Z"));
    const trigger = TriggerConfig.parse({ id: "c3", type: "cron", cron: "0 0 * * 0" }); // Sundays
    const { service } = await makeService({ id: "g1", name: "G", nodes: [], edges: [], triggers: [trigger] });
  ...
  it("does not schedule disabled cron triggers", async () => {
    vi.setSystemTime(new Date("2026-01-01T00:00:00Z"));
    const trigger = TriggerConfig.parse({ id: "c2", type: "cron", cron: "0 0 * * *", enabled: false });
    const { service, calls } = await makeService({ id: "g1", name: "G", nodes: [], edges: [], triggers: [trigger] });
    const scheduler = new TriggerScheduler(service);
    scheduler.start();
  ...
  it("re-arms when a trigger is synced after an update", async () => {
    vi.setSystemTime(new Date("2026-01-01T00:00:00Z"));
    ...
    await service.upsert("g1", updated);
