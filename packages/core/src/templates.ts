...
const docIngestGraph = {
  ...
  fields: [
    {
      key: "docUrl",
      label: "文档链接（可选）",
      placeholder: "填入 PDF / DOCX 等文档的公开链接；也可直接在「文档入口」上传本地文件",
  ...
  graph: {
    ...
    edges: [
      ...
      // 文档解析（parse）。fileParse 节点只处理第一个 file 产物——
...
const reconciliationGraph = {
  id: "tpl-reconciliation",
  name: "银行流水对账",
  description: "银行流水 vs 企业账簿 → 逐笔配对 → 差异清单 → 对账报告 → 质检",
  category: "财务审计",
  graph: {
    id: "tpl-reconciliation",
    name: "银行流水对账",
    nodes: [
      { id: "intake", kind: "source", name: "流水投料台", x: 80, y: 300 },
      {
        id: "reconcile",
        kind: "code",
        name: "逐笔配对",
        ...
        code: {
          ...
          code: [
            ...
            "    var t = lines[i].trim();",
            '    if (t === "银行流水") { cur = bank; continue; }',
            '    if (t === "企业账簿") { cur = book; continue; }',
            "    if (!t) continue;",
            "    if (cur) cur.push(t);",
            "  }",
            "  return { bank: bank, book: book };",
            "}",
            '// 解析一行交易：日期 金额 摘要（日期归一化为 YYYY-MM-DD）',
            "function parseLine(line) {",
            '  var dm = line.match(/(\\d{4})\\s*[-/年]\\s*(\\d{1,2})\\s*[-/月]\\s*(\\d{1,2})/);',
            '  var date = dm ? dm[1] + "-" + ("0" + dm[2]).slice(-2) + "-" + ("0" + dm[3]).slice(-2) : "";',
            '  var rest = dm ? line.replace(dm[0], " ") : line;',
            '  var nums = rest.match(/\\d+(?:\\.\\d+)?/g) || [];',
            "  var amountNum = nums.length ? parseFloat(nums[0]) : NaN;",
            '  var amount = isNaN(amountNum) ? "" : amountNum.toFixed(2);',
            '  var memo = rest.replace(/\\d+(?:\\.\\d+)?/g, " ").replace(/[¥￥元]/g, " ").trim();',
            "  return { date: date, amountNum: amountNum, amount: amount, memo: memo };",
            "}",
            'function fmtKey(t) { return t.date + "|" + t.amount; }',
            "var sec = splitSections(raw);",
            "var bankTx = sec.bank.map(parseLine).filter(function (t) { return t.date && !isNaN(t.amountNum); });",
            "var bookTx = sec.book.map(parseLine).filter(function (t) { return t.date && !isNaN(t.amountNum); });",
            '// 逐个配对：按「日期+金额」匹配，匹配掉的从账侧移除，剩下的就是差异',
            "var bookMatched = bookTx.map(function () { return false; });",
            "var bankOnly = [];",
            "var matched = 0;",
            "for (var i = 0; i < bankTx.length; i++) {",
            "  var bt = bankTx[i];",
            "  var k = fmtKey(bt);",
            "  var found = -1;",
            "  for (var j = 0; j < bookTx.length; j++) {",
            '    if (!bookMatched[j] && fmtKey(bookTx[j]) === k) { found = j; break; }',
            "  }",
            "  if (found >= 0) { bookMatched[found] = true; matched++; }",
            '  else { bankOnly.push({ side: "银行有、账无", date: bt.date, amountNum: bt.amountNum, amount: bt.amount, memo: bt.memo }); }',
            "}",
            "var bookOnly = [];",
            "for (var m = 0; m < bookTx.length; m++) {",
            "  if (!bookMatched[m]) {",
            '    bookOnly.push({ side: "账有、银行无", date: bookTx[m].date, amountNum: bookTx[m].amountNum, amount: bookTx[m].amount, memo: bookTx[m].memo });',
            "  }",
            "}",
            "var rows = bankOnly.concat(bookOnly);",
            "var summary = {",
            "  bankCount: bankTx.length,",
            "  bookCount: bookTx.length,",
            "  matchedCount: matched,",
            "  diffCount: rows.length,",
            "};",
            '// 表格节点要求至少一行：完全无差异也给一行占位',
            "if (!rows.length) {",
            '  rows = [{ side: "无差异", date: "", amountNum: 0, amount: "", memo: "两方流水完全一致" }];',
            "}",
            "console.log(JSON.stringify({ rows: rows, summary: summary }));",
          ].join("\n"),
        },
      },
      {
        ...
        table: {
          steps: [
            { op: "sort", column: "amountNum", direction: "desc" },
            { op: "output", format: "json" },
          ],
        },
      },
      {
        ...
        textGen: {
          ...
          prompt:
            ...
            "②输出差异明细 Markdown 表：方向｜日期｜金额｜摘要，按金额从大到小；" +
            "③每条差异给出排查建议（银行有账无→查未达账项/漏记账；账有银行无→查在途款项/重复记账）；" +
            "④若 diffCount 为 0，明确写「对账结论：两方流水完全一致」。" +
            "要求：不得虚构上游不存在的交易；不得把已匹配的交易列为差异；金额以 rows 为准，不得推测。",
          skills: [],
        },
      },
      {
        ...
        gate: {
          maxAttempts: 2,
          criterion:
            "对账报告的总览统计与上游 summary 一致、差异明细覆盖上游 rows 的全部差异条目且无虚构、每条差异有排查建议、无差异时明确写「两方流水完全一致」。",
          onExhausted: "halt",
        },
      },
      { id: "depot", kind: "sink", name: "对账成品", x: 1380, y: 300 },
    ],
    edges: [
      { id: "e1", from: "intake", to: "reconcile", kind: "flow" },
      { id: "e2", from: "reconcile", to: "report", kind: "flow" },
      { id: "e3", from: "reconcile", to: "diffTable", kind: "flow" },
      { id: "e4", from: "report", to: "qc", kind: "flow" },
      { id: "e5", from: "qc", to: "depot", kind: "flow" },
      { id: "r1", from: "qc", to: "report", kind: "rework" },
    ],
...
const auditSamplingGraph = {
  id: "tpl-audit-sampling",
  name: "审计抽样底稿",
  description: "账目明细 → 抽样规则（大额/重复/非工作日）→ 抽样清单 → 审计底稿 → 质检",
  category: "财务审计",
  graph: {
    id: "tpl-audit-sampling",
    name: "审计抽样底稿",
    nodes: [
      { id: "intake", kind: "source", name: "账目明细台", x: 80, y: 300 },
      {
        id: "sample",
        kind: "code",
        name: "抽样规则",
        ...
        code: {
          ...
          code: [
            ...
            "  raw = String(Object.values(inputs)[0] ?? '').trim();",
            "} catch (e) { raw = ''; }",
            '// 解析交易行（CSV：日期,金额,科目,对方）',
            "var lines = raw.split(/\\r?\\n/).map(function (s) { return s.trim(); }).filter(Boolean);",
            '// 跳过表头（同时含「日期」与「金额」）',
            "var items = lines.filter(function (l) { return !(l.indexOf('日期') >= 0 && l.indexOf('金额') >= 0); })",
            "  .map(function (line, i) {",
            "    var f = line.split(/[,，\\t]/).map(function (s) { return s.trim(); }).filter(Boolean);",
            '    // 日期归一化 YYYY-MM-DD',
            "    var dm = line.match(/(\\d{4})\\s*[-/年]\\s*(\\d{1,2})\\s*[-/月]\\s*(\\d{1,2})/);",
            '    var date = dm ? dm[1] + "-" + ("0" + dm[2]).slice(-2) + "-" + ("0" + dm[3]).slice(-2) : "";',
            '    var rest = dm ? line.replace(dm[0], " ") : line;',
            "    var nums = rest.match(/\\d+(?:\\.\\d+)?/g) || [];",
            "    var amount = nums.length ? parseFloat(nums[0]) : NaN;",
            "    var rest2 = rest.replace(/\\d+(?:\\.\\d+)?/g, \" \");",
            '    // 科目：第一个含中文的字段；对方：最后一个字段',
            "    var category = '';",
            "    for (var k = 0; k < f.length; k++) {",
            "      if (/[\\u4e00-\\u9fa5]/.test(f[k]) && !/\\d{4}/.test(f[k])) { category = f[k]; break; }",
            "    }",
            "    var counterparty = f.length ? f[f.length - 1] : '';",
            "    return { no: i + 1, date: date, amount: amount, category: category, counterparty: counterparty };",
            "  });",
            '// 重复检测（同对方 + 同金额）',
            "var seen = Object.create(null);",
            'items.forEach(function (it) { var k = it.counterparty + "|" + it.amount; seen[k] = (seen[k] || 0) + 1; });',
            '// 抽样规则：大额（≥10 万）必查、重复交易必查、非工作日（周六日）重点',
            "var rows = items.map(function (it) {",
            "  var flags = [];",
            "  if (isNaN(it.amount) || it.amount === '') flags.push('金额缺失');",
            "  else if (it.amount >= 100000) flags.push('大额必查');",
            "  if (it.date) {",
            "    var d = new Date(it.date + 'T00:00:00');",
            "    var wd = d.getDay();",
            "    if (wd === 0 || wd === 6) flags.push('非工作日');",
            "  }",
            "  if (seen[it.counterparty + '|' + it.amount] > 1) flags.push('重复交易');",
            "  return {",
            "    no: it.no, date: it.date, amount: it.amount, category: it.category, counterparty: it.counterparty,",
            "    flags: flags.join('；') || '常规',",
            "    sampled: flags.length ? '是' : '否'",
            "  };",
            "});",
            '// 保证至少一行',
            "if (!rows.length) rows = [{ no: 0, date: '', amount: '', category: '', counterparty: '', flags: '未粘贴账目', sampled: '是' }];",
            "var summary = {",
            "  totalCount: rows.length,",
            "  sampledCount: rows.filter(function (r) { return r.sampled === '是'; }).length,",
            "  largeCount: rows.filter(function (r) { return r.flags.indexOf('大额') >= 0; }).length,",
            "  duplicateCount: rows.filter(function (r) { return r.flags.indexOf('重复') >= 0; }).length,",
            "  weekendCount: rows.filter(function (r) { return r.flags.indexOf('非工作日') >= 0; }).length",
            "};",
            "console.log(JSON.stringify({ rows: rows, summary: summary }));",
          ].join("\n"),
        },
      },
      {
        ...
        table: {
          steps: [
            { op: "sort", column: "amount", direction: "desc" },
            { op: "output", format: "json" },
          ],
        },
      },
      ...
      },
      {
        ...
        gate: {
          maxAttempts: 2,
          criterion: "审计底稿覆盖 summary 的全部统计、抽样清单与上游 rows 一致、重点行（大额/重复/非工作日）有核查要点、无虚构账目。",
          onExhausted: "halt",
        },
      },
      { id: "depot", kind: "sink", name: "底稿成品", x: 1380, y: 300 },
    ],
    edges: [
      { id: "e1", from: "intake", to: "sample", kind: "flow" },
      { id: "e2", from: "sample", to: "report", kind: "flow" },
      { id: "e3", from: "sample", to: "sampleTable", kind: "flow" },
      { id: "e4", from: "report", to: "qc", kind: "flow" },
      { id: "e5", from: "qc", to: "depot", kind: "flow" },
      { id: "r1", from: "qc", to: "report", kind: "rework" },
    ],
    ...
    nodes: [
      { id: "intake", kind: "source", name: "需求", x: 80, y: 300 },
      {
        ...
        fanout: {
          ...
          strategy: "prompt",
          prompts: [
            "用「痛点 → 方案 → 行动号召」结构，语气直接，少形容词。",
            "用「场景代入 → 细节描写 → 金句收尾」结构，口语化、有画面感。",
            "用「数字对比 → 反差结论 → 限时行动」结构，短句为主。",
          ],
        },
      },
      {
        ...
        textGen: {
          model: "agnes-2.0-flash",
          prompt:
            "你是电商投放文案助手。按给定需求和该泳道的写法要求写一条投放文案，不超过 120 字，只输出文案正文。",
          skills: [],
        },
      },
      {
